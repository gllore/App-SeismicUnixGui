...
!      next 7 lines
      ...
      if (previous_model == 0.0 .and. new_model == 1.0) then
       ...
       print *, 'readmmod, unexpected model answer'
      end if
...
110	INQUIRE(FILE=FIN,EXIST=EX)
        IF(EX) then
		write(*,*) ''
		write(*,*) 'Default file mmodpg.out exists.'
		write(*,*) ''
!      call read_par_i4('0- use it , 1- No',idef)
!              if(idef.eq.1) 117
              go to 117
	 else
	       print *, 'Default file mmodpg.out is missing!'
