...
sub Populate {
    ...
    foreach my $i ( keys %{ $args[0] } ) {
        ...
        if ( $i =~
/^\-(?:HistFile|History|QuickSelect|PathFile|HistDeleteOk|HistUsePath|HistUsePathButton|SortButton|SelDir)$/
          )
        {
    ...
    if ( $^O =~ /Win/i ) {
        ...
        $FDialog->protocol(
            'WM_DELETE_WINDOW' => sub {
                if ( defined( $FDialog->{'Can'} )
                    && $FDialog->{'Can'}->IsWidget )
                {
                    $FDialog->{'Can'}->invoke;
                }
            }
    ...
    $FDialog->{'inactivefore'} =
      $FDialog->{'SABox'}->cget( -disabledforeground );
    ...
    $FDialog->ConfigSpecs(
        ...
        , #ADDED 20050416 TO PERMIT MULTIFILE SELECTIONS, THANKS TO Paul Falbe FOR THIS PATCH!
        -ShowAll      => [ 'PASSIVE', undef, undef, 0 ],
        -Title        => [ 'PASSIVE', undef, undef, "Select File:" ],
        -EDlgTitle    => [ 'PASSIVE', undef, undef, 'File does not exist!' ],
...
sub Show {
    ...
    $self->{'FPat'}->configure( -state => ( $self->{Configure}{-DisableFPat} )
        ? 'disabled'
        : 'normal' );    #ADDED 20050126.
    ## Enable or disable the file entry box
    if ( $self->{Configure}{-SelDir} == 1 ) {
        $self->{Configure}{-File} = '';
        $self->{'FileEntry'}->configure(
            -state      => 'disabled',
            -foreground => $self->{'inactivefore'}
        );
        $self->{'FileList'}
          ->configure( -selectforeground => $self->{'inactivefore'} );
        $self->{'FileList'}
          ->configure( -foreground => $self->{'inactivefore'} );
    }
    ## Set the title
    ...
    if ( defined( $self->{Configure}{-Geometry} ) ) {
        ...
        my ( $x, $y );
        $x = int( ( $self->screenwidth - $self->reqwidth ) / 2 -
              $self->parent->vrootx );
        $y = int( ( $self->screenheight - $self->reqheight ) / 2 -
              $self->parent->vrooty );
        $self->geometry("+$x+$y");
    }
    ...
    select( undef, undef, undef, 0.1 )
      if ( $ENV{DESKTOP_SESSION} =~ /AfterStep version 2.2.1[2-9]/i )
      ; #JWT:ADDED FANCY SLEEP FUNCTION 20140606 B/C TO GET AFTERSTEP TO GIVE "TRANSIENT" WINDOWS THE FOCUS?!;
    my ($i) = 0;
    while ( !$i ) {
    ...
    #	$self->parent->focus();
    ( $self->{Configure}{-DestroyOnHide} == 1 )
      ? $self->destroy
      : $self->withdraw;
    return $self->{'RetFile'};
}
...
sub BuildListBox {
    ...
    $self->{"$fvar"} = $self->{'DFFrame'}->Frame->pack(
...
sub BindDir {
    ...
    $lbdir->bind(
        "<ButtonRelease-1>", sub    #CHGD. 20020122 TO MAKE SINGLE-CLICK CHDIR.
        {
    ...
    $self->{'DirEntry'}->bind(
        '<Key>' => [ \&keyFn, \$self->{Configure}{'Path'}, $self->{'DirList'} ]
    );
}
...
#PREV. CHGD. TO NEXT 20050416 TO PERMIT MULTIFILE SELECTIONS, THANKS TO Paul Falbe FOR THIS PATCH!
                $self->{Configure}{-File} = join ",", map {
                    ( my $f = $self->{'FileList'}->get($_) ) =~
                      s/^\d\d\d\d\d\d\d\d \d\d\d\d //o;
                    $f
...
#PREV. CHGD. TO NEXT 20050416 TO PERMIT MULTIFILE SELECTIONS, THANKS TO Paul Falbe FOR THIS PATCH!
    ...
    $self->{'FileEntry'}->bind( '<Key>' =>
          [ \&keyFn, \$self->{Configure}{'File'}, $self->{'FileList'} ] );
}
...
sub BuildEntry {
    ...
    if ( $LabelType eq 'Path'
      )    #NEXT 26 ADDED 20010130 TO ADD DRIVE-LETTER SELECTION IN WINDOZE!
    {
        if ($Win32) {
    ...
    $self->{"$entry"} =
      $eFrame->Entry( @sunken, -textvariable => \$self->{Configure}{$LabelVar} )
      ->pack( @leftPack, @expand, @xfill );
    if (
        $LabelType eq 'File'
        && ( !defined( $self->{Configure}{-SelDir} )
            || $self->{Configure}{-SelDir} != 1 )
      )
    {
        $self->{"$entry"}->bind(
            "<Return>",
        ...
        if ( $self->{Configure}{-SortButton} ) {
            $self->{'SortxButton'} = $eFrame->Checkbutton(
                -variable => \$self->{Configure}{-SortOrder},
                -onvalue  => 'Date',
                -offvalue => 'Name',
                -text     => 'Atime',
...
sub BuildBrowse {
    ...
    if ( $LabelVar eq '-Hist' ) {
        $self->{"histToggleVal"} =
          ( defined( $self->{Configure}{-HistUsePathButton} )
              && $self->{Configure}{-HistUsePathButton} == 1 ) ? 1 : 0;
        if (   $self->{Configure}{-HistUsePath}
            && $self->{Configure}{-HistUsePath} != 1 )
        {
            my $pathLabel = $self->{Configure}{-HistUsePath};
            $pathLabel = 'Keep Path'
              if ( $self->{Configure}{-HistUsePath} =~ /^\-?\d/ );
    ...
    $self->{"$entry"} = $eFrame->JBrowseEntry(
        @raised,
        -label         => '',
        -state         => 'readonly',
        ...
        -browsecmd     => sub {
            $self->{'OK'}->invoke
              unless ( !$self->{Configure}{-QuickSelect} or $_[2] =~ /space$/ );
            $self->{'FileEntry'}->delete( '0.0', 'end' );
            $self->{'FileEntry'}
              ->insert( 'end', $self->{Configure}{$LabelVar} );
            $self->{'FileEntry'}->focus;
            if (
                $self->{Configure}{-HistUsePath} == 1
                || (   $self->{Configure}{-HistUsePath}
                    && $self->{"histToggleVal"} )
              )
            {
                $self->{Configure}{-Path} = $self->{Configure}{$LabelVar};
                $self->{Configure}{-Path} =~ s#\/[^\/]+$##
                  unless ( -d $self->{Configure}{-Path} );
                $driveletter = $1
                  if ( $^O =~ /Win/i
                    && $self->{Configure}{-Path} =~ s/^(\w\:)// );
                &RescanFiles($self);
            }
...
sub BuildFAV {
    ...
    my ( $l, $r, $s, $dir );
    if ( $Win32
        && -d $self->{Configure}{-PathFile}
      )    #ADDED 20081029 TO ALLOW USAGE OF WINDOWS' "FAVORITES" (v1.4)
    {
    ...
    elsif ( $self->{Configure}{-PathFile}
        && open( TEMP, $self->{Configure}{-PathFile} ) )
    {
        while (<TEMP>) {
            chomp;
            if ($_) {
    ...
    $self->{"$entry"} = $eFrame->JBrowseEntry(
        ...
        -browsecmd => sub {
            $self->{Configure}{-Path} =
              $self->{$entry}->dereference( $self->{Configure}{$LabelVar} );
            &RescanFiles($self)
              unless ( !$self->{Configure}{-QuickSelect} or $_[2] =~ /space$/ );
            $self->{'FileEntry'}->focus;
        },
    ...
    $eFrame->packForget
      unless ( $self->{Configure}{-PathFile}
        && -r $self->{Configure}{-PathFile} );
    return $eFrame;
}
...
sub BuildListBoxes {
    ...
    &BuildListBox( $self, 'FileFrame', $self->{Configure}{-FileLBCaption},
...
sub BuildFDWindow {
    ...
    my $patFrame =
      $self->Frame->pack( -padx => '4m', -pady => '2m', @topPack, @xfill )
      ;    # !!!
    ...
    ## and the radio box
    $self->{'SABox'} = $patFrame->Checkbutton(
    ...
    $self->{'OK'} = $butFrame->Button(
        -command => sub {
            &GetReturn($self);
        }
    )->pack( -padx => '2m', -pady => '2m', @leftPack, @expand, @xfill );

    $self->{'Rescan'} = $butFrame->Button(
        -command => sub {
            &RescanFiles($self);
        }
    )->pack( -padx => '2m', -pady => '2m', @leftPack, @expand, @xfill );

    $self->{'Can'} = $butFrame->Button(
        -command => sub {
            $self->{'Retval'} = -1;
        }
    )->pack( -padx => '2m', -pady => '2m', @leftPack, @expand, @xfill );
...
sub RescanFiles {
    ...
    $self->{'Rescan'}->grab('release')
      if $self->grab('current') == $self->{'Rescan'};
...
sub add2Hist {
    ...
    if (   $self->{Configure}{HistList}
        && $self->{Configure}{-HistFile}
        && open( TEMP, ">$self->{Configure}{-HistFile}" ) )
    {
        ...
        while ( @{ $self->{Configure}{HistList} } ) {
            ...
            unless ( $t eq $fname ) {
                ...
                ++$i;
                last
                  if ( $self->{Configure}{-History}
                    && $i >= $self->{Configure}{-History} );
            }
        }
        close TEMP;
        if ( $self->{Configure}{-HistUsePath} == 1
            || ( $self->{Configure}{-HistUsePath} && $self->{"histToggleVal"} )
          )
        {
            ...
            $self->{Configure}{-Path} =~ s#\/[^\/]+$##
              unless ( -d $self->{Configure}{-Path} );
            $driveletter = $1
              if ( $^O =~ /Win/i && $self->{Configure}{-Path} =~ s/^(\w\:)// );
...
sub GetReturn {
    ...
    if ( $self->{Configure}{-Hist} ) {
        ...
        if ( $self->{Configure}{-SelDir} != 1 ) {
            ...
            elsif ($self->{Configure}{-SelDir} != 2
                || $self->{Configure}{-File} gt ' ' )
            {
                #WINNT: $fname = $path . $self->{Configure}{-File};
                $fname = $path
                  unless ( $self->{Configure}{-File} =~ /^[a-zA-Z]\:/ );
                $fname .= $self->{Configure}{-File};
            }
            ## Make sure that the file exists, if the user is not allowed
            ## to create
            if (  !$self->{Configure}{-Create}
                && $self->{Configure}{-File} gt ' '
                && !( -f $fname )
                && !( ( -d $fname ) && $self->{Configure}{-SelDir} ) )
            {
                ...
                $self->Dialog(
                    -title  => $self->{Configure}{-EDlgTitle},
                    -text   => $fname,
                    -bitmap => 'error'
                )->Show;
                return;
...
sub chgDriveLetter    #ADDED 20010130 BY JWT.
{
    ...
    $driveletter =~ tr/a-z/A-Z/;
    $driveletter = substr( $driveletter, 0, 1 ) . ':'
      if ( length($driveletter) >= 2 || $driveletter =~ /^[A-Z]$/ );
