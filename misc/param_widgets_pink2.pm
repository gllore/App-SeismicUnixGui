...
#	reader    => 'get_hash_ref',
#	writer    => 'set_hash_ref',
#	predicate => 'has_hash_ref',
#	trigger   => \&_update_hash_ref,
#);
...
sub set_hash_ref {
	...
	if ( $param_widgets_color_href->{_log_view} eq $true ) {
...
sub _changes {
	...
	# print(" 1. param_widgets_pink, _changes, entry_in_switch is $entry_in_switch\n");
	...
	if ( $param_widgets_color_href->{_current_program_name} ) {
		...
		if ( $entry_in_switch eq $off ) {    # =0
			...
			# print(" 2. param_widgets_pink, _changes, exit index=$ans ENTERED INDEX=$idx\n");
			...
			# print(" 3. param_widgets_pink, _changes, entry index=$ans LEFT INDEX=$idx \n");
			$param_widgets_color_href->{_parameter_index_on_exit} = $idx;

		}
		else {
			print(" param_widgets_pink, changes, bad switch \n");
		}
		...
		# print(" param_widgets_pink, _changes, _parameter_index_on_exit: $ans \n");
		# $ans= ($self->get_hash_ref())->{_parameter_index_on_entry};		
		# print(" param_widgets_pink, _changes, _parameter_index_on_entry: $ans \n");	
...
sub gui_full_clear {
	...
	# print("param_widgets_pink, gui_full_clear, length $param_widgets_color_href->{_length} \n");
	# print("param_widgets_pink, gui_clean, _values_w_aref, $param_widgets_color_href->{_values_w_aref} \n");
	# print("param_widgets_pink, gui_clean, _labels_w_aref, $param_widgets_color_href->{_labels_w_aref} \n");
	$wipe->range($param_widgets_color_href);
	$wipe->values();
...
sub get_check_buttons_aref {
	...
	# print("param_widgets_pink,get_check_buttons_aref: @{$param_widgets_color_href->{_check_buttons_settings_aref}}\n");
...
sub get_values_aref {
	...
	if ( $param_widgets_color_href->{_values_aref} ne $empty_string ) {
		...
		# print("param_widgets_pink,get_values_aref,value[0]= @{$param_widgets_color_href->{_values_aref}}[0]\n");
		return ($values_aref);
...
#   my $first 		= $param_widgets_color_href->{_first_idx};
#   my $length  		= $param_widgets_color_href->{_length};
#		     # print("1. param_widgets_pink,_checks,number of items in program: $length\n");
#		     # print("1. param_widgets_pink,_checks,recently current entry was index: $index\n");
#		    # print("1. param_widgets_pink,_checks,is new listbox selected? : $param_widgets_color_href->{_is_new_listbox_selection}\n\n");
#		   # print("1. param_widgets_pink,_checks,result of add2flow_button selected? : $param_widgets_color_href->{_is_add2flow_button}\n\n");
#
...
#   if ($param_widgets_color_href->{_is_new_listbox_selection} || $param_widgets_color_href->{_is_add2flow_button} || $param_widgets_color_href->{_is_moveNdrop_in_flow}) {
#
#					# work through a program's entries
#	 if( $index == ($length-1) ) {      				# at last  entry (via local subs: redisplay or gui_update)
#	    $param_widgets_color_href->{_is_new_listbox_selection} = $false;
#	    $param_widgets_color_href->{_is_add2flow_button} 		= $false;
#										  # print("2. param_widgets_pink,_checks, at last item#; # $length\n\n");
#   	    return($false);  # false change detected
#     } elsif( $index < ($length-2) ) {  #unlikely case but answer is still false
#	     $param_widgets_color_href->{_is_new_listbox_selection} = $true;
#	      $param_widgets_color_href->{_is_add2flow_button} 	 = $true;
#										   # print("3. param_widgets_pink,_checks,new flow selection, idx=$index\n\n");
#   	    return($false);  # false change detected
#     }
#   } elsif ($param_widgets_color_href->{_is_delete_from_flow_button} || $param_widgets_color_href->{_is_sunix_listbox} ) {
#   	    return($false);	# do nothing, false change detected
#
#   } else {  # By exclusion of everything we can ONLY be but truly CHANGING entries
#	    								   print("4. param_widgets_pink,_checks,leaving index:  $index\n\n");
#   	    								  print("4. param_widgets_pink,_checks, value just changed to: @{$param_widgets_color_href->{_values_aref}}[$index]\n");
...
sub range {
	my ( $self, $ref_hash ) = @_;
	my $key = '_param_sunix_first_idx';
	# print(" param_widgets_pink2,range, key is $key, value is $ref_hash->{$key}\n");
...
sub set_labels {
	...
	# my $length = scalar @{ $param_widgets_color_href->{_labels_aref} };
	# print("param_widgets_pink2,set_labels, length=$length\n");
	return ();
}
