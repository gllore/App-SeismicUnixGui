...
my $param_widgets   = param_widgets_pink2->new();
...
sub _add2flow {
	...
	$conditions_gui->set_gui_widgets($color_flow_href);     # 26 used  / 114 in
	$conditions_gui->set_hash_ref($color_flow_href);        # 62 used / 114 in
	$conditions_gui->set4start_of_add2flow($flow_color);    #  set and 10 widgets configured
	$color_flow_href = $conditions_gui->get_hash_ref();     # 88 returned
	...
	# my $length = scalar @{$color_flow_href->{_values_aref}};
	...
	$conditions_gui->set_gui_widgets($color_flow_href);    # includes _values_w_aref; 29 used / 114 in
	$conditions_gui->set_hash_ref($color_flow_href);       # 70 used / 114 in
	...
	$color_flow_href = $conditions_gui->get_hash_ref();
	...
	$color_flow_href->{_is_last_parameter_index_touched_pink} = $true;
...
sub _FileDialog_button {
	...
	if ($option_sref) {    # e.g., can be 'Data' or 'Flow'
		...
		$color_flow_href->{_is_last_flow_index_touched_pink} = $true;
...
=head2  _check4flow_changes
	...
	if ($param_widgets->get_entry_change_status && $color_flow_href->{_last_flow_index_touched} >= 0) {		
    	  foreach my $key (sort keys %$color_flow_href) {
...
sub _check4flow_changes {
	...
	# print("1 color_flow _check4flow_changes, last_flow_color is:  $color_flow_href->{_last_flow_color} \n");
	# print("1 color_flow _check4flow_changes, current flow_color is:  $color_flow_href->{_flow_color} \n");
	# print("1 color_flow _check4flow_changes, _last_parameter_index_touched_color ($this_color) is:  $color_flow_href->{_last_parameter_index_touched_color} \n");
	# print("1 color_flow _check4flow_changes, _last_flow_index_touched is:  $color_flow_href->{_last_flow_index_touched} \n");

	# both must be defined but can = 0
	# a flow in the pink box must have been touched or a parameter value in the flow must have been touched
	# < -1 does exist and means the flow/parameter was not previously touched
	if (   $color_flow_href->{_last_flow_index_touched} >= 0
		or $color_flow_href->{_last_parameter_index_touched_color} >= 0 )
	{
		# prior flow must have the same color as the current one or 
		# we have just clicked an sunix program (neutral-flow case)

		if (   $color_flow_href->{_last_flow_color} eq 'neutral'
			or $color_flow_href->{_last_flow_color} eq $color_flow_href->{_flow_color}
			...
			)
		{
			...
			# print("7\n");
			# $param_flow_color_pkg->view_data();
			my $last_flow_index_touched = $color_flow_href->{_last_flow_index_touched};
			...
			# FOR EXPORT
			$last_flow_color = $color_flow_href->{_flow_color};    # reset to working only in current flow color
...
sub _check4parameter_changes {
	...
	if ( $color_flow_href->{_last_parameter_index_touched_color} >= 0 ) {
		...
		my $last_parameter_idx_touched = $color_flow_href->{_last_parameter_index_touched_color};

		# print("color_flow _check4parameter_changes,last changed entry index was $last_parameter_idx_touched \n");
		# print("color_flow _check4parameter_changes current program name in use: ${$color_flow_href->{_prog_name_sref}}\n");
		...
		if ( $current_flow_listbox_index >= 0 ) {    # >=0 11-14-2018 JML
			...
			# print("2. color_flow, _check4parameter_changes, last_flow_index_touched $color_flow_href->{_last_flow_index_touched}\n");
			# $param_flow_color_pkg->view_data();
			...
			my $last_idx_chng = $color_flow_href->{_last_flow_index_touched};
...
sub _flow_select {
	...
	$gui_history->set_color($this_color);
	$gui_history->set_button('flow_select');
	
	flow_select();
	
	if ( $color_flow_href->{_log_view} == $true ) {
		
		my $ans = $color_flow_href->{_log_view};
		print("color_flow,_flow_select: ->get_log_view:$ans \n");
		$gui_history->view();
...
sub _get_flow_listbox_color_w {
	...
	if ( $flow_color eq $this_color ) {
		...
		$this_flow_listbox_color_w = $color_flow_href->{_flow_listbox_pink_w};
...
sub _SaveAs_button {
	...
	if ( $topic eq 'SaveAs' ) {
		...
		$conditions_gui->set_hash_ref($color_flow_href);       # 57 used of 114 in
		$conditions_gui->set_gui_widgets($color_flow_href);    # 26 used of 114 in
		...
		$color_flow_href = $conditions_gui->get_hash_ref();           # 79 out
...
sub _perl_flow {
	...
	for ( my $prog_idx = 0; $prog_idx < $num_prog_names; $prog_idx++ ) {
		...
		# my $values = scalar @{$color_flow_href->{_values_aref}};
...
sub _set_flow_listbox_color_w {
	...
	if ( $flow_color eq $this_color ) {
		...
		$color_flow_href->{_flow_listbox_color_w} = $color_flow_href->{_flow_listbox_pink_w};
...
sub _set_flow_name_color_w {
	...
	if ( $flow_color eq $this_color ) {
		...
		$flow_name_color_w = $color_flow_href->{_flow_name_pink_w};
...
sub _set_flowNsuperflow_name_w {
	...
	if ( $flowNsuperflow_name && $flowNsuperflow_name_w ) {
		...
		$color_flow_href->{_flowNsuperflow_name_w} = $flowNsuperflow_name_w;
...
sub _set_is_last_parameter_index_touched_color {
	...
	if ( $flow_color eq $this_color ) {
		...
		# not for export
		$color_flow_href->{_is_last_parameter_index_touched_color} = $color_flow_href->{_is_last_parameter_index_touched_pink};

		print("color_flow,_set_is_last_parameter_index_touched_color, is_last_parameter_index_touched_pink: $color_flow_href->{_is_last_parameter_index_touched_pink}\n");
...
sub _set_last_parameter_index_touched_color {
	...
	if ( $flow_color eq $this_color ) {
		...
		# not for export
		$color_flow_href->{_last_parameter_index_touched_color} = $color_flow_href->{_last_parameter_index_touched_pink};

		print("color_flow,_set_last_parameter_index_touched_color, last_parameter_index_touched_pink: $color_flow_href->{_last_parameter_index_touched_pink}\n");
...
=head2 sub _stack_flow
  ...
  print("color_flow stack_flow,last left listbox flow program touched had index = $color_flow_href->{_last_flow_index_touched}\n");
  print("color_flow stack_flow,values= @{$color_flow_href->{_values_aref}}\n");
...
sub FileDialog_button {
	...
	if ($dialog_type_sref) {
		...
		if ( $topic eq $file_dialog_type->{_SaveAs} ) {
			...
			$file_dialog->set_flow_color( $color_flow_href->{_flow_color} );    # uses 1/ 114 in
			...
			#			print("color_flow,FileDialog_button,_is_flow_listbox_pink_w:	$color_flow_href->{_is_flow_listbox_pink_w}\n");
			_check4parameter_changes();
				...
				|| $color_flow_href->{_path} eq '' )
			{
				...
				# displays user-built flow name at top of pink-flow gui
			...
			if ($file_exists) {
				...
				# Place names of the programs at the head of the pink listbox
				$flow_name_color_w->configure( -text => $color_flow_href->{_flow_name_in} );

				# $color_flow_href->{_flow_name_pink_w}     -> configure(-text => $color_flow_href->{_flow_name_in});
			...
			#			print("3 color_flow,FileDialog_button,_is_flow_listbox_pink_w:	$color_flow_href->{_is_flow_listbox_pink_w}\n");
...
=head2 sub add2flow_button
	...
	namespace
  		...
  		log_view is not set by conditions_gui and returns unscathed!
$ans = $color_flow_href->{_log_view};
...
sub add2flow_button {
	...
	# print("0. color_flow, add2flow_button,color_flow_href->{_log_view} = $ans\n");
	...
	$gui_history->set_gui_widgets($color_flow_href);
	...
	# print("1. color_flow, add2flow_button,color_flow_href->{_log_view} = $ans\n");
	...
	# $gui_history->set_location('add2flow_button');
	$param_widgets->set_location_in_gui($here);
	...
	$param_widgets->set_values_w_aref( $color_flow_href->{_values_w_aref} );
	$param_widgets->set_check_buttons_w_aref( $color_flow_href->{_check_buttons_w_aref} );
	my $first_idx = $color_flow_href->{_param_sunix_first_idx};
	...
	$gui_history->set_flow_index_last_touched($flow_index);    # flow color is not reset
	$color_flow_href = $gui_history->get_hash_ref();
	...
	#	$ans = $color_flow_href->{_log_view};
	#		print("color_flow,add2flow_button: ->get_log_view:$ans \n");
	#
	#	if ( $color_flow_href->{_log_view} == $true ) {
	#
	#		$gui_history->view();
...
sub delete_from_flow_button {
	...
	if ($flow_color) {
		...
		$conditions_gui->set_hash_ref($color_flow_href);                       # 57 used  /114 in
		$conditions_gui->set_gui_widgets($color_flow_href);                    # 26 used  /114 in
		$conditions_gui->set4start_of_delete_from_flow_button($flow_color);    # 3 set /  72 max
		$color_flow_href = $conditions_gui->get_hash_ref();                    # find out if delete button is active    64 returned / 72 max
		...
		if ($pre_req_ok) {
			...
			$conditions_gui->set_hash_ref($color_flow_href);
			$conditions_gui->set_gui_widgets($color_flow_href);
			...
			if ( $index == 0 && $param_flow_color_pkg->get_num_items == 1 ) {
				...
				# @{$color_flow_href->{_check_buttons_settings_aref}}, index;$index\n#");
...
sub drag {
	...
	# 	if( $prog_name && $flow_widgets->is_drag_deleted($flow_listbox_pink_w) ) {
	#
	#			print("2.color_flowdrag; prior index was deleted\n");
	#        my $this_index = $flow_widgets->get_drag_deleted_index($flow_listbox_pink_w);
	#
	...
	my $num_items = $flow_widgets->get_num_items($flow_listbox_pink_w);

	if ( $num_items > 1 ) {    # num_items PRIOR to deletion
		$flow_widgets->drag_start($dnd_token_pink);
...
sub drop {
	my ($self) = @_;
	my $done;
	my $chosen_index_to_drop = $flow_widgets->drag_end($dnd_token_pink);
	my $prog_name;
	...
	if ( $prog_name && $chosen_index_to_drop && $chosen_index_to_drop >= 0 ) {    # same as destination_index
		...
		# highlight new index
		_set_flow_listbox_color_w($flow_color);    # in "color"_flow namespace
		$flow_listbox_color_w->selectionSet( $color_flow_href->{_destination_index}, );

		#$flow_listbox_pink_w    ->selectionSet( $color_flow_href->{_destination_index},);
...
If so, delete stored values from param_flow
  ...
  if($color_flow_href->{_prog_name_sref}) {
 		print("\n1. color_flow flow_select, program name is $color_flow_href->{_prog_name_sref}\n");
 	}
...
last_parameter_index_touched = 0  
    ...
    Important variables:
      $color_flow_href->{_prog_name_sref}  : selected program name as a scalar reference
...
sub flow_select {
	...
	# my $ans = $color_flow_href->{_log_view};
	# print("0. color_flow,flow_select, log_view: $ans\n");
	...
	$gui_history->set_color($flow_color);
	...
	$gui_history->set_gui_widgets($color_flow_href);
	$gui_history->set_hash_ref($color_flow_href);
	$gui_history->set4start_of_flow_select($flow_color);
	$color_flow_href = $gui_history->get_hash_ref();
...
=cut
		...
		# print( "1 color_flow , flow_select, last_flow_color is:  $color_flow_href->{_last_flow_color}\n" );
		# print( "1 color_flow , flow_select, flow_color is:  $color_flow_href->{_flow_color}\n" );
		# print("1 color_flow , flow_select, this flow color is:  $this_color\n");
		# print( "1 color_flow , flow_select, _is_last_flow_index_touched_grey:  $color_flow_href->{_is_last_flow_index_touched_grey}\n" );
		# print( "1 color_flow , flow_select, _is_last_flow_index_touched_gpink:  $color_flow_href->{_is_last_flow_index_touched_pink}\n" );
		# print( "1 color_flow , flow_select, _is_last_flow_index_touched_green:  $color_flow_href->{_is_last_flow_index_touched_green}\n" );
		# print( "1 color_flow , flow_select, _is_last_flow_index_touched_blue:  $color_flow_href->{_is_last_flow_index_touched_blue}\n" );
		# print( "1 color_flow , flow_select, last_flow_index_touched:  $color_flow_href->{_last_flow_index_touched}\n" );
	}
	...
	if ($pre_req_ok) {
		...
		# $param_widgets->focus_on(0); # put focus on first entry widget, index=0
		...
		# _check4parameter_changes(); # is only active if $color_flow_href->{_last_parameter_index_touched_color} >= 0
		...
		$prog_name_sref = $color_flow_href->{_prog_name_sref};

		# will not reflect the Entry focus change
		if ( $color_flow_href->{_log_view} eq $true ) {
...
sub save_button {
	...
	if ( $topic eq 'Save' ) {
		...
		my $last_flow_index = $color_flow_href->{_last_flow_index_touched};
		$last_flow_color = $color_flow_href->{_last_flow_color};
		...
		if (   $last_flow_color
			...
			&& $flow_listbox_color_w )
		{
			# print("CASE 1 pink_flow, save_button \n");
			...
			print("pink_flow, save_button , unexpected missing variables NADA\n");
		}
...
=cut
		...
		# $conditions_gui				-> set_gui_widgets($color_flow_href);  # uses 29/  114 in
		# $conditions_gui				-> set_hash_ref($color_flow_href);     # uses 70/  114 in
...
=head2 sub set_hash_ref
	...
	accessory
 	...
 	local extra  
 	sest_log_view =1 after setting defaults
 	because log_view is defaulted normally to false
 	
$ans = $color_flow_href->{_log_view};
print("3. color_flow,set_hash_ref: ->get_log_view:$ans \n");
my $ans = $hash_ref->{_log_view};
...
sub set_hash_ref {
	...
	$Data_menubutton               = $color_flow_href->{_Data_menubutton};
	$Flow_menubutton               = $color_flow_href->{_Flow_menubutton};
	$FileDialog_sub_ref            = $color_flow_href->{_FileDialog_sub_ref};
	$FileDialog_option             = $color_flow_href->{_FileDialog_option};
	$SaveAs_menubutton             = $color_flow_href->{_SaveAs_menubutton};
	# $add2flow_button_grey          = $color_flow_href->{_add2flow_button_grey};
	# $add2flow_button_pink          = $color_flow_href->{_add2flow_button_pink};
	# $add2flow_button_green         = $color_flow_href->{_add2flow_button_green};
	# $add2flow_button_blue          = $color_flow_href->{_add2flow_button_blue};
	# $check_code_button             = $color_flow_href->{_check_code_button};
	$check_buttons_w_aref          = $color_flow_href->{_check_buttons_w_aref};
	$delete_from_flow_button       = $color_flow_href->{_delete_from_flow_button};
	$dialog_type                   = $color_flow_href->{_dialog_type};
	$dnd_token_grey                = $color_flow_href->{_dnd_token_grey};
	$dnd_token_pink                = $color_flow_href->{_dnd_token_pink};
	$dnd_token_green               = $color_flow_href->{_dnd_token_green};
	$dnd_token_blue                = $color_flow_href->{_dnd_token_blue};
	$dropsite_token_grey           = $color_flow_href->{_dropsite_token_grey};
	$dropsite_token_pink           = $color_flow_href->{_dropsite_token_pink};
	$dropsite_token_green          = $color_flow_href->{_dropsite_token_green};
	$dropsite_token_blue           = $color_flow_href->{_dropsite_token_blue};
	$file_menubutton               = $color_flow_href->{_file_menubutton};
	$flow_color                    = $color_flow_href->{_flow_color};
	$flow_item_down_arrow_button   = $color_flow_href->{_flow_item_down_arrow_button};
	$flow_item_up_arrow_button     = $color_flow_href->{_flow_item_up_arrow_button};
	$flow_listbox_grey_w           = $color_flow_href->{_flow_listbox_grey_w};
	$flow_listbox_pink_w           = $color_flow_href->{_flow_listbox_pink_w};
	$flow_listbox_green_w          = $color_flow_href->{_flow_listbox_green_w};
	$flow_listbox_blue_w           = $color_flow_href->{_flow_listbox_blue_w};
	$flowNsuperflow_name_w         = $color_flow_href->{_flowNsuperflow_name_w};
	$flow_name_grey_w              = $color_flow_href->{_flow_name_grey_w};
	$flow_name_pink_w              = $color_flow_href->{_flow_name_pink_w};
	$flow_name_green_w             = $color_flow_href->{_flow_name_green_w};
	$flow_name_blue_w              = $color_flow_href->{_flow_name_blue_w};
	$flowNsuperflow_name_w         = $color_flow_href->{_flowNsuperflow_name_w};
	$gui_history_aref              = $color_flow_href->{_gui_history_aref};
	$is_pre_built_superflow        = $color_flow_href->{_is_pre_built_superflow};
	$is_user_built_flow            = $color_flow_href->{_is_user_built_flow};
	$labels_w_aref                 = $color_flow_href->{_labels_w_aref};
	$last_flow_color               = $color_flow_href->{_last_flow_color};                 # used in flow_select
	                                                                                       # $log_view					 = $color_flow_href->{_log_view};
	                                                                                       #	$lightning_button = $color_flow_href->{_lightning_button};
	$message_w                     = $color_flow_href->{_message_w};
	$mw                            = $color_flow_href->{_mw};
	$parameter_names_frame         = $color_flow_href->{_parameter_names_frame};
	...
	$prog_name_sref                = $color_flow_href->{_prog_name_sref};
	$save_button                   = $color_flow_href->{_save_button};
	$sunix_listbox                 = $color_flow_href->{_sunix_listbox};
	$values_w_aref                 = $color_flow_href->{_values_w_aref};
