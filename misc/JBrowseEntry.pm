...
L<Tk::BrowseEntry> L<Tk::HListbox> L<Tk::Listbox> L<Tk::JComboBox> L<Tk::Entry>
...
BEGIN
{
	...
	use Tk;
	use Tk::Frame;
	eval 'use Tk::HListbox; $haveHListbox = ($Tk::HListbox::VERSION >= 2.1) ? 1 : 0; 1';
...
sub Populate
{
	...
	# print("952JBrowseEntty, width= $args->{'-width'}\n");
	# print("953JBrowseEntty, maxwidth= $args->{'-maxwidth'}\n");
...
# modified by Juan Lorenzo 11-27-20
$w->{'-width'} = $w->{'-maxwidth'}  if (defined($w->{'-maxwidth'}) && $w->{'-maxwidth'} > 0 );	
	my $w_width = (defined $w->{'-width'}) ? $w->{'-width'} : 0;  #NEEDED B/C PERL'S BECOME A NANNY-STATE W/WARNINGS!
	my $width = $w_width;
	 # print("2354JBrowseEntty, width= $w->{'-width'}\n");
	# print("2354JBrowseEntty, maxwidth= $w->{'-maxwidth'}\n");
	unless ($width)
	{
		$width = 0;
		for (my $i=0;$i<=$#l;$i++)
