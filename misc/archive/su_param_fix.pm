...
=head2 CHANGES and their DATES
	...
	June 2022 V 0.3 is a copy of su_param
	to prevent circularity
	with a reference to L_SU_local_user_constants
	removed
...
sub _set_sub_category_directory {
	...
	if ( defined $sub_category_directory
		...
		print("su_param, _set_sub_category_directory, $sub_category_directory\n");
	}
...
#	print("su_param, get_program_config, program_sref= $$program_sref\n");
	...
	if (   defined $path
		...
		$program_config = $path . '/' . $sub_category_directory . '/' . $$program_sref . '.config';
...
sub get {
	...
	if (   defined $program_sref
		...
		use LSeismicUnix::misc::developer;
...
sub _check4user_config {
	...
	return ($ans);    #=head2 sub check4local_config
}
