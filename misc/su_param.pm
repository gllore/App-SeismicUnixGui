...
sub _set_perl_category_directory {

	my ($perl_category_directory) = @_;

	if ( defined $perl_category_directory
		&& $perl_category_directory ne $empty_string )
	{

		$su_param->{_perl_category_directory} = $perl_category_directory;

	}
	else {
		print("su_param, _set_perl_category_directory, missing value\n");
		print("su_param, _set_perl_category_directory, $perl_category_directory\n");
	}
...
sub _get_program_config {
	...
	my ( $program_config, $perl_category_directory, $program_sref );
	
	$path 					 = $su_param->{_path };
	$perl_category_directory = $su_param->{_perl_category_directory};
	$program_sref 			 = $su_param->{_program_sref };

	if (  defined $path
		&& $path ne $empty_string
		&& defined $perl_category_directory
		&& $perl_category_directory ne $empty_string
...
sub get {
	...
	if ( defined $program_sref ) {
		...
		my $perl_category_directory = $developer->get_program_category();
		# print("su_param, get,perl_category_directory: $perl_category_directory\n");
	   ...
	   #			 print("su_param, get,user_config_exists: $user_config_exists\n");
		...
		if ($local_config_exists) {    # within $PL_SEISMIC and Legacy code
			use Project_config;
 ...
 # print("1.2 su_param,get,PL_SEISMIC path is now $su_param->{_local_path} \n");
		...
		{    # ./L_SU/configuration/active/Project.config
			...
			# print("2.2 su_param,get,active path is now $su_param->
			# {_user_config_path} \n");
			...
			# print("3.2 su_param,get,global lib path is now $su_param->
			# {_lib_path} \n");
		}

		# share local variables with the package namespace
		_set_path($path);
		_set_perl_category_directory($perl_category_directory);
