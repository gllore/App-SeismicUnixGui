...
sub _pre_built_superflow_close_path {
	...
	if ($pre_req_ok) {
		...
		if (    defined( $file_dialog->{_last_path_touched} )
			...
			if ( ${ $file_dialog->{_prog_name_sref} } eq 'Sucat' ) {

				$result = $file_dialog->{_last_path_touched};    # no abbreviation

			} elsif ( ${ $file_dialog->{_prog_name_sref} } ne 'Sucat' ) {
...
sub _user_built_flow_close_path {
	...
	if ($pre_req_ok) {
		...
		if ( defined( $file_dialog->{_last_path_touched} )
			...
			# only keep the last directory name
			...
			print(
				"4. file_dialog,_user_built_flow_close_path,last path touched was
			$file_dialog->{_last_path_touched}\n"
			);
		}
		...
		#1. collect parameter widget values
		$file_dialog->{_values_aref} = $param_widgets->get_values_aref();
		# print("2. file_dialog  _user_built_flow_close_path, selected_Entry_widget: $selected_Entry_widget\n");
		# 2. file_dialog,_user_built_flow_close_path values aref= @{$file_dialog->{_values_aref}}\n");

		# 2. assign new path name to the array of values
		@{ $file_dialog->{_values_aref} }[$current_index] = $result;
...
##		my $selected_Entry_widget = $file_dialog->{_parameter_values_frame}->focusCurrent;
		...
		#
			@{ $file_dialog->{_check_buttons_settings_aref} }[$current_index] = $on;
		#
		#		sumute	# print("2. file_dialog,_user_built_flow_close_data_file, name after control = $file_dialog->{_selected_file_name}\n");
		#
		#		} else {
		#
		#			# print("file_dialog,user_built_flow_close_data_file No file was selected\n");
...
sub _user_built_flow_open_path {
	...
	if ( $widget_type eq 'Entry' ) {
		...
		if ( $file_dialog->{_parameter_value_index} >= 0 ) {    # for additional certainty; but is it needed?
			...
			print("1.file_dialog,_user_built_flow_path, PATH:  $file_dialog->{_path} \n");
			print("1.file_dialog,_user_built_flow_path, _values_aref: @{$file_dialog->{_values_aref}}[0]\n");
