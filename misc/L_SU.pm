	...
	if ( defined $dialog_type_sref ) {
		...
		if (   $$dialog_type_sref eq 'Flow'
			...
			if ( $vacancy eq $true ) {
				...
				print("CASE 2 1.L_SU,FileDialog_button, $next_available_color will be occupied\n");

			} elsif ( $vacancy eq $false ) {

				my $which_color = $color_listbox->get_flow_listbox_vacancy_color();

				print("CASE 2 1.L_SU,FileDialog_button, $which_color is available\n");
				...
				if ( $ans == $true ) {
					...
					if (   length $L_SU_gui->{_my_dialogs_ans4cancel}
						...
						if ( $L_SU_gui->{_my_dialogs_ans4ok} eq $yes ) {
							...
							# CASE 2B.2
							# User has another go
							# occupancy is cancelled and new vacancy is set
		...
		if (
			...
			print(
				"CASES 3A-D L_SU,FileDialog_button, L_SU_gui->{_occupied_listbox_aref},@{$L_SU_gui->{_occupied_listbox_aref}}\n"
			);
			print(
				"CASES 3A-D L_SU,FileDialog_button, L_SU_gui->{_vacant_listbox_aref},@{$L_SU_gui->{_vacant_listbox_aref}}\n"
			);
