...
sub drag_end {
    ...
    if ( $nearest < 0 ) {
        ...
        $max_y_listbox_inuse =
          $last_idx * ( $height_item + $min_y_listbox_inuse ) +
          $min_y_listbox_inuse;

        print(
" 3 flow_widgets, drag_end, destination item's x,y dx, dy in lb coords\n"
        );
        print(
" 4 Destination box xy values in lb coords xy[0]:$xy[0], xy[1]:$xy[1], xy[2]:$xy[2], xy[3]:$xy[3]  \n"
        );
        print(
" 5 Destination lbox L-R boundaries, lb coord xmin: $xmin_destination_box xmax: $xmax_destination_box \n"
        );
        print(
" 6 Top-Bot boundaries destination box        ymin:$ymin_destination_box ymax:$ymax_destination_box \n"
        );
        print(
" 7 YMin YMax box area of lb in use: $min_y_listbox_inuse, $max_y_listbox_inuse \n"
        );
...
#             $site->insert( $nearest, $text );
        ...
        {
            print(
"8. within Top_Bot boundaries in lb coord: $min_y_listbox_inuse $max_y_listbox_inuse\n"
            );
            $site->insert( $nearest, $text );
            ...
            if ( $nearest < 0 ) {    # CASE 0
                ...
                print(
"9.flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n"
                );
                ...
                $flow_widgets->{_chosen_index2drop} = $final_insertion_point;
                print(
"10 flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n"
                );
                ...
                $flow_widgets->{_chosen_index2drop} = $final_insertion_point;
                print(
"11.flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n"
                );
                ...
                $flow_widgets->{_chosen_index2drop} = $final_insertion_point;
                print(
"12.flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n"
                );
            ...
            $flow_widgets->{_chosen_index2drop} = $final_insertion_point;
            print(
"13.flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n"
            );
...
sub drag_start {
    ...
    my $site = $token->parent;
    my $e    = $site->XEvent;
    my $idx  = $site->index( '@' . $e->x . ',' . $e->y )
      ;    # arrow start location just B4 drag
...
# print("flow_widgets,get_current_program, selection index is @selection_index \n");
   ...
   # print("1. flow_widgets  get_current_program program name is $prog_name\n");
            ...
            print(
"flow_widgets,get_current_program, unexpected selection index:@selection_index \n"
            );
            return ();
        }
...
# print("2. flow_widgets,get_current_program, flow_num_items=$flow_num_items\n");
        ...
        if ( $flow_num_items > 0 ) {
            my $index = $flow_num_items -
              1;    # unfounded assumption.. possible future error
            ...
            print(
"flow_widgets,get_current_program, unexpected num_items=$flow_num_items\n"
            );
...
# print("flow_widgets,get_flow_selection,selection_index is missing, need to select an item again NADA\n");
        ...
        print(
            "flow_widgets,get_flow_selection, missing flow_listbox widget \n");
        return ();
    }
