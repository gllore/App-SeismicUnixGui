...
#	print("number of continuation lines = $num_conts\n");
	...
	for($i=1; $i <= $num_conts; $i++) {
	    ...
	    $num_new_values = $$ref_length[$local_row +3];
            ...
            for ($i= $offset, $j=2; $i <= $offset + $num_new_values -1; $i++,$j++) {

                $$ref_line_array[$local_row-1][$i]  = $$ref_line_array[$local_row + 2][$j];
                $$ref_line_array[$local_row][$i]    = $$ref_line_array[$local_row + 3][$j];
                $$ref_line_array[$local_row+1][$i]  = $$ref_line_array[$local_row + 1][$j];
...
#		print("$$ref_line_array[$local_row+1][$i]\n");
	...
	for ($j=1; $j <= $num_conts; $j++) {
		...
		for ($i =$local_row ; $i <= $num_rows - 3;  $i=$i+3) {
			...
			$$ref_line_array[$i +1][$k] = $$ref_line_array[$local_row + 3][$k];
			$$ref_line_array[$i +2][$k] = $$ref_line_array[$local_row + 4][$k];
			$$ref_line_array[$i +3][$k] = $$ref_line_array[$local_row + 5][$k];
	...
	$new_num_rows1 = $num_rows - ($num_conts *3);
return ();
}
...
#  open(OUT,">$$ref_origin") || die ("Can't open file_name, $!\n");
