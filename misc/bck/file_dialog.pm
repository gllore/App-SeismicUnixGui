...
=head2 CHANGES and their DATES
 ...
 V 0.0.2 refactoring of 2017 version of L_SU.pl
	...
	# print("1. file_dialog,_pre_built_superflow_close_data_file, pre_req_ok= $pre_req_ok \n");
   	...
   	if ( $pre_req_ok ) { 	
		...
		if( $file_dialog->{_selected_file_name}) {
  			...
  			print("file_dialog,_pre_built_superflow_close_data_file selected_file_name: $file_dialog->{_selected_file_name} \n");
		...
		$conditions_gui				-> set4superflow_close_data_file_end();
   		$file_dialog				 = $conditions_gui->get_hash_ref();  # retrieves 93
 ...
 sub _pre_built_superflow_close_path {
	...
	# print("1. file_dialog,_pre_built_superflow_close_path, pre_req_ok= $pre_req_ok \n");
   	...
   	if ( $pre_req_ok ) {
		...
		$conditions_gui				-> set4superflow_close_path_end();
   		$file_dialog				= $conditions_gui->get_hash_ref();  # retrieves 99
...
=cut 
 ...
 sub _pre_built_superflow_open_data_file {
    ...
    # for both superflow selection (necessary?? w alredy know what we are)
    # and for file dialog
        		       	
	$conditions_gui				-> set_hash_ref($file_dialog);
	$conditions_gui				-> set_gui_widgets($file_dialog);
	$conditions_gui   			-> set4FileDialog_open_start();
	$conditions_gui				-> set4superflow_open_data_file_start();
	$file_dialog				 = $conditions_gui->get_hash_ref(); # gets 93	
	...
	# print("file_dialog  _pre_built_superflow_open_data_file selected  parameter_values_frame = $parameter_values_frame 	\n");						    				  
	...
	if ($widget_type eq 'Entry' ) {
		...
		my $message          = $message_w->FileDialog_button(0);
	...
	$conditions_gui				-> set4superflow_open_data_file_end();
   	$conditions_gui  			-> set4FileDialog_open_end(); # 2 set
   	$file_dialog				 = $conditions_gui->get_hash_ref();  # retrieves 93
...
=cut 
 ...
 sub _pre_built_superflow_open_path {
	...
	$conditions_gui				-> set_gui_widgets($file_dialog);
	$conditions_gui   			-> set4FileDialog_open_start();
	$conditions_gui				-> set4superflow_open_path_start();
	$file_dialog				 = $conditions_gui->get_hash_ref(); # gets 93	
	...
	# print("file_dialog  _pre_built_superflow_open_path selected  parameter_values_frame = $parameter_values_frame \n");
	...
	$conditions_gui				-> set4superflow_open_path_end();
	$conditions_gui  			-> set4FileDialog_open_end(); # 2 set
	$file_dialog				 = $conditions_gui->get_hash_ref();  # retrieves 93
 }
 ...
 sub  _set_selected_file_name {
 	...
 	if ( $selected_file_name ne $empty_string ) {
		...
		$file_dialog->{_selected_file_name} = $selected_file_name;
 		...
 		print("file_dialog, _set_selected_file_name, empty string\n");
 	}
...
=cut
 ...
 sub _user_built_flow_SaveAs_perl_file {
	...
	$conditions_gui				-> set_hash_ref($file_dialog);
	$conditions_gui				-> set_gui_widgets($file_dialog);    # uses 23 / 38 in
	$conditions_gui				-> set4FileDialog_SaveAs_start();    # sets  3
	$file_dialog				 = $conditions_gui->get_hash_ref();  # retrieves 93
	...
	if ( $pre_req_ok ) {
		...
		$conditions_gui->set4FileDialog_SaveAs_end();  # ?? purpose? needed?
	}
 	return();	
 }
...
=cut
 ...
 sub _user_built_flow_open_perl_file {
	...
	$conditions_gui				->set_gui_widgets($file_dialog);
				# print("1. file_dialog,_user_built_flow_open_perl_file,_flowNsuperflow_name_w:$file_dialog->{_flowNsuperflow_name_w} \n");
	$conditions_gui   			->set4FileDialog_open_perl_file_start();
				# print("1. file_dialog,_user_built_flow_open_perl_file,_flowNsuperflow_name_w:$file_dialog->{_flowNsuperflow_name_w} \n");
	...
	$conditions_gui				->set_hash_ref($file_dialog); # uses 74 / 124 given
	$conditions_gui				->set_gui_widgets($file_dialog); # uses 29  /124 given
   	...
   	$file_dialog				= $conditions_gui-> get_hash_ref();
   			# print("1. file_dialog,_user_built_flow_open_data_file,_flowNsuperflow_name_w:$file_dialog->{_flowNsuperflow_name_w} \n");
...
=cut
 ...
 sub get_hash_ref {
    ...
    if($file_dialog) {
		...
		$file_dialog->{_FileDialog_sub_ref}				= $FileDialog_sub_ref,
		$file_dialog->{_FileDialog_option}				= $FileDialog_option,
     	$file_dialog->{_SaveAs_menubutton}				= $SaveAs_menubutton;    	   	
    	...
    	$file_dialog->{_check_buttons_settings_aref}	= $check_buttons_settings_aref;
		...
		$file_dialog->{_items_versions_aref} 				= $items_versions_aref;
  		$file_dialog->{_labels_w_aref}						= $labels_w_aref;			
   		$file_dialog->{_last_flow_index_touchedd_grey}		= $last_flow_index_touched_grey;
