 ...
 sub _get_values_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
 		...
 		@values_aref 	= @{@{$param_flow_grey->{_values_aref2}}[$item_index]};
 		 
		#print("param_flow_grey, _get_values_aref, $values_aref[0], index=$item_index\n");
  		return (\@values_aref);
	} 
 } 
...
=head2 sub _set_good_labels4item 
 	...
 	my $num_items4flow = $param_flow_grey->{_num_items4flow};
...
sub	_set_good_labels4item {
	...
	my (@good);
	my ($j) ;
    					# print("1. param_flow_grey,_set_good_labels4item, 
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		...
   		if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
   			...
   			if ( @$values_aref[$i] ne "'nu'"  ) {  #
				...
				my $name = ${@{$param_flow_grey->{_names_aref2}}[$idx]}[$i];
    				# print("2. param_flow_grey,_set_good_labels4item,
			...
			# NADAprint("param_flow_grey, _set_good_labels_4item: no values are present, can not be saved\n"); 
		} 								
	}
	...
	$param_flow_grey->{_num_good_labels_aref}	= \@num_good_labels;
	$good_labels[$idx] 							= \@good;
	$param_flow_grey->{_good_labels_aref2} 		= \@good_labels;

	#print("	3. param_flow_grey,_set_good_labels4item,
	#num_good_labels= $num_good_labels[$idx] , 
	#labels/names are:	@{@{$param_flow_grey->{_good_labels_aref2}}[$idx]}\n");
...
sub	_set_good_values4item {
	...
	my ($j) ;
    	...
    	# print("1. param_flow_grey,_set_good_values4item, flow index:$idx, prog name:@{$param_flow_grey->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 	# print("param_flow_grey, _set_good_values_4item: values_aref is @$values_aref[$i]\n");
	  	if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
	  		
	  		if (@$values_aref[$i] ne "'nu'" ) {
	
				my $value = ${@{$param_flow_grey->{_values_aref2}}[$idx]}[$i];
			...
			# print("param_flow_grey, _set_good_values_4item: no values are present, can not be saved\n"); 
		} 	
	} 
...
=cut
 ...
 sub clear {
 	...
 	$param_flow_grey->{_checkbuttons_aref}			= '';
	$param_flow_grey->{_checkbuttons_aref2}			= '';
	$param_flow_grey->{_destination_index}			= '';
	$param_flow_grey->{_end}						= '';
	$param_flow_grey->{_first_idx}					= 0;
	$param_flow_grey->{_flow_prog_names_aref}		= '';
	$param_flow_grey->{_good_checkbuttons_aref2}	= '';
	$param_flow_grey->{_good_labels_aref2}			= '';
	$param_flow_grey->{_good_values_aref2}			= '';
	$param_flow_grey->{_index2move}					= '';
	...
	$param_flow_grey->{_num_good_labels_aref}		= 0;
	...
	$param_flow_grey->{_selection_index}		    = '';
	$param_flow_grey->{_start}						= 0;
...
sub clear_flow_items_version_aref {
   ...
   if ($param_flow_grey->{_prog_version_aref}) {
 ...
 sub delete_selection {
  ...
  if ( $index2delete >= 0 && $index2delete < $end )  {
    ...
    for ( my $i=$index_after, my $j=$index2delete;  $i<=$end; $i++,$j++) {
     	...
     	@{@{$param_flow_grey->{_checkbuttons_aref2}}[$j]}    = @{@{$param_flow_grey->{_checkbuttons_aref2}}[$i]};
     	@{@{$param_flow_grey->{_names_aref2}}[$j]}			= @{@{$param_flow_grey->{_names_aref2}}[$i]};
     	@{@{$param_flow_grey->{_values_aref2}}[$j]}			= @{@{$param_flow_grey->{_values_aref2}}[$i]};
     	@{$param_flow_grey->{_prog_names_aref}}[$j]			= @{$param_flow_grey->{_prog_names_aref}}[$i];
    } 
...
=cut 
 ...
 sub first_idx {
  ...
  $param_flow_grey->{_first_idx} = 0;
  return($param_flow_grey->{_first_idx});
 ...
 sub get_flow_prog_names_aref {
	...
	if( $param_flow_grey->{_prog_names_aref} ne $empty_string) {
		...
		return($hash->{_prog_names_aref});
   		...
   		# print("param_flow_grey, no program names exist \n");
	}
 }
...
sub get_good_values_aref2 {
   my ($self) = @_;
	...
	if ( $param_flow_grey->{_good_values_aref2} ){
		my $good_values_aref2 = $param_flow_grey->{_good_values_aref2};
						 # print(" param_flow_grey,get_good_values_aref2,
						 # good_values for index 0= :
						 # @{@{$param_flow_grey->{_good_values_aref2}}[0]}\n");
...
=cut
 ...
 sub get_names_aref {
	my ($self) 	= @_ ;
	my $index 	= $param_flow_grey->{_selection_index};
		# print(" param_flow_grey, get_name_aref names: index is $index\n");
	...
	if ($index >=0) {
  	...
  	print(" param_flow_grey, get_names names:index <0 \n");
  }
 }
...
sub get_num_good_labels_aref {
	...
	if ( $param_flow_grey->{_num_good_labels_aref} ){
		my $num_good_labels_aref = $param_flow_grey->{_num_good_labels_aref};
...
=cut
 ...
 sub get_num_items {
   ...
   if ( $param_flow_grey->{_num_items} >= 0 ) {
		my $num_items =  $param_flow_grey->{_num_items4flow};
   	
		# print("param_flow_grey,get_num_items, num_items = $param_flow_grey->{_num_items} \n");
		return($num_items);
   	   
   } else {
   	print("param_flow_grey,get_num_items,- no number of items \n");
   	return();
   }
...
=cut
 ...
 sub insert_selection {
	...
	$tmp_prog_name			= @{$param_flow_grey->{_prog_names_aref}}[$idx2mv];
    @tmp_names_aref			= @{@{$param_flow_grey->{_names_aref2}}[$idx2mv]};
    @tmp_values_aref 		= @{@{$param_flow_grey->{_values_aref2}}[$idx2mv]};
   	@tmp_checkbuttons_aref	= @{@{$param_flow_grey->{_checkbuttons_aref2}}[$idx2mv]};
	...
	if ($idx2mv > $first) {
		...
		for (my $i=$first,my $j=$first; $j<$idx2mv; $i++,$j++){
       		$swap_prog_names[$i] 		= @{$param_flow_grey->{_prog_names_aref}}[$j];
       		$swap_names_aref[$i] 		= clone(\@{@{$param_flow_grey->{_names_aref2}}[$j]});
       		$swap_values_aref[$i] 		= clone(\@{@{$param_flow_grey->{_values_aref2}}[$j]});
       		$swap_checkbuttons_aref[$i] = clone(\@{@{$param_flow_grey->{_checkbuttons_aref2}}[$j]});
	...
	if ($destn_idx > $first) { # assume $destn_idx > 0 
		...
		for (my $i=$first, my $j=$first; $i<$destn_idx; $i++,$j++){
       		@{$param_flow_grey->{_prog_names_aref}}[$i] 		    = $swap_prog_names[$j];
   			@{@{$param_flow_grey->{_names_aref2}}[$i]}   		= @{$swap_names_aref[$j]};
   			@{@{$param_flow_grey->{_values_aref2}}[$i]}   		= @{$swap_values_aref[$j]};
   			@{@{$param_flow_grey->{_checkbuttons_aref2}}[$i]}   	= @{$swap_checkbuttons_aref[$j]};
		...
		for (my $i=($first+1), my $j=$first; $j<$end; $i++,$j++){
       		...
       		@{$param_flow_grey->{_prog_names_aref}}[$destn_idx] 		    = $tmp_prog_name;
   			@{@{$param_flow_grey->{_names_aref2}}[$destn_idx]}   		= @tmp_names_aref;
   			@{@{$param_flow_grey->{_values_aref2}}[$destn_idx]}  		= @tmp_values_aref;
   			@{@{$param_flow_grey->{_checkbuttons_aref2}}[$destn_idx]}   	= @tmp_checkbuttons_aref;
...
=cut 
 ...
 sub length {
 	...
 	$index 				= $param_flow_grey->{_selection_index}; 
 	if($index >= 0) {
 		@values_aref 	= @{@{$param_flow_grey->{_values_aref2}}[$index]};
 		$length			= scalar @values_aref;
 ...
 sub set_check_buttons_settings_aref {
	...
	my $chkbut_aref = $check_buttons_settings_aref;
  	...
  	if ($index >=0) {
 		...
 		@{$param_flow_grey->{_checkbuttons_aref2}}[$index] 	= $chkbut_aref;
 ...
 sub set_flow_index {
    ...
    if  ($index) { # if defined	
    	...
    	if ($index ne $empty_string && $index >= 0) {
			...
			print("param_flow_grey, set_flow_index,unexpected index, index:$index\n");
		}
 ...
 sub set_names_aref {
  ...
  if ($index >=0) {
    ...
    my ($length);
  	...
  	@{$param_flow_grey->{_names_aref2}}[$index] = $names_aref;
    # $length 		= scalar @names_aref;
...
sub	set_good_labels4item {
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
	  	...
	  	if (@$values_aref[$i] ne "'nu'" ) {
			my $name = ${@{$param_flow_grey->{_names_aref2}}[$idx]}[$i];
    					 #print("2. param_flow_grey,set_good_labels4item,
    					 #good index #$i, name:$name \n");
	...
	$param_flow_grey->{_good_labels_aref2} 	= \@good_labels;
						...
						# print("	3. param_flow_grey,set_good_,names4item,
						# num_good_labels= $num_good_labels , names are:
						# @{@{$param_flow_grey->{_good_labels_aref2}}[$idx]}\n");
	return();
}
...
sub	set_good_values4item {
	...
	my ($num_good_values,$j) ;
    					...
    					# print("1. param_flow_grey,set_good_values4item, 
    					# flow index:$idx, prog name: 
    					# @{$param_flow_grey->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
	  	...
	  	if (@$values_aref[$i] ne "'nu'" ) {
			my $value = ${@{$param_flow_grey->{_values_aref2}}[$idx]}[$i];
    					# print("2. param_flow_grey,set_good_values4item,
	...
	$param_flow_grey->{_good_values_aref2} 	= \@good_values;
						...
						# print("	3. param_flow_grey,set_good_,values4item,
						# num_good_values= $num_good_values , values are:
						# @{@{$param_flow_grey->{_good_values_aref2}}[$idx]}\n");
	return();
}
 	...
 	if ($param_flow_grey->{_selection_index} >= 0) {
 		...
 		if ($index >=0) {
 			...
 			@{$param_flow_grey->{_values_aref2}}[$index]	= $values_aref;
  			$length			=  scalar @values_aref;
   		 	# print("param_flow_grey,set_values :values_aref is @$values_aref\n");

  			for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
    			$values[$j]  = $values_aref[$i]; 
    			# print("param_flow_grey, set_values :index $j values: $values[$j]\n");
  			}
		} 		
 	} else {
 		print("param_flow_grey, set_values_aref: missing selection index\n");		
 	}
    		# print("param_flow_grey,set_values_aref :values_aref are @$values_aref[0]\n");
			# print("param_flow_grey,set_values_aref, index: $index\n");					
 return ();
 } 
...
=cut 
 ...
 sub stack_flow_item {
   ...
   if ($program_name_sref) {
     ...
     $param_flow_grey->{_prog_names_aref} 	= \@program_names;

     $param_flow_grey->{_indices}				= $index;
     $param_flow_grey->{_index4flow}			= $index;
     $param_flow_grey->{_num_items4flow}++;
     $param_flow_grey->{_num_items}++ ;
 ...
 sub stack_names_aref2 {
  ...
  if ($names_aref) {
  	my $index    						= $param_flow_grey->{_index4names}  + 1;
  		# print("param_flow_grey,names_aref2,index: $index\n");
  	$names[$index]						= $names_aref;
  	$param_flow_grey->{_names_aref2}			= \@names;
  	$param_flow_grey->{_indices}				= $index;
  	$param_flow_grey->{_index4names}++;
  	$param_flow_grey->{_num_items4names}++;
...
#     
#  	for (my $i=0; $i<=$index;$i++) {
#     	print("param_flow_grey,stack_names_aref2, an accumulating array of arrays: @{$param_flow_grey->{_names_aref2}} item $i\n");
#  	} 
 	
  }else {  	
  	print("param_flow_grey,stack_names_aref2 missing names_aref \n");
  }
  return();
 }
...
DB
  #for (my $i=0; $i<=$index;$i++) {
   # print("param_flow_grey,values_aref2,@{@{$param_flow_grey->{_values_aref2}}[$i]} index $i\n");
  #}
 ...
 sub stack_values_aref2 {
	...
	my ($self,$values_aref) 			= @_;
   	...
   	if ($values_aref) {
  		...
  		$param_flow_grey->{_values_aref2}	= \@values;
...
#  		} 
   		  	...
   		  	print("param_flow_grey,stack_values_aref2 missing values_aref \n");   		
   	}
    return();
 }
...
#	 print("\n param_flow_grey,view_data, _prog_names @{$param_flow_grey->{_prog_names_aref}}\n");
   	...
   	print("param_flow_grey,view_data:  prog_names:   @{$param_flow_grey->{_prog_names_aref}}\n");
