 ...
 sub _get_names_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
		...
		# print("param_flow_green, _get_names_aref,
 ...
 sub _get_values_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
		...
		#print("param_flow_green, _get_values_aref, $values_aref[0], index=$item_index\n");
  		return (\@values_aref);
	} 
 } 
...
=head2 sub _set_good_labels4item 
 	...
 	my $num_items4flow = $param_flow_green->{_num_items4flow};
...
sub	_set_good_labels4item {
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		...
   		if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
			...
			# NADAprint("param_flow_green, _set_good_labels_4item: no values are present, can not be saved\n"); 
		} 								
	}
...
sub	_set_good_values4item {
	...
	my ($j) ;
    	...
    	# print("1. param_flow_green,_set_good_values4item, flow index:$idx, prog name:@{$param_flow_green->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 	# print("param_flow_green, _set_good_values_4item: values_aref is @$values_aref[$i]\n");
	  	if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
			...
			# print("param_flow_green, _set_good_values_4item: no values are present, can not be saved\n"); 
		} 	
	} 
...
=cut
 ...
 sub clear {
	...
	$param_flow_green->{_selection_index}		    = '';
	$param_flow_green->{_start}						= 0;
 ...
 sub delete_selection {
  ...
  if ( $index2delete >= 0 && $index2delete < $end )  {
    ...
    for ( my $i=$index_after, my $j=$index2delete;  $i<=$end; $i++,$j++) {
     	...
     	@{@{$param_flow_green->{_checkbuttons_aref2}}[$j]}    = @{@{$param_flow_green->{_checkbuttons_aref2}}[$i]};
     	@{@{$param_flow_green->{_names_aref2}}[$j]}			= @{@{$param_flow_green->{_names_aref2}}[$i]};
     	@{@{$param_flow_green->{_values_aref2}}[$j]}			= @{@{$param_flow_green->{_values_aref2}}[$i]};
     	@{$param_flow_green->{_prog_names_aref}}[$j]			= @{$param_flow_green->{_prog_names_aref}}[$i];
    } 
   ...
   my ($self) = @_;
	...
	if ( $param_flow_green->{_good_values_aref2} ){
		my $good_values_aref2 = $param_flow_green->{_good_values_aref2};
						 # print(" param_flow_green,get_good_values_aref2,
						 # good_values for index 0= :
						 # @{@{$param_flow_green->{_good_values_aref2}}[0]}\n");
...
=cut
 ...
 sub get_names_aref {
	my ($self) 	= @_ ;
	my $index 	= $param_flow_green->{_selection_index};
		# print(" param_flow_green, get_name_aref names: index is $index\n");
...
=cut
 ...
 sub get_num_items {
   ...
   if ( $param_flow_green->{_num_items} >= 0 ) {
		my $num_items =  $param_flow_green->{_num_items4flow};
...
=cut
 ...
 sub insert_selection {
    ...
    @tmp_values_aref 		= @{@{$param_flow_green->{_values_aref2}}[$idx2mv]};
   	@tmp_checkbuttons_aref	= @{@{$param_flow_green->{_checkbuttons_aref2}}[$idx2mv]};
	...
	if ($idx2mv > $first) {
		...
		for (my $i=$first,my $j=$first; $j<$idx2mv; $i++,$j++){
       		$swap_prog_names[$i] 		= @{$param_flow_green->{_prog_names_aref}}[$j];
       		$swap_names_aref[$i] 		= clone(\@{@{$param_flow_green->{_names_aref2}}[$j]});
       		$swap_values_aref[$i] 		= clone(\@{@{$param_flow_green->{_values_aref2}}[$j]});
       		$swap_checkbuttons_aref[$i] = clone(\@{@{$param_flow_green->{_checkbuttons_aref2}}[$j]});
	...
	if ($destn_idx > $first) { # assume $destn_idx > 0 
		...
		for (my $i=$first, my $j=$first; $i<$destn_idx; $i++,$j++){
       		@{$param_flow_green->{_prog_names_aref}}[$i] 		    = $swap_prog_names[$j];
   			@{@{$param_flow_green->{_names_aref2}}[$i]}   		= @{$swap_names_aref[$j]};
   			@{@{$param_flow_green->{_values_aref2}}[$i]}   		= @{$swap_values_aref[$j]};
   			@{@{$param_flow_green->{_checkbuttons_aref2}}[$i]}   	= @{$swap_checkbuttons_aref[$j]};
		...
		for (my $i=($first+1), my $j=$first; $j<$end; $i++,$j++){
   			...
   			@{@{$param_flow_green->{_checkbuttons_aref2}}[$destn_idx]}   	= @tmp_checkbuttons_aref;
...
#  						print("param_flow_green, index: $index\n");
 		...
 		print("param_flow_green,length,  index does not exist\n");
 		print("param_flow_green, length, num values: $length\n");
 		return();
 	}
 }
 ...
 sub set_flow_index {
    ...
    if  ($index) { # if defined	
    	...
    	if ($index ne $empty_string && $index >= 0) {
			...
			print("param_flow_green, set_flow_index,unexpected index, index:$index\n");
		}
 	...
 	if ($param_flow_green->{_selection_index} >= 0) {
 		...
 		if ($index >=0) {
 			...
 			@{$param_flow_green->{_values_aref2}}[$index]	= $values_aref;
  			$length			=  scalar @values_aref;
   		 	# print("param_flow_green,set_values :values_aref is @$values_aref\n");

  			for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
    			$values[$j]  = $values_aref[$i]; 
    			# print("param_flow_green, set_values :index $j values: $values[$j]\n");
  			}
		} 		
 	} else {
 		print("param_flow_green, set_values_aref: missing selection index\n");		
 	}
    		# print("param_flow_green,set_values_aref :values_aref are @$values_aref[0]\n");
			# print("param_flow_green,set_values_aref, index: $index\n");					
 return ();
 } 
 ...
 sub stack_names_aref2 {
  ...
  if ($names_aref) {
  	my $index    						= $param_flow_green->{_index4names}  + 1;
  		# print("param_flow_green,names_aref2,index: $index\n");
...
#     
#  	for (my $i=0; $i<=$index;$i++) {
#     	print("param_flow_green,stack_names_aref2, an accumulating array of arrays: @{$param_flow_green->{_names_aref2}} item $i\n");
#  	} 
...
# $num_progs[5] = scalar  ( @{$param_flow_green->{_prog_names_aref}});
...
#	 print("\n param_flow_green,view_data, _prog_names @{$param_flow_green->{_prog_names_aref}}\n");
   	...
   	print("param_flow_green,view_data:  prog_names:   @{$param_flow_green->{_prog_names_aref}}\n");
