...
# print("param_widgets, gui_clean, _labels_w_aref, $param_widgets->{_labels_w_aref} \n");
    ...
    $wipe->range($param_widgets);    # sets values, labels and check_buttons
    $wipe->values();
    $wipe->labels();
    $wipe->check_buttons();
...
clear the gui compeltely of parameter values
...
sub gui_full_clear {
    ...
    $param_widgets->{_length} = $default_param_specs->{_length};

# print("param_widgets, gui_clean, _values_w_aref, $param_widgets_grey->{_values_w_aref} \n");
# print("param_widgets, gui_clean, _labels_w_aref, $param_widgets_grey->{_labels_w_aref} \n");
...
# empty or apparently empty cases
# print("param_widgets,local_update_check_button_setting :index $idx value: $values[$idx]\n");
    if ( !( $values[$idx] ) && defined $values[$idx] ) {

        #print("2. param_widgets,local_update_check_button_setting, defined\n");
        if ( $values[$idx] eq '0' ) {
            $on_off[$idx] = $on;

#print("param_widgets,,local_update_check_button_setting value for index $idx is $values[$idx]\n");
        }
        elsif ( $values[$idx] eq "" ) {
            $on_off[$idx] = $off;

   #print("2. param_widgets,local_update_check_button_setting, empty string\n");
        }
        elsif ( $values[$idx] eq '' ) {

            #print("1. param_widgets,local_update_check_button_setting, '' \n");
            $on_off[$idx] = $off;
        }
        else {
            print(
"param-widgets,local_update_check_button_setting, apparently unconsidered empty case\n"
            );
        }
    }
   ...
   #print("2. param_widgets,local_update_check_button_setting, empty string\n");
       ...
       #print("3. param_widgets,local_update_check_button_setting, all else\n");
            $on_off[$idx] = $on;
        }
    }
...
# print("param_widgets: local_update_check_button_setting :index $idx setting is: $on_off[$idx]\n");
...
#	my @self = @_;
#    my $idx = $param_widgets->{_last_changed_program_index};
#    return($idx);
#}
...
sub get_values_aref {
    ...
    if ( $param_widgets->{_values_aref} ) {
        my $values_aref = \@{ $param_widgets->{_values_aref} };

  # print("param_widgets,get_values_aref: @{$param_widgets->{_values_aref}}\n");
        return ($values_aref);

    }
    else {
        print(
"param_widgets,get_values_aref missing param_widgets->{_values_aref}\n"
        );
        return ();
    }
...
=head2 sub set_values_frame
 ...
 a widget reference
  #$param_widgets->{_values_frame_href} 
...
=head2 sub range 
    ...
    the last index of the previous flow is used to clean the gui labels and values
 
   foreach my $key (sort keys %$param_widgets) {
         print (" param_widgets,key is $key, value is $param_widgets->{$key}\n");
      }
...
# print("1. param_widgets,range, _is_superflow_select_button,$ref_hash->{_is_superflow_select_button}\n");
# print("2. param_widgets,range, _is_superflow_select,$param_widgets->{_is_superflow_select_button}\n");

# foreach my $key (sort keys %$param_widgets) {
#  print (" param_widgets,range, key is $key, value is $param_widgets->{$key}\n");
# }
    ...
    if ( $param_widgets->{_is_add2flow_button} ) {
        ...
        print(
"param_widgets,range, first idx:$param_widgets->{_first_idx}, and length:$param_widgets->{_length}\n"
        );
    }

    # for sunix selections
    if ( $param_widgets->{_is_sunix_listbox} ) {
        $param_widgets->{_first_idx} = $ref_hash->{_param_sunix_first_idx};
        $param_widgets->{_length}    = $ref_hash->{_param_sunix_length};
        print(
"param_widgets,range, first idx:$param_widgets->{_first_idx}, and length:$param_widgets->{_length}\n"
    ...
    if (   $param_widgets->{_is_flow_listbox_grey_w}
...
# print("param_widgets,range, first idx:$param_widgets->{_first_idx}, and length:$param_widgets->{_length}\n");
    }
}
...
sub redisplay_check_buttons {
    ...
    my $first          = $param_widgets->{_first_idx};
    my $length         = $param_widgets->{_length};
    my $variables_aref = $param_widgets->{_check_buttons_settings_aref};

    if ( $button_w_aref && $variables_aref ) {

        for ( my $i = $first ; $i < $length ; $i++ ) {

# print("1. param_widgets,redisplay_check_buttons,variables @{$variables_aref}[$i]\n");
# print("2. param_widgets,redisplay_check_buttons,variables @{$param_widgets->{_check_buttons_settings_aref}}[$i]\n");
        ...
        print(
            "param_widgets_grey, redisplay_check_buttons missing parameters\n");
    }
    return ();
}
...
sub redisplay_labels {
    ...
    if ($labels_w_aref) {
        ...
        print(
"param_widgets_grey,redisplay labels, Warning parameters or labels_w_aref missing \n"
        );
    }
    return ();
}
...
# print("param_widgets, redisplay_values, value is @{$param_widgets->{_values_aref}}[$i]\n");
...
# print("param_widget,set_entry_button_chosen,first_index=$param_widgets->{_first_idx}\n");
# print("param_widget,set_entry_button_chosen,length_=$param_widgets->{_length} \n");

    my $widget = $param_widgets->{_entry_button_chosen_widget};

    # print("param_widget,set_entry_button_chosen,widget,=$widget\n");

    # run through widgets until the match is made
    for ( my $choice = $first ; $choice < $length ; $choice++ ) {
        if ( $widget eq @{ $param_widgets->{_values_w_aref} }[$choice] ) {

            @{ $param_widgets->{_values_w_aref} }[$choice] = $value;

            print("param_widget,set_entry_button_chosen,value,=$value\n");
            print(
                "param_widgets_grey,set_entry_button_chosen,choice= $choice\n");
...
sub set_location_in_gui {
    ...
    if ( $here->{_is_flow_listbox_grey_w} ) {

        _reset $param_widgets->{_is_flow_listbox_grey_w} = $true;
        $param_widgets->{_is_flow_listbox_color_w} = $true;

    # print("param_widgets, set_location_in_gui, _is_flow_listbox_grey_w ++\n");
...
sub set_values {
    ...
    if ($values_aref) {
        ...
        print("param_widgets,set_values,_values_aref missing\n");
    }
