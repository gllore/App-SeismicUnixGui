 ...
 sub local_update_check_button_setting {
    ...
    if ( !($values[$idx]) && defined $values[$idx]) {
        ...
        if ( $values[$idx] eq '0'  ) {
    		...
    		print("param-widgets,local_update_check_button_setting, apparently unconsidered empty case\n");
    	}
    };	
...
sub get_values_aref {
	...
	if ( $param_widgets->{_values_aref}) {
		...
		print("param_widgets,get_values_aref missing param_widgets->{_values_aref}\n");
		return();
	}
...
sub range {
  	...
  	# print("2. param_widgets,range, _is_superflow_select,$param_widgets->{_is_superflow_select_button}\n");
		...
		# for user-built flows
       	...
       	if ( $ref_hash->{_is_superflow_select_button} || $ref_hash->{_is_superflow})  {  # button for L_SU and no button for project selector
