...
# open and write to output file
   my($ref_cdp,$ref_values,$ref_file_name) = @_;
   my ($i,$number_of_cdp,$number_of_lines);

   #print("\nThe output file is called $$ref_file_name\n");
   $number_of_cdp = scalar @$ref_cdp -1; 
   $number_of_lines = 2 * $number_of_cdp;
   #print("number of cdps is $number_of_cdp\n");
   #print("number of lines is $number_of_lines\n");
   ...
   for($i=2; $i<=$number_of_cdp; $i++) {
         print OUT (",$$ref_cdp[$i]");
   }
          print OUT ("\n");

      for (my $i=1; $i<$number_of_lines;$i=$i+2) {
          print OUT ("tnmo=$$ref_values[$i]\n");
          print OUT ("vnmo=$$ref_values[($i+1)]\n");
      }
