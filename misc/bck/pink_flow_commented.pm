...
# print("color_flow,_add2flow,check_buttons_settings : @{$color_flow->{_check_buttons_settings_aref}}\n");
    ...
    $conditions_gui->set4start_of_add2flow($flow_color)
      ;    #  set and 10 widgets configured
...
# print("8. color_flow, add2flow_button, _is_flow_listbox_pink_w $color_flow->{_is_flow_listbox_pink_w}\n");
...
# selection occurs inside conditions_gui
    $color_flow->{_flow_listbox_color_w}
      ->insert( "end", ${ $color_flow->{_prog_name_sref} }, );
    ...
    # because focus is transferred to a flow list box  (pink)
...
#  }
    ...
    $param_widgets->set_check_buttons_w_aref(
        $color_flow->{_check_buttons_w_aref} );
...
# print("18. color_flow, add2flow_button, _values_w_aref @{$color_flow->{_values_w_aref}}}[0]\n");
    $conditions_gui->set_gui_widgets($color_flow)
      ;    # includes _values_w_aref; 29 used / 114 in
...
# print("4 color_flow,add2flow_button,_flowNsuperflow_name_w : $color_flow->{_flowNsuperflow_name_w} \n");
    ...
    my $index =
      $flow_widgets->get_flow_selection( $color_flow->{_flow_listbox_color_w} );
...
# print("5 color_flow,add2flow_button,_flowNsuperflow_name_w : $color_flow->{_flowNsuperflow_name_w} \n");
    ...
    $color_flow->{_is_last_parameter_index_touched_pink} = $true;

# print("1. color_flow, _add2flow, color_flow->{_flow_type: $color_flow->{_flow_type} \n");

# print("1. color_flow, _add2flow, is_last_parameter_index_touched_pink: $color_flow->{_is_last_parameter_index_touched_pink} \n");
...
# print("11 color_flow, _FileDialog_button, param_widgets values_aref: @{$color_flow->{_values_aref}}\n");
        ...
        $file_dialog->set_flow_color( $color_flow->{_flow_color} )
          ;                                             # uses 1/ 31 in
        $file_dialog->set_hash_ref($color_flow)
          ;    # uses 7/ 31 in  ; uses values_aref
...
# update endtry those parameters
        ...
        $color_flow->{_parameter_value_index} =
          $param_widgets->get_entry_button_chosen_index();
        $color_flow->{_entry_button_label} =
...
##########
        ...
        $color_flow->{_is_last_flow_index_touched_pink} = $true;
...
#	    print("1 color_flow _check4flow_changes, _last_flow_index_touched is:  $color_flow->{_last_flow_index_touched} \n");
    ...
    {    # must be defined but can be =0
        ...
        if (
            ...
            && (
                ( $color_flow->{_last_flow_color} eq 'neutral' )
                ||    # first item in flow was just put in
                $color_flow->{_last_flow_color} eq $color_flow->{_flow_color}
            )         #  used the same colored flow
          )
        {
...
# print("29 color_flow check4flow_changes,changed values_aref: @{$color_flow->{_values_aref}}\n");
            ...
            $color_flow->{_check_buttons_settings_aref} =
              $param_widgets->get_check_buttons_aref();
...
# save old changed values and names but not the versions fo the following 3 lines
            ...
            $param_flow_color_i->set_check_buttons_settings_aref(
                $color_flow->{_check_buttons_settings_aref} );
...
# $param_flow_color_i	-> view_data();
            ...
            # FOR EXPORT
            $last_flow_color = $color_flow->{_flow_color}
              ;    # reset to working only in current flow color
...
# print("2. color_flow,_SaveAs_button parameter names: @{$color_flow->{_names_aref}}[2]\n");
        ...
        $color_flow->{_good_labels_aref2} =
          $param_flow_color_i->get_good_labels_aref2();
        $color_flow->{_items_versions_aref} =
          $param_flow_color_i->get_flow_items_version_aref();
        $color_flow->{_good_values_aref2} =
          $param_flow_color_i->get_good_values_aref2();
        $color_flow->{_prog_names_aref} =
          $param_flow_color_i->get_flow_prog_names_aref();
...
# print("color_flow,flow_listbox_color_w, $flow_listbox_pink_w, _perl_flow\n");
...
sub _set_flow_listbox_color_w {
    ...
    if ( $flow_color eq 'pink' ) {
        ...
        $color_flow->{_flow_listbox_color_w} =
          $color_flow->{_flow_listbox_pink_w};

    }
    else {
        print(
"color_flow,_set_flow_listbox_color_w, _set_flow_listbox_color_w, missing color\n"
        );
    }
...
# print("color_flow,_set_flow_name_color_w, flow_name_color_w,   $color_flow->{_flow_name_pink_w} \n");
...
sub _set_is_last_parameter_index_touched_color {
    ...
    if ( $flow_color eq 'pink' ) {
        ...
        # not for export
        $color_flow->{_is_last_parameter_index_touched_color} =
          $color_flow->{_is_last_parameter_index_touched_pink};

# print("color_flow,_set_is_last_parameter_index_touched_color, color is $flow_color\n");
        print(
"color_flow,_set_is_last_parameter_index_touched_color, is_last_parameter_index_touched_pink: $color_flow->{_is_last_parameter_index_touched_pink}\n"
        );

    }
    else {
        print(
"color_flow,_set_is_last_parameter_index_touched_color, missing color \n"
        );
    }
...
sub _set_last_parameter_index_touched_color {
    ...
    if ( $flow_color eq 'pink' ) {
        ...
        # not for export
        $color_flow->{_last_parameter_index_touched_color} =
          $color_flow->{_last_parameter_index_touched_pink};

# print("color_flow,_set_last_parameter_index_touched_color, color is $flow_color\n");
        print(
"color_flow,_set_last_parameter_index_touched_color, last_parameter_index_touched_pink: $color_flow->{_last_parameter_index_touched_pink}\n"
        );

    }
    else {
        print(
"color_flow,_set_last_parameter_index_touched_color, , missing color \n"
        );
    }
...
sub _set_param_flow_color_i {
    ...
    if ($flow_color) {
        ...
        print(
"color_flow,_set_param_flow_color_i, __set_param_flow_color_i, missing color \n"
        );
    }
...
# $param_flow_color_i->view_data();
    ...
    $param_flow_color_i->stack_checkbuttons_aref2(
        $color_flow->{_check_buttons_settings_aref} );
...
# print("color_flow, _stack_versions ,flow_color: $flow_color \n");
    ...
    $param_flow_color_i->set_flow_items_version_aref(
        $color_flow->{_items_versions_aref} );
...
{   # N.B., Another _FileDialog_button in L_SU.pm needs to be updated too!!!!!!!
...
# print ("2. color_flow,FileDialog_button, is_last_flow_index_touched: $color_flow->{_is_last_flow_index_touched_pink} \n");
...
# print ("0. color_flow,FileDialog_button, is_last_parameter_index_touched: $color_flow->{_is_last_parameter_index_touched_pink} \n");
# print ("0. color_flow,FileDialog_button, is_last_flow_index_touched: $color_flow->{_is_last_flow_index_touched_pink} \n");
            $color_flow->{_values_aref} = $param_widgets->get_values_aref();
            ...
            $color_flow->{_dialog_type} =
              $$dialog_type_sref;    # dereference scalar

            $file_dialog->set_flow_color( $color_flow->{_flow_color} )
              ;                      # uses 1/ 114 in
            $file_dialog->set_hash_ref($color_flow)
              ;                      # uses 86/ 114 in  ; uses values_aref
            $file_dialog->set_gui_widgets($color_flow);    # uses 34/ 114 in
             ...
             # print ("1. color_flow,FileDialog_button, is_last_parameter_index_touched: $color_flow->{_is_last_parameter_index_touched_pink} \n");
             # print ("1. color_flow,FileDialog_button, is_last_flow_index_touched: $color_flow->{_is_last_flow_index_touched_pink} \n");

            $file_dialog->FileDialog_director();

# print ("2. color_flow,FileDialog_button, is_last_parameter_index_touched: $color_flow->{_is_last_parameter_index_touched_pink} \n");
...
#			print("color_flow,FileDialog_button,_is_flow_listbox_pink_w:	$color_flow->{_is_flow_listbox_pink_w} \n");
            _check4parameter_changes();

            use message_director;
            my $color_flow_messages = message_director->new();

            $color_flow->{_flow_name_out} =
...
# print("color_flow, _perl_flow, does file=$color_flow->{_flow_name_in} exist? ANS= $file_exists \n");
            ...
            if ($file_exists) {
                ...
                $flow_name_color_w->configure(
                    -text => $color_flow->{_flow_name_in} );

# $color_flow->{_flow_name_pink_w}     -> configure(-text => $color_flow->{_flow_name_in});

                # Place names of the programs at the head of the GUI
                $color_flow->{_flowNsuperflow_name_w}
                  ->configure( -text => $color_flow->{_flow_name_in} );

                # populate gui, and bot param_flow and param_widgets namespaces
                _perl_flow();

# print("3 color_flow,FileDialog_button,_is_last_flow_index_touched_pink:$color_flow->{_is_last_flow_index_touched_pink} \n");
...
#			print("3 color_flow,FileDialog_button,_is_flow_listbox_pink_w:	$color_flow->{_is_flow_listbox_pink_w} \n");
         ...
         # Assume we are still dealing with the current flow item selected
            ...
            $color_flow->{_last_flow_listbox_color_w} =
              $color_flow->{_flow_listbox_color_w};
...
"color_flow,_FileDialog_button(binding), last_parameter_index_touched_color: $color_flow->{_last_parameter_index_touched_color} \n"
       ...
       # update to parameter values occurs in file_dialog
            ...
            print(
"color_flow,FileDialog_button(binding), last_flow_index_touched:$color_flow->{_last_flow_index_touched} \n"
            );
...
# print ("2. color_flow,FileDialog_button, is_last_parameter_index_touched_color: $color_flow->{_is_last_parameter_index_touched_pink} \n");
# print ("2. color_flow,FileDialog_button, is_last_flow_index_touched: $color_flow->{_is_last_flow_index_touched_pink} \n");
    return ();
}
...
# print("4. color_flow, add2flow_button, _values_w_aref $color_flow->{_values_w_aref}\n");
    ...
    $conditions_gui->set4start_of_add2flow_button($flow_color)
      ;    # 5 set and 10 widgets configured
    $color_flow = $conditions_gui->get_hash_ref();    # 88 returned
...
# print("11. color_flow, add2flow_button, _labels_w_aref $color_flow->{_labels_w_aref}\n");
    ...
    $color_flow->{_param_sunix_first_idx} =
...
# print("18. color_flow, add2flow_button, _values_aref[1] @{$color_flow->{_values_aref}}[1]\n");
    $conditions_gui->set_gui_widgets($color_flow)
      ;    # includes _values_w_aref; 26 used / 61 in
...
# print("21. color_flow, add2flow_button, color: $flow_color \n");
    ...
    $conditions_gui->set_flow_index_last_touched($flow_index)
      ;    # flow color is not reset
    $color_flow = $conditions_gui->get_hash_ref();
...
# print("22. color_flow, add2flow_button, _is_flow_listbox_pink_w $color_flow->{_is_flow_listbox_pink_w}\n");
...
# print("27. color_flow,add2flow_button,last pink-flow program touched had index: $color_flow->{_last_flow_index_touched}\n");
...
# print("31. color_flow add2flow_button: _is_last_parameter_index_touched_pink $color_flow->{_is_last_parameter_index_touched_pink}\n");
# print("color_flow,add2flow,_is_flow_listbox_pink_w:	$color_flow->{_is_flow_listbox_pink_w} \n");
    flow_select();
...
# print("1. color_flow,delete_from_flow_button, flow_listbox_color_w: $_flow_listbox_color_w \n");
        ...
        $conditions_gui->set4start_of_delete_from_flow_button($flow_color)
          ;                                               # 3 set /  72 max
        $color_flow = $conditions_gui->get_hash_ref()
          ;    # find out if delete button is active    64 returned / 72 max

# print("1. color_flow,delete_from_flow_button , is_flow_listbox_pink_w  $color_flow->{_is_flow_listbox_color_w} \n");
        ...
        if ($pre_req_ok) {
         ...
         # clear 31 parameters
            ...
            {    #  i.e., more than one item remains in a listbox
  ...
  # print("color_flow delete_from_flow_button, last index touched :$index..\n");
        ...
        #print("2. color_flow delete_from_stored,index2delete:$index2delete\n");
                ...
                $param_flow_color_i->set_flow_index(
                    $next_idx_selected_after_deletion);
                $color_flow->{_names_aref} =
                  $param_flow_color_i->get_names_aref();

                # print("2. color_flow delete_from_flow_button,parameter names
                # is @{$color_flow->{_names_aref}}\n");
                $color_flow->{_values_aref} =
                  $param_flow_color_i->get_values_aref();
            ...
            # @{$color_flow->{_check_buttons_settings_aref}}, index;$index\n#");
                ...
                $color_flow->{_param_flow_first_idx} =
                  $param_flow_color_i->first_idx();
                $color_flow->{_param_flow_length} =
                  $param_flow_color_i->length();
...
#$flow_widgets->set_vigil_on_delete();
    ...
    if ( $num_items > 1 ) {    # num_items PRIOR to deletion
        ...
        # print("color_flowdrag,start dnd_token_pink:$dnd_token_pink\n");
    }
    else {
...
sub drop {
    ...
    if ( $prog_name && $chosen_index_to_drop && $chosen_index_to_drop >= 0 )
    {    # same as destination_index
...
# print(" color_flow drop, destination index = $color_flow->{_last_flow_index_touched}\n");
   ...
   #$flow_listbox_pink_w    ->selectionSet( $color_flow->{_destination_index},);
...
# modify flow listbox
# _set_flow_listbox_color_w($flow_color); # in "color"_flow namespace
# print("3. color_flow,flow_item_down_arrow_button , _is_flow_listbox_pink_w $color_flow->{_is_flow_listbox_pink_w}\n");
# print("2. color_flow,flow_item_down_arrow_button,color_flow->listbox_widget: $color_flow->{_flow_listbox_color_w}, color: $flow_color\n");
            ...
            $color_flow->{_flow_listbox_color_w}
              ->delete( $color_flow->{_index2move} );
            $color_flow->{_flow_listbox_color_w}
              ->insert( $color_flow->{_destination_index}, $saved_item );
            $color_flow->{_flow_listbox_color_w}
              ->selectionSet( $color_flow->{_destination_index} );
...
# _set_flow_listbox_color_w($flow_color); # in "color"_flow namespace
# print("3. color_flow,flow_item_up_arrow_button , _is_flow_listbox_pink_w $color_flow->{_is_flow_listbox_pink_w}\n");
...
# print("1.color_flow, flow_select,color->{_is_last_parameter_index_touched_pink}:$color_flow->{_is_last_parameter_index_touched_pink}\n");
...
# print("1. color_flow, flow_select, _is_last_parameter_index_touched_pink:$color_flow->{_is_last_parameter_index_touched_pink}, color:$flow_color\n");
...
# print("2. color_flow, flow_select,color->{_is_last_parameter_index_touched_pink}:$color_flow->{_is_last_parameter_index_touched_pink}\n");
...
#  independently set conditions to make pink box available
#  print("8. color_flow, flow_select,_is_flow_listbox_pink_w $color_flow->{_is_flow_listbox_pink_w}\n");
...
# print("2. color_flow _check4flow_changes, NEW last_flow_color = $color_flow->{_last_flow_color}\n");
        ...
        $color_flow->{_is_last_parameter_index_touched_pink} = $true;
        my $num_indices =
          $flow_widgets->get_num_indices( \$_flow_listbox_color_w );
...
# program
        ...
        &&    # must stay as grey--- a magic number.... TODO
...
# print("1 color_flow , flow_select, _is_last_flow_index_touched_pink:  $color_flow->{_is_last_flow_index_touched_pink}\n");
# print("1 color_flow , flow_select, _is_last_parameter_index_touched_pink :  $color_flow->{_is_last_parameter_index_touched_pink}\n");
...
# print("16. color_flow,flow_select,_prog_name_sref: ${$color_flow->{_prog_name_sref}} \n");
        ...
        $color_flow->{_flowNsuperflow_name_w}
          ->configure( -text => ${ $color_flow->{_prog_name_sref} } );

        # needed in possible export via get_hash_ref to help
        $prog_name_sref = $color_flow->{_prog_name_sref};

# print("color_flow,flow_select,_is_flow_listbox_pink_w:	$color_flow->{_is_flow_listbox_pink_w} \n");
...
#  	 	$color_flow->{_is_flow_listbox_blue_w}		= $is_flow_listbox_blue_w;   # set in condition_gui
    ...
    $color_flow->{_parameter_values_button_frame} =
...
=head2 sub get_last_flow_color
		...
		returns current folor (pink) as the last fow color
...
sub save_button {
    ...
    if ( $topic eq 'Save' ) {
        ...
        my $current_flow_index =
          $flow_widgets->get_flow_selection($flow_listbox_color_w);
        ...
        if ( ( !($current_flow_index) || $current_flow_index eq $empty_string )
            && $last_flow_color
            && $last_flow_index
            && $flow_listbox_color_w )
        {
        ...
        elsif (( $current_flow_index || $current_flow_index ne $empty_string )
            && $last_flow_color
            && $last_flow_index
            && $flow_listbox_color_w )
        {
...
sub set_hash_ref {
    ...
    $color_flow->{_delete_from_flow_button} =
      $hash_ref->{_delete_from_flow_button};
    ...
    $color_flow->{_flow_item_down_arrow_button} =
      $hash_ref->{_flow_item_down_arrow_button};
    $color_flow->{_flow_item_up_arrow_button} =
    ...
    $color_flow->{_is_pre_built_superflow} =
    ...
    $color_flow->{_parameter_values_frame} =
      $hash_ref->{_parameter_values_frame};
    $color_flow->{_parameter_values_button_frame} =
      $hash_ref->{_parameter_values_button_frame};
