...
=head2 sub _set_good_labels4item 
 	...
 	my $num_items4flow = $param_flow_pink->{_num_items4flow};
...
sub	_set_good_labels4item {
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		...
   		if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
			...
			# NADAprint("param_flow_pink, _set_good_labels_4item: no values are present, can not be saved\n"); 
		} 								
	}
...
=cut
 ...
 sub clear {
	...
	$param_flow_pink->{_selection_index}		    = '';
	$param_flow_pink->{_start}						= 0;
 ...
 sub delete_selection {
  ...
  if ( $index2delete >= 0 && $index2delete < $end )  {
    ...
    for ( my $i=$index_after, my $j=$index2delete;  $i<=$end; $i++,$j++) {
     	...
     	@{@{$param_flow_pink->{_checkbuttons_aref2}}[$j]}    = @{@{$param_flow_pink->{_checkbuttons_aref2}}[$i]};
...
=cut
 ...
 sub get_names_aref {
	my ($self) 	= @_ ;
	my $index 	= $param_flow_pink->{_selection_index};
		# print(" param_flow_pink, get_name_aref names: index is $index\n");
 ...
 sub get_values_aref {
 	my ($self) 	= @_ ;
  	...
  	if ( ($param_flow_pink->{_selection_index} >=0) && 
  		...
  		my $index = $param_flow_pink->{_selection_index};
  		# print("param_flow_pink, get_values :_values_aref2:@{@{$param_flow_pink->{_values_aref2}}[$index]}\n");	
  		my ($i,$j,$length);
  		my (@values_aref,@values);
  			# print("param_flow_pink, get_values :index $index\n");

 		@values_aref 	= @{@{$param_flow_pink->{_values_aref2}}[$index]};
  		$length			=  scalar @values_aref;
   			# print("param_flow_pink,get_values :values_aref is @values_aref\n");

  		for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
    		$values[$j]  = $values_aref[$i]; 
    		# print("param_flow_pink, get_values :index $j values: $values[$j]\n");
  		}
  		return (\@values_aref);		
 ...
 sub set_flow_index {
    ...
    if  ($index) { # if defined	
    	...
    	if ($index ne $empty_string && $index >= 0) {
			...
			print("param_flow_pink, set_flow_index,unexpected index, index:$index\n");
		}
...
=cut
 ...
 sub set_values_aref {
 	...
 	if ($param_flow_pink->{_selection_index} >= 0) {
 		...
 		print("param_flow_pink, set_values_aref: missing selection index\n");		
 	}
...
=cut
 ...
 sub stack_checkbuttons_aref2 { 
  ...
  $param_flow_pink->{_index4checkbuttons}++;
  $param_flow_pink->{_num_items4checkbuttons}++;
  $param_flow_pink->{_num_items}     = $param_flow_pink->{_num_items4checkbuttons};
...
#	 print("\n param_flow_pink,view_data, _prog_names @{$param_flow_pink->{_prog_names_aref}}\n");
   	...
   	print("param_flow_pink,view_data:  prog_names:   @{$param_flow_pink->{_prog_names_aref}}\n");
