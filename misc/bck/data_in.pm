...
sub _get_DIR {
	...
	if ( $data_in->{_suffix_type} ) {
		...
		use SeismicUnix qw ($segy $sgy $su $txt $bin);
...
sub _get_suffix {
	...
	if ( $data_in->{_suffix_type}) {
		...
		use SeismicUnix qw ($suffix_sgy $suffix_su $suffix_bin $suffix_txt $sgy $su $txt $bin);
		...
		if ($suffix_type eq $su) {
			...
			print("data_in, suffix_type=($suffix_type) is not su, bin, sgy or txt\n"); 
