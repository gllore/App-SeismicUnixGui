...
=cut
      ...
      if ($ref_picks_file && $inbound) {
        ...
        my($ref_time,$ref_key,$max_index) = $read->cols_2p($ref_picks_file);
...
=cut
	#print("keys are @$ref_key\n");
 	...
 	$xtract->{_waveform_key_min}  	= $sorted_key[0];
        ...
        $xtract->{_file_out}            = $$ref_picks_file.'_'.$xtract->{_waveform_tmin}.'s_'.$xtract->{_waveform_tmax}.'s'.$suffix_su;
       print("file out is $xtract->{_file_out}\n\n");
...
=cut
      ...
      if($xtract->{_waveform_key_min} <=1){
    	print("smallest key is $xtract->{_waveform_key_min}\n");
        print("updated to 1 \n");
        $xtract->{_waveform_key_min}  = 1;
      }

      if($xtract->{_waveform_key_max} <=1){
        print("largest key is $xtract->{_waveform_key_max}\n");
        print("updated to 1 \n");
