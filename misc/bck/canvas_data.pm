...
=cut
 ...
 sub _time2col_index{
	...
	if ( $canvas_data->{_y_value} 			ne $empty_string &&
		...
		my $result					= ($time_s - $t_min_s) * ($num_time_samples -1) / $t_range_s;		
...
=cut
 ...
 sub _time2col_num{
	...
	if ( $canvas_data->{_y_value} 			ne $empty_string &&
		...
		my @dims_amplitudes  		= dims $amplitudes;
		print("canvas_data, _time2col_num, dims_amplitudes = $dims_amplitudes[0] time samples\n");
		print("canvas_data, _time2col_num, dims_amplitudes = $dims_amplitudes[1] traces\n");
		my $t_range_s				= $t_max_s- $t_min_s;
		my $num_time_samples		= $dims_amplitudes[0];
		my $result					= ($time_s - $t_min_s) * ($num_time_samples -1) / $t_range_s;		
		my $rounded_result			= sprintf("%.0f", $result);
		$canvas_data->{_time_col_num}	= $rounded_result;		
		print("canvas_data, _time2col_num, column no. = $canvas_data->{_time_col_num}\n");		
		return();
...
=cut
 ...
 sub _x_value{
	...
	if ( $canvas_data->{_x_pix} 				ne $empty_string &&
		...
		my $seismic_plot_x_offset_pix	= $canvas_data->{_seismic_plot_x_offset_pix};
		...
		my $result 						= $x_min + $increment_per_pix_x * ($x_pix - $seismic_plot_x_offset_pix) ;
		my $rounded_result				= sprintf("%.0f", $result);
...
=cut
 ...
 sub _y_value{
	...
	if ( $canvas_data->{_y_pix} 					ne $empty_string &&
		...
		my $result 						= $t_min_s + $increment_per_pix_y * ($y_pix - $seismic_plot_y_offset_pix) ;	
...
sub	set_seismic_plot_y_offset_pix {
	...
	if ($y_offset ne $empty_string ) {
		...
		print("canvas_data, set_seismic_plot_y_offset_pix , missing y_offset \n");
	}
	return();
} 
