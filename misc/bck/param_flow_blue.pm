...
=head2 SYNOPSIS 
 ...
 Perl package: param_flow_blue.pm 
 AUTHOR: Juan Lorenzo
 DATE: Aug 3 2017 
 ...
 sub _get_names_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
		...
		# print("param_flow_blue, _get_names_aref,
 ...
 sub _get_values_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
 		...
 		@values_aref 	= @{@{$param_flow_blue->{_values_aref2}}[$item_index]};
 		 
		#print("param_flow_blue, _get_values_aref, $values_aref[0], index=$item_index\n");
  		return (\@values_aref);
	} 
 } 
...
sub	_set_good_labels4item {
	...
	my (@good);
	my ($j) ;
    					# print("1. param_flow_blue,_set_good_labels4item, 
    					# flow index:$idx, prog name: 
    					# @{$param_flow_blue->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 				# print("param_flow_blue, _set_good_labels4item:
   		...
   		if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
   			...
   			if ( @$values_aref[$i] ne "'nu'"  ) {  #
				...
				my $name = ${@{$param_flow_blue->{_names_aref2}}[$idx]}[$i];
    				# print("2. param_flow_blue,_set_good_labels4item,
    				# good index #$i, name:$name \n");  					
				$good[$j]	= $name;
				$j++;

	  		}  else {
	  			print("param_flow_blue, _set_good_labels_4item: unexpected ending\n"); 		
	  		}
	...
	$param_flow_blue->{_num_good_labels_aref}	= \@num_good_labels;
	$good_labels[$idx] 							= \@good;
	$param_flow_blue->{_good_labels_aref2} 		= \@good_labels;

	#print("	3. param_flow_blue,_set_good_labels4item,
	#num_good_labels= $num_good_labels[$idx] , 
	#labels/names are:	@{@{$param_flow_blue->{_good_labels_aref2}}[$idx]}\n");
...
sub	_set_good_values4item {
	...
	my ($j) ;
    	...
    	# print("1. param_flow_blue,_set_good_values4item, flow index:$idx, prog name:@{$param_flow_blue->{_prog_names_aref}}[$idx] \n");
    					
    my $values_aref  	= _get_values_aref($idx);
	my $length 			= scalar @$values_aref;
		# print("2. param_flow_blue,_set_good_values4item, length: $length\n");

	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 	# print("param_flow_blue, _set_good_values_4item: values_aref is @$values_aref[$i]\n");
	  	if ( defined(@{$values_aref}[$i])  && (@{$values_aref}[$i] ne $empty_string) ) {
	  		
	  		if (@$values_aref[$i] ne "'nu'" ) {
	
				my $value = ${@{$param_flow_blue->{_values_aref2}}[$idx]}[$i];
    				# print("2. param_flow_blue,_set_good_values4item,good index #$i\n");
    				# print("2. param_flow_blue,_set_good_values4item,value:$value \n");		
				$good[$j]	= $value;
				$j++;
	  		
	  		}  else {
	  			print("param_flow_blue, _set_good_values_4item: unexpected ending\n"); 		
	  		}
	  		
		} else {
			# print("param_flow_blue, _set_good_values_4item: no values are present, can not be saved\n"); 
		} 	
	} 

	$num_good_values[$idx] 				= $j;
			# print("param_flow_blue,_set_good_values4item,good_values=@good \n");
						
	$param_flow_blue->{_num_good_values_aref} = \@num_good_values;
	
	$good_values[$idx] 					= \@good;
	$param_flow_blue->{_good_values_aref2} 	= \@good_values;

	# print("	3. param_flow_blue,_set_good_values4item,num_good_values= $num_good_values[$idx], values are:	@{@{$param_flow_blue->{_good_values_aref2}}[$idx]}\n");
	return();
}
...
=cut
 ...
 sub clear {
 	...
 	$param_flow_blue->{_checkbuttons_aref}			= '';
	$param_flow_blue->{_checkbuttons_aref2}			= '';
	$param_flow_blue->{_destination_index}			= '';
	$param_flow_blue->{_end}						= '';
	$param_flow_blue->{_first_idx}					= 0;
	$param_flow_blue->{_flow_prog_names_aref}		= '';
	$param_flow_blue->{_good_checkbuttons_aref2}	= '';
	$param_flow_blue->{_good_labels_aref2}			= '';
	$param_flow_blue->{_good_values_aref2}			= '';
	$param_flow_blue->{_index2move}					= '';
	...
	$param_flow_blue->{_label_boxes_w}				= '';
	$param_flow_blue->{_length}						= '';
	$param_flow_blue->{_names_aref}					= '';
	$param_flow_blue->{_names_aref2}				= '';
	$param_flow_blue->{_num_good_values_aref}		= 0;
	$param_flow_blue->{_num_good_labels_aref}		= 0;
	...
	$param_flow_blue->{_selection_index}		    = '';
	$param_flow_blue->{_start}						= 0;
...
sub clear_flow_items_version_aref {
   ...
   if ($param_flow_blue->{_prog_version_aref}) {
   	
     $param_flow_blue->{_prog_version_aref} 	= '';
     
     
     # if ($param_flow_blue->{_prog_version_aref} eq '') {
     	
     # 	print("param_flow_blue, clear_flow_items_version_aref, param_flow_blue->{_prog_version_aref} = '' \n");
     # }
     
   } else {   	
   		print("param_flow_blue, clear_flow_items_version_aref, missing program_version_aref\n");  	
   }
...
=cut
 ...
 sub delete_selection {
  ...
  my $first				= 0;
  $param_flow_blue->{_index2delete} 	= $index2delete;
  ...
  if ( $index2delete >= 0 && $index2delete < $end )  {
    ...
    for ( my $i=$index_after, my $j=$index2delete;  $i<=$end; $i++,$j++) {
     	 				# print("Prog names B4 delete  @{$param_flow_blue->{_prog_names_aref}}	\n");

     	@{@{$param_flow_blue->{_checkbuttons_aref2}}[$j]}    = @{@{$param_flow_blue->{_checkbuttons_aref2}}[$i]};
     	@{@{$param_flow_blue->{_names_aref2}}[$j]}			= @{@{$param_flow_blue->{_names_aref2}}[$i]};
     	@{@{$param_flow_blue->{_values_aref2}}[$j]}			= @{@{$param_flow_blue->{_values_aref2}}[$i]};
     	@{$param_flow_blue->{_prog_names_aref}}[$j]			= @{$param_flow_blue->{_prog_names_aref}}[$i];
    } 
...
=cut 
 ...
 sub first_idx {
  ...
  $param_flow_blue->{_first_idx} = 0;
  return($param_flow_blue->{_first_idx});
...
my ($i,$j, $length);
 		...
 		@values_aref 	= @{@{$param_flow_blue->{_checkbuttons_aref2}}[$index]};
 		$length			= scalar @values_aref;

   		print("param_flow_blue,get_check_buttons_settings: is @values_aref\n");

  		#for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
  		for ($i=0; $i < $length; $i++ ) {
    		#$values[$j]  = $values_aref[$i]; 
    			#print("param_flow_blue, get_check_buttons_settings :index $j values: $values[$j]\n");
     	if($values_aref[$i] eq $nu || ) {
     	  	$on_off[$i]     = $off;
		  	# print(" 1. param_flow_blue, get_check_buttons_settings,$on_off[$i]\n");
     	}
     	else {
       		$on_off[$i]     = $on;
		    # print(" 2. param_flow_blue, get_check_buttons_settings,$on_off[$i]\n");
     	}
     	# print("param_flow_blue: get_check_buttons_settings :index $i setting $nu is: $on_off[$i]\n");
   		}
 ...
 sub get_flow_prog_names_aref {
	...
	if( $param_flow_blue->{_prog_names_aref} ne $empty_string) {
		...
		return($hash->{_prog_names_aref});
   		...
   		# print("param_flow_blue, no program names exist \n");
	}
 }
...
sub get_good_values_aref2 {
   my ($self) = @_;
	...
	if ( $param_flow_blue->{_good_values_aref2} ){
		my $good_values_aref2 = $param_flow_blue->{_good_values_aref2};
						 # print(" param_flow_blue,get_good_values_aref2,
						 # good_values for index 0= :
						 # @{@{$param_flow_blue->{_good_values_aref2}}[0]}\n");
...
=cut
 ...
 sub get_names_aref {
	...
	if ($index >=0) {
		...
		@names_aref 	= @{@{$param_flow_blue->{_names_aref2}}[$index]};
		$length 		= scalar @names_aref;

		# print(" param_flow_blue, get_names_aref:  @names_aref, index is $index\n");
		return (\@names_aref);
  } else {
  	print(" param_flow_blue, get_names names:index <0 \n");
  }
 }
...
sub get_num_good_labels_aref {
	...
	if ( $param_flow_blue->{_num_good_labels_aref} ){
		my $num_good_labels_aref = $param_flow_blue->{_num_good_labels_aref};
...
=cut
 ...
 sub get_num_items {
   ...
   if ( $param_flow_blue->{_num_items} >= 0 ) {
		my $num_items =  $param_flow_blue->{_num_items4flow};
   	
		# print("param_flow_blue,get_num_items, num_items = $param_flow_blue->{_num_items} \n");
		return($num_items);
   	   
   } else {
   	print("param_flow_blue,get_num_items,- no number of items \n");
   	return();
   }
...
=cut
 ...
 sub get_values_aref {
 	my ($self) 	= @_ ;
 		...
 		# print("param_flow_blue, get_values :_selection_index $param_flow_blue->{_selection_index}\n");
 		   
  	if ( ($param_flow_blue->{_selection_index} >=0) && 
  	    $param_flow_blue->{_values_aref2}) {
  	    	
  		my $index = $param_flow_blue->{_selection_index};
  		# print("param_flow_blue, get_values :_values_aref2:@{@{$param_flow_blue->{_values_aref2}}[$index]}\n");	
  		my ($i,$j,$length);
  		my (@values_aref,@values);
  			# print("param_flow_blue, get_values :index $index\n");

 		@values_aref 	= @{@{$param_flow_blue->{_values_aref2}}[$index]};
  		$length			=  scalar @values_aref;
   			# print("param_flow_blue,get_values :values_aref is @values_aref\n");

  		for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
    		$values[$j]  = $values_aref[$i]; 
    		# print("param_flow_blue, get_values :index $j values: $values[$j]\n");
  		}
  		return (\@values_aref);		

	} else {		
		print ("param_flow_blue,get_values_aref :selection_index <=0  or values_aref2\n");
		return();
	} 
 } 
...
=cut
 ...
 sub insert_selection {
	my ($self) 	= @_;
	my $first				= 0;
	my $idx2mv				= $param_flow_blue->{_index2move};
	my $destn_idx			= $param_flow_blue->{_destination_index};
	my $end					= $param_flow_blue->{_indices};
	my $num_items			= $param_flow_blue->{_num_items};
	...
	$tmp_prog_name			= @{$param_flow_blue->{_prog_names_aref}}[$idx2mv];
    @tmp_names_aref			= @{@{$param_flow_blue->{_names_aref2}}[$idx2mv]};
    @tmp_values_aref 		= @{@{$param_flow_blue->{_values_aref2}}[$idx2mv]};
   	@tmp_checkbuttons_aref	= @{@{$param_flow_blue->{_checkbuttons_aref2}}[$idx2mv]};

 				 		 # print(" param_flow_blue,insert_selection,mobile prog name: $tmp_prog_name	 \n");		
	...
	if ($idx2mv > $first) {
		...
		for (my $i=$first,my $j=$first; $j<$idx2mv; $i++,$j++){
       		$swap_prog_names[$i] 		= @{$param_flow_blue->{_prog_names_aref}}[$j];
       		$swap_names_aref[$i] 		= clone(\@{@{$param_flow_blue->{_names_aref2}}[$j]});
       		$swap_values_aref[$i] 		= clone(\@{@{$param_flow_blue->{_values_aref2}}[$j]});
       		$swap_checkbuttons_aref[$i] = clone(\@{@{$param_flow_blue->{_checkbuttons_aref2}}[$j]});
	...
	if ($destn_idx > $first) { # assume $destn_idx > 0 
		...
		for (my $i=$first, my $j=$first; $i<$destn_idx; $i++,$j++){
       		@{$param_flow_blue->{_prog_names_aref}}[$i] 		    = $swap_prog_names[$j];
   			@{@{$param_flow_blue->{_names_aref2}}[$i]}   		= @{$swap_names_aref[$j]};
   			@{@{$param_flow_blue->{_values_aref2}}[$i]}   		= @{$swap_values_aref[$j]};
   			@{@{$param_flow_blue->{_checkbuttons_aref2}}[$i]}   	= @{$swap_checkbuttons_aref[$j]};
						   # print(" 1. final vector at new index $i uses swap @{$swap_names_aref[$j]}\n");
						   # print(" 1. final vector at new index $i is @{@{$param_flow_blue->{_names_aref2}}[$i]}\n");
		}
		...
		for (my $i=($first+1), my $j=$first; $j<$end; $i++,$j++){
       		...
       		@{$param_flow_blue->{_prog_names_aref}}[$i] 		    = $swap_prog_names[$j];
   			@{@{$param_flow_blue->{_names_aref2}}[$i]}   		= @swp_nam_tr;
   			@{@{$param_flow_blue->{_values_aref2}}[$i]}   		= @{$swap_values_aref[$j]};
   			@{@{$param_flow_blue->{_checkbuttons_aref2}}[$i]}   	= @{$swap_checkbuttons_aref[$j]};
       		...
       		@{$param_flow_blue->{_prog_names_aref}}[$destn_idx] 		    = $tmp_prog_name;
   			@{@{$param_flow_blue->{_names_aref2}}[$destn_idx]}   		= @tmp_names_aref;
   			@{@{$param_flow_blue->{_values_aref2}}[$destn_idx]}  		= @tmp_values_aref;
   			@{@{$param_flow_blue->{_checkbuttons_aref2}}[$destn_idx]}   	= @tmp_checkbuttons_aref;
...
=cut 
 ...
 sub length {
 	...
 	$index 				= $param_flow_blue->{_selection_index}; 
 	if($index >= 0) {
 		@values_aref 	= @{@{$param_flow_blue->{_values_aref2}}[$index]};
 		$length			= scalar @values_aref;
...
#  						print("param_flow_blue, index: $index\n");
 		...
 		print("param_flow_blue,length,  index does not exist\n");
 		print("param_flow_blue, length, num values: $length\n");
 		return();
 	}
 }
 ...
 sub set_check_buttons_settings_aref {
	...
	my $chkbut_aref = $check_buttons_settings_aref;
  	...
  	if ($index >=0) {
 		...
 		@{$param_flow_blue->{_checkbuttons_aref2}}[$index] 	= $chkbut_aref;
 		 # $length			= scalar @$chkbut_aref;

   		# print("param_flow_blue,set_check_buttons_settings: is @$chkbut_aref\n");

  		#for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
  		#for ($i=0; $i < $length; $i++ ) {
    		#$values[$j]  = $values_aref[$i]; 
    			#print("param_flow_blue, set_check_buttons_settings :index $j values: $values[$j]\n");
     	#if(@$values_aref[$i] eq $nu) {
     	#  	$on_off[$i]     = $off;
		  	# print(" 1. param_flow_blue, set_check_buttons_settings,$on_off[$i]\n");
     	#}
     	#else {
       #		$on_off[$i]     = $on;
		    # print(" 2. param_flow_blue, set_check_buttons_settings,$on_off[$i]\n");
     	#}
     	# print("param_flow_blue: set_check_buttons_settings :index $i setting $nu is: $on_off[$i]\n");
   		#}
 		#return();
	}
	return();
...
=cut 
 ...
 sub set_flow_items_version_aref {
   ...
   if ($program_version_aref) {
     $param_flow_blue->{_prog_version_aref} 	= $program_version_aref;
     #print("param_flow_blue, set_flow_items_version_aref @{$param_flow_blue->{_prog_version_aref}}\n");
   }
  }
...
=head2 sub set_flow_index 
 ...
 select an item from which to extract data
      			...
      			print("param_flow_blue, set flow_index, prog name is @{$param_flow_blue->{_prog_names_aref}}[$index]\n");
 ...
 sub set_flow_index {
    ...
    if  ($index) { # if defined	
    	...
    	if ($index ne $empty_string && $index >= 0) {
			...
			$param_flow_blue->{_selection_index}	= $index;
		
		} else {
			print("param_flow_blue, set_flow_index,unexpected index, index:$index\n");
		}
		...
		$index = 0;
		$param_flow_blue->{_selection_index}	= $index;
		# print("param_flow_blue, set_flow_index,index does not exist, index:$index\n");		
	}
 ...
 sub set_names_aref {
  ...
  if ($index >=0) {
    ...
    my ($length);
  	...
  	@{$param_flow_blue->{_names_aref2}}[$index] = $names_aref;
    # $length 		= scalar @names_aref;
...
sub	set_good_values {
	...
	my($self) = @_;
	my $length = $param_flow_blue->{_num_items4flow}; # no. programs in flow

	  # print("param_flow_blue,set_good_values num_items4flow:	$param_flow_blue->{_num_items4flow}\n");
...
sub	set_good_labels4item {
	...
	my ($num_good_labels,$j) ;
    					...
    					# print("1. param_flow_blue,set_good_labels4item, 
    					 #flow index:$idx, prog name: 
    					 # @{$param_flow_blue->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
	  	...
	  	if (@$values_aref[$i] ne "'nu'" ) {
			my $name = ${@{$param_flow_blue->{_names_aref2}}[$idx]}[$i];
    					 #print("2. param_flow_blue,set_good_labels4item,
    					 #good index #$i, name:$name \n");
	...
	$param_flow_blue->{_good_labels_aref2} 	= \@good_labels;
						...
						# print("	3. param_flow_blue,set_good_,names4item,
						# num_good_labels= $num_good_labels , names are:
						# @{@{$param_flow_blue->{_good_labels_aref2}}[$idx]}\n");
	return();
}
...
sub	set_good_values4item {
	...
	my ($num_good_values,$j) ;
    					...
    					# print("1. param_flow_blue,set_good_values4item, 
    					# flow index:$idx, prog name: 
    					# @{$param_flow_blue->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
	  	...
	  	if (@$values_aref[$i] ne "'nu'" ) {
			my $value = ${@{$param_flow_blue->{_values_aref2}}[$idx]}[$i];
    					# print("2. param_flow_blue,set_good_values4item,
    					# good index #$i, value:$value \n");
    					
			$good[$j]	= $value;
			$j++;
		} else {
			print("param_flow_blue,set_good_values4item, a bad value detected\n");
	...
	$param_flow_blue->{_good_values_aref2} 	= \@good_values;
						...
						# print("	3. param_flow_blue,set_good_,values4item,
						# num_good_values= $num_good_values , values are:
						# @{@{$param_flow_blue->{_good_values_aref2}}[$idx]}\n");
	return();
}
...
=cut
 ...
 sub set_values_aref {
 	...
 	# print("param_flow_blue,set_values param_flow_blue->{_selection_index}= $param_flow_blue->{_selection_index}\n");
 	...
 	if ($param_flow_blue->{_selection_index} >= 0) {
 		...
 		if ($index >=0) {
 			...
 			@{$param_flow_blue->{_values_aref2}}[$index]	= $values_aref;
  			$length			=  scalar @values_aref;
   		 	# print("param_flow_blue,set_values :values_aref is @$values_aref\n");

  			for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
    			$values[$j]  = $values_aref[$i]; 
    			# print("param_flow_blue, set_values :index $j values: $values[$j]\n");
  			}
		} 		
 	} else {
 		print("param_flow_blue, set_values_aref: missing selection index\n");		
 	}
    		# print("param_flow_blue,set_values_aref :values_aref are @$values_aref[0]\n");
			# print("param_flow_blue,set_values_aref, index: $index\n");					
 return ();
 } 
...
=head2 sub stack_checkbuttons_aref2
  ...
  #for (my $i=0; $i<=$index;$i++) {
    #print("param_flow_blue,checkbuttons_aref,@{@{$param_flow_blue->{_checkbuttons_aref2}}[$i]} index $i\n");
  #}
     #print("param_flow_blue, checkbuttons_aref2, num_items4checkbuttons $param_flow_blue->{_num_items4checkbuttons}\n");
...
=cut
 ...
 sub stack_checkbuttons_aref2 { 
  ...
  $param_flow_blue->{_indices}      	= $index;
  $param_flow_blue->{_index4checkbuttons}++;
  $param_flow_blue->{_num_items4checkbuttons}++;
  $param_flow_blue->{_num_items}     = $param_flow_blue->{_num_items4checkbuttons};
   					# print("param_flow_blue,stack_checkbuttons_aref2,  @{$checkbuttons[$index]},idx $index num_items $param_flow_blue->{_num_items}\n");
   					# print("param_flow_blue,stack_checkbuttons_aref2,  @{@{$param_flow_blue->{_checkbuttons_aref2}}[$index]},idx $index num_items $param_flow_blue->{_num_items}\n");
...
=head2 sub stack_flow_item 
 ...
 later
     print("param_flow_blue, stack_flow_item @{$param_flow_blue->{_prog_names_aref}}, num_items $param_flow_blue->{_num_items}\n");
 	 print("param_flow_blue, stack_flow_item, index: $index\n");
...
=cut 
 ...
 sub stack_flow_item {
   ...
   if ($program_name_sref) {
     ...
     $param_flow_blue->{_prog_names_aref} 	= \@program_names;

     $param_flow_blue->{_indices}				= $index;
     $param_flow_blue->{_index4flow}			= $index;
     $param_flow_blue->{_num_items4flow}++;
     $param_flow_blue->{_num_items}++ ;
...
=head2 sub stack_names_aref2 
 ...
 DrBN
     ...
     #print("param_flow_blue, names_aref2, num_items4names $param_flow_blue->{_num_items4names}\n");

 #print("param_flow_blue,names_aref,$names[$index],item $index\n");
 # print("param_flow_blue, stack_names_aref2, names in program# $index are @{$names[$index]}[0]\n");
 for(my $i = 0; $i <=$index; $i++) {
   	print("param_flow_blue,stack_names_aref2, cumulative first names are:  @{$names[$i]}[0], prog num =$i\n");
 }
 ...
 sub stack_names_aref2 {
  ...
  if ($names_aref) {
  	my $index    						= $param_flow_blue->{_index4names}  + 1;
  		# print("param_flow_blue,names_aref2,index: $index\n");
  	$names[$index]						= $names_aref;
  	$param_flow_blue->{_names_aref2}			= \@names;
  	$param_flow_blue->{_indices}				= $index;
  	$param_flow_blue->{_index4names}++;
  	$param_flow_blue->{_num_items4names}++;
  	$param_flow_blue->{_num_items}			= $param_flow_blue->{_num_items4names};
#     
#  	for (my $i=0; $i<=$index;$i++) {
#     	print("param_flow_blue,stack_names_aref2, an accumulating array of arrays: @{$param_flow_blue->{_names_aref2}} item $i\n");
#  	} 
 	
  }else {  	
  	print("param_flow_blue,stack_names_aref2 missing names_aref \n");
  }
  return();
 }
...
DB
  #for (my $i=0; $i<=$index;$i++) {
   # print("param_flow_blue,values_aref2,@{@{$param_flow_blue->{_values_aref2}}[$i]} index $i\n");
  #}
  #print("param_flow_blue,values_aref2,num_items=$param_flow_blue->{_num_items}\n");
  # print("param_flow_blue,values_aref2, index $index\n");
 ...
 sub stack_values_aref2 {
	...
	my ($self,$values_aref) 			= @_;
   	...
   	if ($values_aref) {
  		...
  		$param_flow_blue->{_values_aref2}	= \@values;

  		$param_flow_blue->{_indices}       	= $index;
  		$param_flow_blue->{_index4values}++;
  		$param_flow_blue->{_num_items4values}++;
  		$param_flow_blue->{_num_items}         = $param_flow_blue->{_num_items4values};
  		
  		# print("param_flow_blue,stack_values_aref2, values: @{$values[$index]},idx $index num_items $param_flow_blue->{_num_items}\n");
  		     
#  		for (my $i=0; $i<=$index;$i++) {
#     		print("param_flow_blue,stack_values_aref2, an accumulating array of arrays: @{$param_flow_blue->{_values_aref2}} item $i\n");
#  		} 
  	
   	} else {
   		  	print("param_flow_blue,stack_values_aref2 missing values_aref \n");   		
   	}
    return();
 }
...
=head2 sub view_data
 ...
 Data viewer for debugging
   print("B4  delete: names are  $param_flow_blue->{_names_aref2}\n");    # ref_Array
   print("B4  delete: names are  @{$param_flow_blue->{_names_aref2}}\n");     # [ref_array0 ref_array1 .... 
   print("B4  delete: names are  @{$param_flow_blue->{_names_aref2}}[0]\n");  # ref_array0

   print("B4  delete: names are  @{$param_flow_blue->{_names_aref2}}[1]\n"); #ref_array1
     
   print("B4  delete: names are  @{@{$param_flow_blue->{_names_aref2}}[0]}\n"); # all names in ref_array 0 
   print("B4  delete: names are  @{@{$param_flow_blue->{_names_aref2}}[1]}[0]\n"); # first name  in ref_array 
   print("param_flow_blue,view_data: list for each item\n");
