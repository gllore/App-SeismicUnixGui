...
=cut
 ...
 sub insert_selection {
    ...
    @tmp_values_aref 		= @{@{$param_flow->{_values_aref2}}[$idx2mv]};
   	@tmp_checkbuttons_aref	= @{@{$param_flow->{_checkbuttons_aref2}}[$idx2mv]};
	...
	if ($idx2mv > $first) {
		...
		for (my $i=$first,my $j=$first; $j<$idx2mv; $i++,$j++){
       		...
       		$swap_names_aref[$i] 		= clone(\@{@{$param_flow->{_names_aref2}}[$j]});
       		$swap_values_aref[$i] 		= clone(\@{@{$param_flow->{_values_aref2}}[$j]});
       		$swap_checkbuttons_aref[$i] = clone(\@{@{$param_flow->{_checkbuttons_aref2}}[$j]});
	...
	if ($destn_idx > $first) { # assume $destn_idx > 0 
		...
		for (my $i=$first, my $j=$first; $i<$destn_idx; $i++,$j++){
   			...
   			@{@{$param_flow->{_names_aref2}}[$i]}   		= @{$swap_names_aref[$j]};
   			@{@{$param_flow->{_values_aref2}}[$i]}   		= @{$swap_values_aref[$j]};
   			@{@{$param_flow->{_checkbuttons_aref2}}[$i]}   	= @{$swap_checkbuttons_aref[$j]};
		...
		for (my $i=($first+1), my $j=$first; $j<$end; $i++,$j++){
   			...
   			@{@{$param_flow->{_checkbuttons_aref2}}[$destn_idx]}   	= @tmp_checkbuttons_aref;
 ...
 sub delete_selection {
  ...
  if ( $index2delete >= 0 && $index2delete < $end )  {
    ...
    for ( my $i=$index_after, my $j=$index2delete;  $i<=$end; $i++,$j++) {
     	...
     	@{@{$param_flow->{_checkbuttons_aref2}}[$j]}    = @{@{$param_flow->{_checkbuttons_aref2}}[$i]};
     	@{@{$param_flow->{_names_aref2}}[$j]}			= @{@{$param_flow->{_names_aref2}}[$i]};
     	@{@{$param_flow->{_values_aref2}}[$j]}			= @{@{$param_flow->{_values_aref2}}[$i]};
     	@{$param_flow->{_prog_names_aref}}[$j]			= @{$param_flow->{_prog_names_aref}}[$i];
    } 
...
=cut 
 ...
 sub set_flow_index {
	...
	if ($index >= 0) {
		...
		print("param_flow, set_flow_index,index does not exist, index:$index\n");
	}
	return();
 }
