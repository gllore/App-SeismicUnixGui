...
=cut
	...
	$project->{_superflow_first_idx}  	
   								= $config_superflows->first_idx();
   	$project->{_superflow_length}  		
   								= $config_superflows->length();
...
chosen:
     ...
     with the on ebelonging to the newly selected
     active project
...
=cut  
 ...
 sub _ok {
	...
	if ($project_selector->{_active_project}) { # extra security
		 ...
		 if ($length_check_buttons_on >1 ) {   # possible mistake by user 
		 			 		...
		 			 		# print("project_selector,_ok, >1 length_check_buttons_on: $length_check_buttons_on\n");
		 ...
		 # CASE 2 no buttons are selected ('on')	 	
		 	...
		 	if ($length == 0) { # i.e. no project names exist is confirmed
		 		# print("project_selector,_ok, length_project names $length\n");
		 ...
		 # CASE 3 an existing project is chosen
			...
			my $ACTIVE_PROJECT		   	= $user_constants		->get_ACTIVE_PROJECT;
		 	my $active_indices_aref		= $param_widgets_pkg 	->get_index_check_buttons_on();
		 	...
		 	my $from 	= $CONFIGURATION.'/'.$new_active_Project_name.'/Project.config';
...
#   		 		 print("project_selector,_ok, prog_name ${$project->{_prog_name_sref}} \n");
   		 ...
   		 if ( not $NEW_PROJECT_exists) {
   		 	...
   		 	my $TO_project_config		= $NEW_PROJECT.'/'.$name.'.config';
   		 		...
   		 		# print("project_selector,_ok, copying from $FROM_project_config to $TO_project_config\n");
...
=cut  
 ...
 sub ok {
 	...
 	if($value) {
 		...
 		_ok();
 		print("project_select,ok,value: $value\n");

 	} else {
 		print("project_select,ok,error, value: $value\n");	
 	}
