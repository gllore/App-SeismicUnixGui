...
=cut 
 ...
 sub  _is_unoccupied_listbox {
	...
	if ( @{$L_SU_gui->{_occupied_listbox_aref}} )  { 
		...
		if (@{$L_SU_gui->{_occupied_listbox_aref}}[0] == $true && $color eq 'grey')	 {
			
			return($false);	
			
		} elsif (@{$L_SU_gui->{_occupied_listbox_aref}}[1] == $true && $color eq 'pink'){

			return($false);
		
		} elsif (@{$L_SU_gui->{_occupied_listbox_aref}}[2] == $true && $color eq 'green'){
			
			return($false);
		
		} elsif (@{$L_SU_gui->{_occupied_listbox_aref}}[3] == $true && $color eq 'blue'){
 	...
 	if ( $L_SU_gui->{_occupied_listbox_aref} ) {
 		...
 		if ($flow_color)     {
 			...
 			if ($flow_color eq '' ) {	
 				...
 				@{$L_SU_gui->{_occupied_listbox_aref}}[0] = $L_SU->{_is_flow_listbox_grey_w};
 				...
 				@{$L_SU_gui->{_occupied_listbox_aref}}[0] = $L_SU->{_is_flow_listbox_grey_w};
 			 	...
 			 	@{$L_SU_gui->{_occupied_listbox_aref}}[1] = $L_SU->{_is_flow_listbox_pink_w};
 			 			
 			} elsif 	( $flow_color eq 'green' ) { 
 			
  				@{$L_SU_gui->{_occupied_listbox_aref}}[2] = $L_SU->{_is_flow_listbox_green_w};
  					
 			} elsif 	( $flow_color eq 'blue' ) {
 			
  				@{$L_SU_gui->{_occupied_listbox_aref}}[3] = $L_SU->{_is_flow_listbox_blue_w};
...
=head2 sub FileDialog_button
	...
	each colored flow will be directed to a different program
 	...
 	(so that we know in advance how many value are occupied without reading t)SeismicUnixPltTk_glocal_cosntants.pm
	...
	# Skip the listbox if there is no flow already in it
 	if ($color eq 'grey'  ||
		...
		# Skip the listbox if there is no flow already in it
 		...
 		# check to see if you are dealing with the right flow and whetherthe flow listbox
		# has a flow already in it?  Skip the listbox if there is no flow already  in it.  			 		
 		...
 		# sunix_select is chosen
 			...
 			# print("61 L_SU,user_built_flows, occupied_listboxes: @{$L_SU_gui->{_occupied_listbox_aref}}\n"); 
