...
sub _reset {
	my ($self) = @_;
		...
		$param_widgets_neutral->{_is_add2flow} 				= $false; # needed? double confirmation?		
		$param_widgets_neutral->{_is_add2flow_button} 			= $false; # needed? double confirmation?
...
sub get_check_buttons_w_aref {
	...
	if ( $param_widgets_neutral->{_check_buttons_w_aref} ) {
		...
		my $check_buttons_w_aref = $param_widgets_neutral->{_check_buttons_w_aref};
...
=cut
 ...
 sub local_update_check_button_setting {
    ...
    @{$param_widgets_neutral->{_check_buttons_settings_aref}}[$idx] = $on_off[$idx];
...
sub  set_check_buttons {
 ...
 if ($check_buttons_settings_aref) {
   $param_widgets_neutral->{_check_buttons_settings_aref} = $check_buttons_settings_aref;
...
=cut
 ...
 sub get_check_buttons_aref {
	my ($self) = @_;
    my $check_buttons_settings_aref = \@{$param_widgets_neutral->{_check_buttons_settings_aref}};
...
sub get_label4entry_button_chosen {
    ...
    for (my $choice = $first;  $choice < $length; $choice++) {  
        if ($widget eq @{$param_widgets_neutral->{_values_w_aref}}[$choice]) {
       		...
       		$label = @{$param_widgets_neutral->{_labels_w_aref}}[$choice]->cget('-text');
       		my $value = @{$param_widgets_neutral->{_values_w_aref}}[$choice]->get;
...
sub get_value4entry_button_chosen {
    ...
    for (my $choice = $first;  $choice < $length; $choice++) {  
        if ($widget eq @{$param_widgets_neutral->{_values_w_aref}}[$choice]) {
       		...
       		$value = @{$param_widgets_neutral->{_values_w_aref}}[$choice]->get;
...
sub  initialize_check_buttons  {
  ...
  $check_buttons	->frame($param_widgets_neutral->{_check_buttons_frame_href});
  $check_buttons	->make(\@off);

  $param_widgets_neutral->{_check_buttons_w_aref} = $check_buttons ->get_w_aref();
...
sub changes {
	...
	if ($param_widgets_neutral->{_current_program_name} ) {
		...
		if ($idx >= 0 &&   $idx <= $control->get_max_index($prog_name) ) {  # cautious, index must be reasonable 
    		...
    		my $changed_entry = $param_widgets_neutral->{_changed_entry}; #always
...
sub range {
  	my ($self, $ref_hash) 	= @_;
  		...
  		if( $param_widgets_neutral->{_is_add2flow_button} || $param_widgets_neutral->{_is_add2flow}) {
  			
  			$param_widgets_neutral->{_first_idx}		= $ref_hash->{_param_sunix_first_idx};
		...
		# for user-built flows
       	if( $param_widgets_neutral->{_is_flow_listbox_grey_w}  || 
  			...
  			$param_widgets_neutral->{_first_idx}		= $ref_hash->{_param_flow_first_idx};
		...
		# button for L_SU and no button for project selector
       	if( $ref_hash->{_is_superflow_select_button} || $ref_hash->{_is_superflow})  {  
       	
  			$param_widgets_neutral->{_first_idx}		= $ref_hash->{_superflow_first_idx};
...
=cut
 ...
 sub redisplay_check_buttons{
	my ($self) 	    	= @_;
  	...
  	my $first 	    	= $param_widgets_neutral->{_first_idx};
  	my $length  	    = scalar @{$param_widgets_neutral->{_check_buttons_settings_aref}}; # $param_widgets_neutral->{_length};
...
sub set_check_buttons_frame{
   ...
   $param_widgets_neutral->{_check_buttons_frame_href} = $check_buttons_frame_href;
...
=cut
 ...
 sub set_location_in_gui {
	...
	if 	( $here->{_is_flow_listbox_grey_w} &&  $here->{_is_add2flow} ) 	{
		...
		$param_widgets_neutral->{_is_new_listbox_selection} = $true; # so change not allowed
		...
		$param_widgets_neutral->{_is_new_listbox_selection} = $true; # so change not allowed
		 # print("param_widgets_neutral, set_location_in_gui, _is_new_listbox_selection \n");
  		...
  		_reset();
		$param_widgets_neutral->{_is_new_listbox_selection} = $true; # so changes not allowed
...
sub set_value4entry_button_chosen {
    ...
    for (my $choice = $first;  $choice < $length; $choice++) {  
        if ($widget eq @{$param_widgets_neutral->{_values_w_aref}}[$choice]) {
       		...
       		@{$param_widgets_neutral->{_values_w_aref}}[$choice]->configure(-text => $value);

			print("param_widgets_neutral,set_entry_button_chosen,value= @{$param_widgets_neutral->{_values_w_aref}}[$choice]\n");
