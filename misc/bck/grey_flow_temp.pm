...
sub _FileDialog_button {
	...
	if ($option_sref) {    # e.g., can be 'Data' or 'Flow'
		...
		# Update the parameter_widgets with the updated value
...
# user-built_flow in current use
		...
		if ( $current_flow_listbox_index >= 0 ) {    # >=0 11-14-2018 JML
			...
			$param_flow_color_pkg->set_values_aref(
				$color_flow->{_values_aref} );       # but not the versions
			$param_flow_color_pkg->set_names_aref( $color_flow->{_names_aref} )
...
sub _set_flow_listbox_color_w {
	...
	if ( $flow_color eq $this_color ) {
		...
		$color_flow->{_flow_listbox_color_w} =
			$color_flow->{_flow_listbox_grey_w};
...
sub _set_is_last_parameter_index_touched_color {
	...
	if ( $flow_color eq $this_color ) {
		...
		$color_flow->{_is_last_parameter_index_touched_color} =
			$color_flow->{_is_last_parameter_index_touched_grey};

		print(
			"color_flow,_set_is_last_parameter_index_touched_color, is_last_parameter_index_touched_grey: $color_flow->{_is_last_parameter_index_touched_grey}\n"
		);
...
sub _set_last_parameter_index_touched_color {
	...
	if ( $flow_color eq $this_color ) {
		...
		# not for export
		$color_flow->{_last_parameter_index_touched_color} =
			$color_flow->{_last_parameter_index_touched_grey};

		print(
			"color_flow,_set_last_parameter_index_touched_color, last_parameter_index_touched_grey: $color_flow->{_last_parameter_index_touched_grey}\n"
		);
...
# Here is where you update the entry button value that displays the currently active
