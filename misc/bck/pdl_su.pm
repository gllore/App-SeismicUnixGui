...
sub	_set_inbound_file_path {
	...
	if ($pdl_su->{_inbound_suffix_type} ne $empty_string ) {
		...
		print("pdl_su,_set_inbound_file_path, missing parameter for file path\n");
	}
...
sub	get_seismic_plot_x_offset {
	...
	if ($pdl_su->{_seismic_page_width}  ne $empty_string  &&
		...
		my $seismic_plot_width      = $seismic_page_height * $seismic_page_height / $seismic_page_width;
		my $seismic_plot_x_offset 	= 0.5 * ($seismic_plot_width - $seismic_page_width);
...
sub read_bytes {
	...
	if ( $pdl_su->{_inbound_base_file_name} 	ne $empty_string  &&
    	...
    	use autodie; 	 	
     	...
     	# Open the file 
		my $file = $pdl_su->{_inbound_file_path}.'/'.$pdl_su->{_inbound_base_file_name};
	...
	if ( $pdl_su->{_inbound_suffix_type} 		eq  'su' 		&&
		...
		$pdl_su->{_sample_interval_us} = $dt * 1000;
     	...
     	($pdl_su->{_number_of_traces}, 
     	 ...
     	 $pdl_su->{_samples_per_trace})  			= ($ntr,$dt,$ns);	
		...
		print("pdl_su,_su2pdl,bin2pdl, missing file name, path or suffix type\n");
	}
