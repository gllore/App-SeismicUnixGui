 ...
 sub delete_selection {
  ...
  if ($index2delete >=0 ) {   # move selection one up
  	if ($index2delete >0) {
     	...
     	$flow_listbox_w	->selectionSet(($index2delete-1)); #last item to delete 
...
sub drag_start {
    ...
    if ($idx >= 0) {
        ...
        $token->configure( -text 		=> $site->get($idx), 
			   				-background => 'white',
 ...
 sub get_flow_selection {
   my ($self,$flow_listbox_w) 			= @_;
	...
	if ($flow_listbox_w ne $empty_string) {
   	 ...
   	 print("flow_widgets,get_flow_selection, missing flow_listbox widget \n");
   	 return();
   }
