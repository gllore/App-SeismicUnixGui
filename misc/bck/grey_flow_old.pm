...
sub _FileDialog_button {
	...
	if ($option_sref) {    # e.g., can be 'Data' or 'Flow'
		...
		@{ $color_flow->{_values_aref} }[$current_index] =
			$file_dialog->get_selected_file_name();
		$param_widgets->redisplay_values();
...
sub _check4flow_changes {
	...
	if ( $color_flow->{_last_flow_index_touched} >= 0 ) {    # must be defined but can be =0
		...
		if (( $color_flow->{_last_flow_index_touched} >= 0 || $color_flow->{_last_parameter_index_touched_color} >= 0 )
...
=cut
		...
		|| ( $color_flow->{_last_flow_color} eq $empty_string ) )
	{
		...
		print(
			"color_flow,flow_select, color_flow->{_last_flow_index_touched}=$color_flow->{_last_flow_index_touched}\n"
		);
		$color_flow->{_last_parameter_index_touched_color} = 0;
...
=cut
	...
	#		)
	#	{
	# print("color_flow,flow_select CASE #3\n");
	# print(
	# 	"1 color_flow , flow_select, last_flow_color is:  $color_flow->{_last_flow_color}\n"
	# );
	# print(
	# 	"1 color_flow , flow_select, flow_color is:  $color_flow->{_flow_color}\n"
	# );
	#print("1 color_flow , flow_select, this flow color is:  $this_color\n");
	# print(
	# 	"1 color_flow , flow_select, _is_last_flow_index_touched_grey:  $color_flow->{_is_last_flow_index_touched_grey}\n"
	# );
	# print(
	# 	"1 color_flow , flow_select, _is_last_flow_index_touched_gpink:  $color_flow->{_is_last_flow_index_touched_pink}\n"
	# );
	# print(
	# # 	"1 color_flow , flow_select, _is_last_flow_index_touched_green:  $color_flow->{_is_last_flow_index_touched_green}\n"
	# );
	# print(
	# 	"1 color_flow , flow_select, _is_last_flow_index_touched_blue:  $color_flow->{_is_last_flow_index_touched_blue}\n"
	# );
	# print(
	# 	"1 color_flow , flow_select, last_flow_index_touched:  $color_flow->{_last_flow_index_touched}\n"
	# );
	#	}
	else {
		print("color_flow,flow_select NADA\n");
	}
