...
=head2 NOTES
	...
	V 0.0.3 Dec. 1 2019 updates new parameter values automatically
...
sub update_all_new_values_preemptively {
	...
	# print("4 param_widgets_color,update_all_new_values_preemptively\n");

	for ( my $i = 0; $i < $length; $i++ ) {

		my $new_entry_value = ( @{$values_w_aref}[$i] )->get();
		# print("5 param_widgets_color, _changes new_entry_value=$new_entry_value\n");
		@{$values_aref}[$i] = $new_entry_value;
	}

	# print("6 param_widgets_color, _changes new value in array:@{$param_widgets_color_href->{_values_aref}}[$current_param_index]\n");
...
=head2 _changes
 ...
 current_index = that of an individual parameter line 
...
sub _changes {
	...
	# two cases possible
	# in general L_SU
	# gui_history values are already initialized
	my $last_entry_index = ( $param_widgets_color_href->{_parameter_index_on_entry_href} )->{_most_recent};
	my $last_exit_index  = ( $param_widgets_color_href->{_parameter_index_on_exit_href} )->{_most_recent};

	# print("param_widgets_color,last_entry_index=$last_entry_index last_exit_index=$last_exit_index\n");

	if ( $param_widgets_color_href->{_current_program_name} ) {

		my $prog_name = $param_widgets_color_href->{_current_program_name};

		if ( $current_index == $last_entry_index ) {

			# still within the same parameter line; NADA
			# indices have not changed although content may have
			# will update the value always below

		}
		elsif ( $current_index != $last_entry_index ) {

			$gui_history->set_parameter_index_on_entry($current_index);
			$gui_history->set_parameter_color_on_entry($this_color);
			$gui_history->set_parameter_index_on_exit($last_entry_index);

			# print("2 param_widgets_color, _changes, current_param_index: $current_index\n");

		}
		else {
			print(" 1. param_widgets_pink, _changes, unexpected result\n");
		}

		# print(" param_widgets_pink,max_index, $max_idx \n");
		# cautious, index must be reasonable
		if (    $current_index >= 0
			and $current_index <= $control->get_max_index($prog_name) )
		{
			_set_entry_change_status($true);
			my $changed_entry = $param_widgets_color_href->{_changed_entry};    # always true!

			# print("param_widgets_pink,changes,changed 1-yes 0 -no? $changed_entry\n");
			_update_check_button_setting($current_index);

			# _update_values($idx);
			_set_index_on_entry($current_index);                                # TODO for depracation
		}

	}
	else {
		# print(" param_widgets_pink, _changes, bad name TODO \n");
	}

	# for Entry widget to say there is no error( always)
...
sub get_entry_change_status {
	...
	# print("param_widgets_pink, get_entry_change_status,changed_entry: $param_widgets_color_href->{_changed_entry}\n");
	return ($status);
}
...
#	my $wipe = new wipe();
#
#	# print("param_widgets_pink, gui_clean, _values_w_aref, $param_widgets_color_href->{_values_w_aref} \n");
#	# print("param_widgets_pink, gui_clean, _labels_w_aref, $param_widgets_color_href->{_labels_w_aref} \n");
...
sub gui_full_clear {
	...
	# print("param_widgets_pink, gui_full_clear, temp save length $param_widgets_color_href->{_length} \n");
	_max_length_in_gui();

	# print("param_widgets_pink, gui_full_clear, length used for cleaning $param_widgets_color_href->{_length} \n");

	# print("param_widgets_pink, gui_full_clear, _values_w_aref, $param_widgets_color_href->{_values_w_aref} \n");
	# print("param_widgets_pink, gui_full_clear, _labels_w_aref, $param_widgets_color_href->{_labels_w_aref} \n");
	$wipe->range($param_widgets_color_href);
	$wipe->values();
	$wipe->labels();
	$wipe->check_buttons();
	...
	#print("param_widgets_pink, gui_full_clear, restored length $param_widgets_color_href->{_length} \n");
...
sub _set_entry_change_status {
	...
	# print("param_widgets_pink,_set_entry_change_status,changed_entry,is: success \n");
...
sub _update_check_button_setting {
	...
	if ( !( $values[$idx] ) ) {
		...
		if ( defined( $values[$idx] ) ) {
			...
			if ( $values[$idx] eq '0' ) {
				...
				# print("1.20 param_widgets_pink,,_update_check_button_setting index: $idx , value:$values[$idx] or '0' \n");
				...
				# print("1.21 param_widgets_pink,_update_check_button_setting, ndex: $idx , value:$values[$idx] or 0.0 \n");
				...
				# print("1.22 param_widgets_pink,,_update_check_button_setting index: $idx , value:$values[$idx] or '0' \n");
				...
				# print("1.23 param_widgets_pink,_update_check_button_setting, ndex: $idx , value:$values[$idx] or 0.0 \n");

			}
			else {

				# print("1.24 param_widgets_pink,_update_check_button_setting; empty value,  :index $idx value: $values[$idx]\n");
				$on_off[$idx] = $off;
			}
			...
			# print("param_widgets_pink,_update_check_button_setting; unexpected cases\n");
			$on_off[$idx] = $off;
		}
		...
		if ( $values[$idx] eq "0" ) {
			...
			# print("1.11.23 1 param_widgets_pink,_update_check_button_setting, index: $idx value: $values[$idx]\n");
			...
			# print("1.12 param_widgets_pink,_update_check_button_setting, ndex: $idx , value:$values[$idx] or 0.0 \n");
			...
			# print("1.13 param_widgets_pink,_update_check_button_setting, \"\" \n");

		}
		elsif ( $values[$idx] eq '' ) {

			# print("1.14 param_widgets_pink,_update_check_button_setting, \'\' \n");
			$on_off[$idx] = $off;
			...
			# print("1. param_widgets_pink,_update_check_button_setting, \"\'nu\'\"  \n");

		}
		elsif ( $values[$idx] eq $nu ) {
			$on_off[$idx] = $off;

			# print("1.15 param_widgets_pink,_update_check_button_setting, \$nu \n");

		}
		elsif ( $values[$idx] eq $no ) {
			$on_off[$idx] = $off;

			# print("2. param_widgets_pink,_update_check_button_setting, \$no \n");

		}
		elsif ( $values[$idx] eq "'0'" ) {
			$on_off[$idx] = $on;

			# print("2.1 param_widgets_pink,_update_check_button_setting, \"\'0\'\" \n");

		}
		else {
			# print("3.1 param_widgets_pink,_update_check_button_setting, all else\n");
			$on_off[$idx] = $on;
		}
	...
	# print("param_widgets_pink: _update_check_button_setting :index $idx setting is: $on_off[$idx]\n");
	# print("param_widgets_pink: update_check_buttons_settings_aref @{$param_widgets_color_href->{_check_buttons_settings_aref}}\n");

	return ();
}

#=head2 sub _update_value
#
# update value for one parameter index
# in currently active program
# Entry widget uses textvariables
#
#=cut
...
#	my ($current_index) = @_;
#
#if ($current_index ne $empty_string) {
#
#	my $parameter_values_frame        = $param_widgets_color_href->{_parameter_values_frame};
#
#	my $ans = @$values_w_aref[$current_index]->;
#
#
#	@{ $param_widgets_color_href->{_values_aref} }[$current_index] $param_widgets_color_href->{_values_aref};
#}else {
#	print("param_widgets_pink: _update_value bad current index\n");
#}
#
#	return ();
#}
...
sub _set_index_on_entry {
	...
	# print("1. param_widgets_pink,_set_index_on_entry:  $param_widgets_color_href->{_index_on_entry} \n");
	return ();
}
...
sub set_check_buttons {
	...
	if ($check_buttons_settings_aref) {
		...
		# print("param_widgets_pink,set_check_buttons, settings are @{$param_widgets_color_href->{_check_buttons_settings_aref}}\n");
	}
	return ();
}
...
#	my $check_buttons_aref          = $check_buttons_settings_aref;
#
#	# print("param_widgets_pink,get_check_buttons_settings_aref: @{$param_widgets_color_href->{_check_buttons_settings_aref}}\n");
#	return ($check_buttons_aref);
#}
...
sub get_value4entry_button_chosen {
	...
	# print("param_widgets_pink,get_entry_button_chosen,first_index=$param_widgets_color_href->{_first_idx}\n");
	# print("param_widgets_pink,get_entry_button_chosen,length_=$param_widgets_color_href->{_length} \n");
...
sub get_values_aref {
	...
	if ( $param_widgets_color_href->{_values_aref} ne $empty_string ) {
		...
		# print("param_widgets_pink,get_values_aref,values=--@{$param_widgets_color_href->{_values_aref}}[0]--\n");
		# print("param_widgets_pink,get_values_aref,values=--@{$param_widgets_color_href->{_values_aref}}[1]--\n");
		return ($values_aref);
...
sub get_labels_aref {
	...
	#	print("param_widgets_pink,get_labels_aref: label 0 @{$param_widgets_color_href->{_labels_aref}}[0]\n"); # all labels in array may not be there
	#	print("param_widgets_pink,get_labels_aref:label 1 @{$param_widgets_color_href->{_labels_aref}}[1]\n"); # all labels in array may not be there
...
sub get_names_aref {
	...
	print("param_widgets_pink,get_labels_aref: @{$param_widgets_color_href->{_labels_aref}}\n");    # all labels in array may not be there
	return ( $param_widgets_color_href->{_labels_aref} );
}
...
sub range {
	...
	# print(" 0. param_widgets_pink2,range, key is $key, value is $value\n");
	# print(" 0. param_widgets_pink2,range, write out gui_history.txt\n");
	# $gui_history->view();
		...
		|| $param_widgets_color_href->{_is_add2flow} )
	{
		...
		# print("1.param_widgets_pink2,range,  (add2flow_button and add2flow)  first idx:$param_widgets_color_href->{_first_idx}, and length:$param_widgets_color_href->{_length}\n");

		# print("1.param_widgets_pink2,range,  (add2flow_button and add2flow)  if length is empty that is Ok the first time through \n");
	}
	...
	# for sunix selections
		...
		print("2. param_widgets_pink,range, (sunix_listbox) first idx:$param_widgets_color_href->{_first_idx}, and length:$param_widgets_color_href->{_length}\n");
	}
		...
		)
	{
		...
		print("3. param_widgets_pink,range, (user-built-flow)  first idx:$param_widgets_color_href->{_first_idx}, and length:$param_widgets_color_href->{_length}\n");
	}
		...
		|| $ref_hash->{_is_superflow} )
	{
		...
		print("4. param_widgets_pink,range, (superflows) first idx:$param_widgets_color_href->{_first_idx}, and length:$param_widgets_color_href->{_length}\n");
		...
		#	 		$param_widgets_color_href->{_first_idx}		= $ref_hash->{_param_flow_first_idx};
		#  			$param_widgets_color_href->{_length}			= $ref_hash->{_param_flow_length};
		#	 		print("5. param_widgets_pink,range, (user-built-flow)  first idx:$param_widgets_color_href->{_first_idx}, and length:$param_widgets_color_href->{_length}\n");
		#
		#		}
	}
	else {
		print("6. param_widgets_pink,range, _missing ref_hash NADA\n");
	}

	# print("6. param_widgets_pink,range, _values_w_aref:  $param_widgets_color_href->{_values_w_aref}\n");
	return ();
}
...
sub redisplay_check_buttons {
	...
	# print("2. param_widgets_pink,redisplay_check_buttons,settings @{$param_widgets_color_href->{_check_buttons_settings_aref}}[0]\n");
...
sub redisplay_labels {
	...
	# print("param_widgets_pink,redisplay_labels, length=$length\n");
	# print("param_widgets_pink,redisplay_labels, first=$first\n");
	if ($labels_w_aref) {
		for ( my $i = $first; $i < $length; $i++ ) {

			# print("i:$i   param_widgets_pink2,redisplay_labels length:$length\n");
...
sub redisplay_values {
		...
		&& $values_aref )
	{
		...
		for ( my $i = $first; $i < $length; $i++ ) {
			...
			# print("1. param_widgets_pink,redisplay_values,chkbtn @{$param_widgets_color_href->{_check_buttons_settings_aref}}[$i]\n");
			...
			#			if ( $i == 0 ) {
			#				print("2. param_widgets_pink, redisplay_values, quoteless value is @{$values_aref}[$i]\n");
			#			}
			...
			#			if ( $i == 0 ) {
			#				print("3. param_widgets_pink, redisplay_values, quoteless value is @{$values_aref}[$i]\n");
			#			}
		}

		# print("2. param_widgets_pink,redisplay_values,chkbtn @{$param_widgets_color_href->{_check_buttons_settings_aref}}[$i]\n");
	}
	else {
...
sub set_current_program {
	...
	if ($prog_name_sref) {
		...
		# print("param_widgets_pink,set_current_program, program name: $param_widgets_color_href->{_current_program_name}\n");
	}
}
...
sub set_length {
	...
	if ($length) {
		...
		# print("param_widgets_pink,set_length = $param_widgets_color_href->{_length}\n");
...
sub set_values_w_aref {
	...
	if ($values_w_aref) {
		...
		# print("param_widgets_pink,set_values_w_aref,  $param_widgets_color_href->{_values_w_aref}\n");
...
sub set_labels {
	...
	# print("param_widgets_pink,set_labels\n");
	if ( defined $labels_aref ) {
		...
		# print("param_widgets_pink,set_labels, length=$length\n");
		# print("param_widgets_pink,set_labels, llabels: @{$param_widgets_color_href->{_labels_aref}}\n");
	}
	else {
...
sub set_value4entry_button_chosen {
	...
	# print("param_widgets_pink,set_entry_button_chosen,length_=$param_widgets_color_href->{_length} \n");
	...
	for ( my $choice = $first; $choice < $length; $choice++ ) {
		if ( $widget eq @{ $param_widgets_color_href->{_values_w_aref} }[$choice] ) {
			...
			print("param_widgets_pink,set_entry_button_chosen,value= @{$param_widgets_color_href->{_values_w_aref}}[$choice]\n");
...
sub set_values {
	...
	if ( defined $values_aref ) {
		...
		# print("param_widgets_pink,set_values,@{$param_widgets_color_href->{_values_aref}}\n");
