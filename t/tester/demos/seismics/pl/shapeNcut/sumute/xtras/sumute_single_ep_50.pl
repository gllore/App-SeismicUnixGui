...
=cut
  ...
  my $first_file 				= 1;
  my $last_file  				= 48;
  my $input_parfilename     = 'itop_mute_par_All_cmp300_ep';

for ($i=$first_file; $i<=$last_file;$i++) {
  $su_inbound[$i]  		= $DATA_SEISMIC_SU.'/'.'split_ep'.$i.'50'.'.su';
  $su_outbound[$i]  	= $DATA_SEISMIC_SU.'/'.'split_ep'.$i.'50'.'_muted'.'.su';
  
  $parfilename[$i]  	= $input_parfilename.$i.'50';
