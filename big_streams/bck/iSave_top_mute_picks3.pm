...
=head2 SYNOPSIS 
 ...
 PROGRAM NAME:iSave_top_mute_picks3.pm
 AUTHOR:  Juan Lorenzo
 DATE:   September 2015  
 Version: 3.0 
...
sub clear {
    $iSave_top_mute_picks3->{_gather_num}    = '';
    $iSave_top_mute_picks3->{_gather_type}   = '';
    $iSave_top_mute_picks3->{_gather_header} = '';
    $iSave_top_mute_picks3->{_inbound}       = '';
    $iSave_top_mute_picks3->{_outbound}      = '';
}
...
sub file_in {
    ...
    $iSave_top_mute_picks3->{_file_in} = $file_in if defined($file_in);

    #print("file name is $iSave_top_mute_picks3->{_file_in} \n\n");
...
sub gather_num {
    ...
    $iSave_top_mute_picks3->{_gather_num} = $gather_num
...
sub gather_type {
    ...
    $iSave_top_mute_picks3->{_gather_type} = $gather_type
...
sub gather_header {
    ...
    $iSave_top_mute_picks3->{_gather_header} = $gather_header
...
sub calc {    #
    ...
    $iSave_top_mute_picks3->{_inbound} =
      $itemp_top_mute_picks_sorted_par_ . $iSave_top_mute_picks3->{_file_in};

    $iSave_top_mute_picks3->{_outbound} =
        $PL_SEISMIC . '/'
      . $itop_mute_par_
      . $iSave_top_mute_picks3->{_file_in}
      . $suffix;

    $cp->from( $iSave_top_mute_picks3->{_inbound} );
    $cp->to( $iSave_top_mute_picks3->{_outbound} );
    $cp[1] = $cp->Step();
