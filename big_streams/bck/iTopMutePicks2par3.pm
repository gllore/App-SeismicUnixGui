...
=head2 SYNOPSIS 
 ...
 PACKAGE NAME:iTopMutePicks2par3.pm
 AUTHOR: Juan Lorenzo
 DATE: May 5 2009
...
sub clear {
    $iTopMutePicks2par3->{_TX_inbound}    = '';
    $iTopMutePicks2par3->{_TX_outbound}   = '';
    $iTopMutePicks2par3->{_taup_inbound}  = '';
    $iTopMutePicks2par3->{_taup_outbound} = '';
    $iTopMutePicks2par3->{_gather_num}    = '';
    $iTopMutePicks2par3->{_exists}        = '';
    $iTopMutePicks2par3->{_textfile_in}   = '';
    $iTopMutePicks2par3->{_textfile_out}  = '';
    $iTopMutePicks2par3->{_type}          = '';
}
...
sub file_in {
    ...
    $iTopMutePicks2par3->{_file_in} = $file_in if defined($file_in);

    #print("file name is $iTopMutePicks2par3->{_file_in} \n\n");
}
...
sub type {
    my ( $variable, $type ) = @_;
    $iTopMutePicks2par3->{_type} = $iTopMutePicks2par3
      if defined($iTopMutePicks2par3);
}
...
=cut
    ...
    $mkparfile_in = $itemp_top_mute_picks_ . $iTopMutePicks2par3->{_file_in};
    $mkparfile_out =
      $itemp_top_mute_picks_sorted_par_ . $iTopMutePicks2par3->{_file_in};

    $iTopMutePicks2par3->{TX_inbound}  = $PL_SEISMIC . '/' . $mkparfile_in;
    $iTopMutePicks2par3->{TX_outbound} = $PL_SEISMIC . '/' . $mkparfile_out;
...
=cut
    ...
    @items = (
        $mkparfile[1], $in, $iTopMutePicks2par3->{TX_inbound},
        $out, $iTopMutePicks2par3->{TX_outbound}
    );
