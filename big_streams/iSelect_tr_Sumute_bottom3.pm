package iSelect_tr_Sumute_bottom3;

=head1 DOCUMENTATION

=head2 SYNOPSIS 

 PACKAGE NAME: iSelect_tr_Sumute_bottom3.pm
 AUTHOR: Juan Lorenzo
...
sub clear {
    $iSelect_tr_Sumute_bottom3->{_TX_outbound}     = '';
    $iSelect_tr_Sumute_bottom3->{_gather_num}      = '';
    $iSelect_tr_Sumute_bottom3->{_file_in}         = '';
    $iSelect_tr_Sumute_bottom3->{_freq}            = '';
    $iSelect_tr_Sumute_bottom3->{_inbound}         = '';
    $iSelect_tr_Sumute_bottom3->{_message_type}    = '';
    $iSelect_tr_Sumute_bottom3->{_gather_type}     = '';
    $iSelect_tr_Sumute_bottom3->{_offset_type}     = '';
    $iSelect_tr_Sumute_bottom3->{_gather_header}   = '';
    $iSelect_tr_Sumute_bottom3->{_number_of_tries} = '';
    $iSelect_tr_Sumute_bottom3->{_file_in}         = '';
    $iSelect_tr_Sumute_bottom3->{_textfile_in}     = '';
}
...
sub gather_num {
    ...
    $iSelect_tr_Sumute_bottom3->{_gather_num} = $gather_num
...
sub gather_header {
    my ( $variable, $type ) = @_;
    $iSelect_tr_Sumute_bottom3->{_gather_header} = $type if defined($type);
}
...
sub gather_type {
    my ( $variable, $type ) = @_;
    $iSelect_tr_Sumute_bottom3->{_gather_type} = $type if defined($type);
}
...
sub offset_type {
    my ( $variable, $type ) = @_;
    $iSelect_tr_Sumute_bottom3->{_offset_type} = $type if defined($type);
}
...
sub file_in {
    ...
    $iSelect_tr_Sumute_bottom3->{_file_in} = $file_in if defined($file_in);

    #$file_out[$N] 		= $iSelect_tr_Sumute_bottom3->{_file_in}.$itop_mute_par_;
    $iSelect_tr_Sumute_bottom3->{_inbound} =
        $DATA_SEISMIC_SU . '/'
      . $iSelect_tr_Sumute_bottom3->{_file_in}
      . $suffix_su;

    #print("file name is $iSelect_tr_Sumute_bottom3->{_file_in} \n\n");
}
...
sub freq {
    my ( $variable, $freq ) = @_;
    $iSelect_tr_Sumute_bottom3->{_freq} = $freq if defined($freq);

    #print("freq is $iSelect_tr_Sumute_bottom3->{_freq}\n\n");
}
...
sub min_amplitude {
    ...
    $iSelect_tr_Sumute_bottom3->{_min_amplitude} = $min_amplitude
      if defined($min_amplitude);

    #print("min_amplitude is $iSelect_tr_Sumute_bottom3->{_min_amplitude}\n\n");
}
...
sub max_amplitude {
    ...
    $iSelect_tr_Sumute_bottom3->{_max_amplitude} = $max_amplitude
      if defined($max_amplitude);

    #print("max_amplitude is $iSelect_tr_Sumute_bottom3->{_max_amplitude}\n\n");
}
...
sub number_of_tries {
    ...
    $iSelect_tr_Sumute_bottom3->{_number_of_tries} = $number_of_tries
      if defined($number_of_tries);

   #print("num of tries is $iSelect_tr_Sumute_bottom3->{_number_of_tries}\n\n");
...
=cut
    ...
    $suwind->clear();
    $suwind->setheaderword( $iSelect_tr_Sumute_bottom3->{_gather_header} );
    $suwind->min( $iSelect_tr_Sumute_bottom3->{_gather_num} );
    $suwind->max( $iSelect_tr_Sumute_bottom3->{_gather_num} );

    #print("gather num is $iSelect_tr_Sumute_bottom3->{_gather_num}\n\n");
...
=cut
    ...
    $sufilter->clear();
    $sufilter->freq( $iSelect_tr_Sumute_bottom3->{_freq} );
...
=cut
    ...
    $windowtitle[1] =
      quotemeta('GATHER = ') . $iSelect_tr_Sumute_bottom3->{_gather_num};
    ...
    $suximage->xlabel( $iSelect_tr_Sumute_bottom3->{_offset_type} );
    $suximage->legend($on);
    $suximage->cmap('rgb0');
    $suximage->loclip( $iSelect_tr_Sumute_bottom3->{_min_amplitude} );
    $suximage->hiclip( $iSelect_tr_Sumute_bottom3->{_max_amplitude} );
    $suximage->verbose($off);

    if ( $iSelect_tr_Sumute_bottom3->{_number_of_tries} > 0 ) {
        $iSelect_tr_Sumute_bottom3->{_TX_outbound} =
          $itemp_bot_mute_picks_ . $iSelect_tr_Sumute_bottom3->{_file_in};
        $suximage->picks( \$iSelect_tr_Sumute_bottom3->{_TX_outbound} );

#print("Writing picks to $itemp_bot_mute_picks_ $iSelect_tr_Sumute_bottom3->{_file_in}  \n\n");
#print("number of tries is $iSelect_tr_Sumute_bottom3 -> {_number_of_tries}\n\n");
...
=cut
    ...
    @items = (
        $suwind[1], $in, $iSelect_tr_Sumute_bottom3->{_inbound},
