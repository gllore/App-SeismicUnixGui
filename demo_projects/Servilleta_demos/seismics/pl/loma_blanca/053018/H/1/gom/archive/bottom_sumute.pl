...
PROGRAM NAME:  bottom_sumute.pl
 Purpose: Mute of Data 
  ...
  DATE:    April 2 2009
           ...
           June 12 2017, V0.4 adapted from Sumute.pl (top mute only) 
...
=cut
 ...
 use suximage;
 use suxwigb;
 use SeismicUnix qw ($in $ibot_mute_par_ $suffix_bot_mute $out $on $go $to $suffix_ascii $off $suffix_su); 
...
=cut
  ...
  my ($ref_cat_array,$bottom_mute_list);
...
=head2 Use only
 ...
 bottom-mute list of parameter files lies in local PL directory
...
=cut
  ...
  $file_out[1] 		        = $file_in[1].$suffix_bot_mute;
...
=cut
 ...
 if ($min_gatherNumber[1] != $max_gatherNumber[1]) {
    ...
    $sumute    	 -> ntaper(50);
    $sumute    	 -> type('bottom');
    $sumute    	 -> list($bottom_mute_list,\$file_in[1],\$gatherType[1]);
...
=head2 DEFINE FLOW(S)
   if list input/ multiple shots: 
   run all the sumute flows, then
