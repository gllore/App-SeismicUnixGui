...
=cut
	...
	$susynlv				->nxo(quotemeta(100));
	$susynlv				->nxs(quotemeta(1));
	$susynlv				->ob(quotemeta(1));
	$susynlv				->reflector(quotemeta('1.0:0.0,1.0;5.0,4.1955'));
...
=cut
	...
	$suximage				->title(quotemeta('40 degree Model'));
	...
	$suximage				->windowtitle(quotemeta('40 degree Model'));
