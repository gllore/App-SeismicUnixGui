...
sub get_binding_index_aref {
	...
	if ( $pdfhistogram_spec->{_binding_index_aref} ) {
		...
		print(
"pdfhistogram_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $pdfhistogram_spec->{_binding_index_aref} ) {
		...
		my $binding_length =
		  scalar @{ $pdfhistogram_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print(
			"pdfhistogram_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	if ( $pdfhistogram_spec->{_file_dialog_type_aref} ) {
		...
		print(
"pdfhistogram_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $pdfhistogram_spec->{_flow_type_aref} ) {
		...
		print(
			"pdfhistogram_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"pdfhistogram, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
