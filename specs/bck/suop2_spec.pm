...
=head2  sub prefix_aref
	...
	assign specific prefixes to parameter
	values
...
sub prefix_aref {
    ...
    $prefix[0] = '$DATA_SEISMIC_SU' . ".'/'.";

    # label 2 in GUI is input file2 and needs a home directory
    $prefix[1] = '$DATA_SEISMIC_SU' . ".'/'.";

    for ( my $i = 2 ; $i < $max_index ; $i++ ) {
...
sub suffix_aref {
    ...
    for ( my $i = 2 ; $i < $max_index ; $i++ ) {
        ...
        if ( $suffix[$i] ) { print("suop2_spec,suffix: $suffix[$i]\n"); }
    }
