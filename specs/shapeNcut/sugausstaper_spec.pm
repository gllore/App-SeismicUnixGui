...
my $sugausstaper = new sugausstaper;
...
sub binding_index_aref {
	...
	$sugausstaper_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$sugausstaper_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$sugausstaper_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $sugausstaper_spec->{_binding_index_aref} ) {

		my $index_aref = $sugausstaper_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"sugausstaper_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $sugausstaper_spec->{_binding_index_aref} ) {
		...
		my $binding_length =
		  scalar @{ $sugausstaper_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print(
			"sugausstaper_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $sugausstaper_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sugausstaper_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"sugausstaper_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sugausstaper_spec->{_flow_type_aref} ) {

		my $index_aref = $sugausstaper_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print(
			"sugausstaper_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sugausstaper, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $sugausstaper_spec->{_prefix_aref} ) {

		my $prefix_aref = $sugausstaper_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("sugausstaper_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $sugausstaper_spec->{_suffix_aref} ) {

		my $suffix_aref = $sugausstaper_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$sugausstaper_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	$sugausstaper_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	$sugausstaper_spec->{_suffix_aref} = \@suffix;
	return ();
