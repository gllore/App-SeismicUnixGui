...
sub binding_index_aref {
	...
	$index[2] = 2;     # inbound/outbound item is bound
	$index[3] = 3;     # inbound/outbound item is bound
	$index[4] = 4;     # inbound/outbound item is bound
	$index[5] = 5;     # inbound/outbound item is bound
	$index[6] = 8;     # inbound/outbound item is bound
	$index[7] = 9;     # inbound/outbound item is bound
	$index[8] = 10;    # inbound/outbound item is bound
	$stiff2vel_spec->{_binding_index_aref} = \@index;
	return ();
...
sub flow_type_aref {
	...
	$stiff2vel_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $stiff2vel_spec->{_binding_index_aref} ) {

		my $index_aref = $stiff2vel_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"stiff2vel_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $stiff2vel_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $stiff2vel_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("stiff2vel_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $stiff2vel_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $stiff2vel_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"stiff2vel_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $stiff2vel_spec->{_flow_type_aref} ) {

		my $index_aref = $stiff2vel_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("stiff2vel_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"stiff2vel, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $stiff2vel_spec->{_prefix_aref} ) {

		my $prefix_aref = $stiff2vel_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("stiff2vel_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $stiff2vel_spec->{_suffix_aref} ) {

		my $suffix_aref = $stiff2vel_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$stiff2vel_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 11 in GUI is input/output xx_file and needs a home directory
	$prefix[ $index[8] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$stiff2vel_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 11 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[8] ] = '' . '' . '$suffix_bin';

	$stiff2vel_spec->{_suffix_aref} = \@suffix;
	return ();
