...
sub binding_index_aref {
	...
	$index[3]                             = 4;  # inbound/outbound item is bound
	$index[4]                             = 5;  # inbound/outbound item is bound
	$index[5]                             = 6;  # inbound/outbound item is bound
	$index[6]                             = 11; # inbound/outbound item is bound
	$index[7]                             = 18; # inbound/outbound item is bound
	$rayt2dan_spec->{_binding_index_aref} = \@index;
	return ();
...
sub flow_type_aref {
	...
	$rayt2dan_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $rayt2dan_spec->{_binding_index_aref} ) {

		my $index_aref = $rayt2dan_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"rayt2dan_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $rayt2dan_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $rayt2dan_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("rayt2dan_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $rayt2dan_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $rayt2dan_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"rayt2dan_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $rayt2dan_spec->{_flow_type_aref} ) {

		my $index_aref = $rayt2dan_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("rayt2dan_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"rayt2dan, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $rayt2dan_spec->{_prefix_aref} ) {

		my $prefix_aref = $rayt2dan_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("rayt2dan_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $rayt2dan_spec->{_suffix_aref} ) {

		my $suffix_aref = $rayt2dan_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$rayt2dan_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 5 in GUI is input/output xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 6 in GUI is input/output xx_file and needs a home directory
	...
	# label 12 in GUI is input/output xx_file and needs a home directory
	$prefix[ $index[6] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 19 in GUI is input/output xx_file and needs a home directory
	$prefix[ $index[7] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$rayt2dan_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 5 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[3] ] = '' . '' . '$suffix_bin';

	# label 6 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[4] ] = '' . '' . '$suffix_bin';

	# label 7 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[5] ] = '' . '' . '$suffix_bin';

	# label 12 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[6] ] = '' . '' . '$suffix_bin';

	# label 19 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[7] ] = '' . '' . '$suffix_bin';

	$rayt2dan_spec->{_suffix_aref} = \@suffix;
	return ();
