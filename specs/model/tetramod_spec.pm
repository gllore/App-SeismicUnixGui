...
sub binding_index_aref {
	...
	$index[1]                             = 6;  # inbound/outbound item is bound
	$index[2]                             = 8;  # inbound/outbound item is bound
	$index[3]                             = 14; # inbound/outbound item is bound
	$index[4]                             = 17; # inbound/outbound item is bound
	$index[5]                             = 19; # inbound/outbound item is bound
	$index[6]                             = 20; # inbound/outbound item is bound
	$index[7]                             = 23; # inbound/outbound item is bound
	$index[8]                             = 24; # inbound/outbound item is bound
	$index[9]                             = 29; # inbound/outbound item is bound
	$index[10]                            = 32; # inbound/outbound item is bound
	$tetramod_spec->{_binding_index_aref} = \@index;
	return ();
...
=cut
	...
	sub flow_type_aref {
		...
		$tetramod_spec->{_flow_type_aref} = \@type;
		return ();
...
=cut
	...
	sub get_binding_index_aref {
		...
		if ( $tetramod_spec->{_binding_index_aref} ) {

			my $index_aref = $tetramod_spec->{_binding_index_aref};
			return ($index_aref);

		}
		else {
			print(
"tetramod_spec, get_binding_index_aref, missing binding_index_aref\n"
			);
			return ();
		}
...
=cut
	...
	sub get_binding_length {
		...
		if ( $tetramod_spec->{_binding_index_aref} ) {
			...
			my $binding_length =
			  scalar @{ $tetramod_spec->{_binding_index_aref} };
			return ($binding_length);

		}
		else {
			print(
				"tetramod_spec, get_binding_length, missing binding_length\n");
			return ();
		}
...
=cut
	...
	sub get_file_dialog_type_aref {
		...
		my $self = @_;
		if ( $tetramod_spec->{_file_dialog_type_aref} ) {

			my $index_aref = $tetramod_spec->{_file_dialog_type_aref};
			return ($index_aref);

		}
		else {
			print(
"tetramod_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
			);
			return ();
		}
...
=cut
	...
	sub get_flow_type_aref {
		...
		if ( $tetramod_spec->{_flow_type_aref} ) {

			my $index_aref = $tetramod_spec->{_flow_type_aref};
			return ($index_aref);

		}
		else {
			print(
				"tetramod_spec, get_flow_type_aref, missing flow_type_aref\n");
			return ();
		}
	...
	sub get_incompatibles {
		...
		if ( $len_1_needed >= 1 ) {
			...
			for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
				...
				print(
"tetramod, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
				);
...
=cut
	...
	sub get_prefix_aref {
		...
		if ( $tetramod_spec->{_prefix_aref} ) {

			my $prefix_aref = $tetramod_spec->{_prefix_aref};
			return ($prefix_aref);

		}
		else {
			print("tetramod_spec, get_prefix_aref, missing prefix_aref\n");
			return ();
		}
...
=cut
	...
	sub get_suffix_aref {
		...
		if ( $tetramod_spec->{_suffix_aref} ) {

			my $suffix_aref = $tetramod_spec->{_suffix_aref};
			return ($suffix_aref);

		}
		else {
			print("$tetramod_spec, get_suffix_aref, missing suffix_aref\n");
			return ();
		}
...
=cut
	...
	sub prefix_aref {
		...
		# label 15 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 18 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 20 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 21 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[6] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 24 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[7] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 25 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[8] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 30 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[9] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		# label 33 in GUI is input/output xx_file and needs a home directory
		$prefix[ $index[10] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

		$tetramod_spec->{_prefix_aref} = \@prefix;
		return ();
...
=cut
	...
	sub suffix_aref {
		...
		# label 15 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[3] ] = '' . '' . '$suffix_bin';

		# label 18 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[4] ] = '' . '' . '$suffix_bin';

		# label 20 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[5] ] = '' . '' . '$suffix_bin';

		# label 21 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[6] ] = '' . '' . '$suffix_bin';

		# label 24 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[7] ] = '' . '' . '$suffix_bin';

		# label 25 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[8] ] = '' . '' . '$suffix_bin';

		# label 30 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[9] ] = '' . '' . '$suffix_bin';

		# label 33 in GUI is input/ouput xx_file and needs a home directory
		$suffix[ $index[10] ] = '' . '' . '$suffix_bin';

		$tetramod_spec->{_suffix_aref} = \@suffix;
		return ();
