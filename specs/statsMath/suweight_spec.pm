...
sub get_binding_index_aref {
	...
	if ( $suweight_spec->{_binding_index_aref} ) {
		...
		print(
"suweight_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	if ( $suweight_spec->{_file_dialog_type_aref} ) {
		...
		print(
"suweight_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"suweight, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
