...
use LSeismicUnix::misc::SeismicUnix qw ($su $suffix_su $suffix_txt);
use LSeismicUnix::misc::L_SU_global_constants;
use LSeismicUnix::sunix::statsMath::suxcor;
...
sub get_binding_index_aref {
	...
	if ( $suxcor_spec->{_binding_index_aref} ) {

		my $index_aref = $suxcor_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
			"suxcor_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $suxcor_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $suxcor_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("suxcor_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my ($self) = @_;
	if ( $suxcor_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $suxcor_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"suxcor_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $suxcor_spec->{_flow_type_aref} ) {

		my $index_aref = $suxcor_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("suxcor_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"suxcor, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $suxcor_spec->{_prefix_aref} ) {

		my $prefix_aref = $suxcor_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("suxcor_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $suxcor_spec->{_suffix_aref} ) {

		my $suffix_aref = $suxcor_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$suxcor_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
