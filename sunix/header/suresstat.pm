...
=head3 SEISMIC UNIX NOTES
 ...
 Perl wrapper MODIFIED BY: Martial Morrison
...
=cut
 ...
 sub fn {
	...
	if ( $fn ne $empty_string ) {
		...
		$suresstat->{_fn}		= $fn;
		$suresstat->{_note}		= $suresstat->{_note}.' fn='.$suresstat->{_fn};
		$suresstat->{_Step}		= $suresstat->{_Step}.' fn='.$suresstat->{_fn};

	} else { 
		print("suresstat, fn, missing fn,\n");
	 }
 }
...
=cut
 ...
 sub imax {
	...
	my ( $self,$imax )		= @_;
	if ( $imax ne $empty_string ) {

		$suresstat->{_imax}		= $imax;
		$suresstat->{_note}		= $suresstat->{_note}.' imax='.$suresstat->{_imax};
		$suresstat->{_Step}		= $suresstat->{_Step}.' imax='.$suresstat->{_imax};

	} else { 
		print("suresstat, imax, missing imax,\n");
	 }
 }
