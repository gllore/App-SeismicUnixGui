...
sub _basic_dirs {
	...
	my $ACTIVE_PROJECT		= _get_ACTIVE_PROJECT();
			# print(" Project, _basic_dirs, ACTIVE_PROJECT: $ACTIVE_PROJECT\n");
    ...
    if(-e $prog_name_old.'.config') { 
    	...
    	$Project->{_ref_DIR} 			= $ref_DIR;
    	    	    # print(" 2. Project,_basic_dirs,ref_DIR:@{$Project->{_ref_DIR}}\n");
    	    	    
    	    	    # parameter widget labels/names
    	$Project->{_ref_DIR_FUNCTION} 	= $ref_DIR_FUNCTION;
    	    	    # print(" 3. Project,_basic_dirs,ref_DIR:@{$Project->{_ref_DIR_FUNCTION}}\n");
    	...
    	my $PATH_N_file				=  $ACTIVE_PROJECT.'/Project.config';
    	 	# print("Project,PATH_N_file 	: $PATH_N_file\n");
   ...
   # 1. check local directory first LEGACY Project_Variables file
    if (-e $prog_name_old.'.config')  {
      ...
      # in local user configuration directory
    	...
    	$prog_name 						= $prog_name_new;
      	    		# print("Project,basic_dirs,using $ACTIVE_PROJECT/$prog_name.config\n");  	
    	my ($ref_DIR_FUNCTION,$ref_DIR) = $read->configs(($ACTIVE_PROJECT.'/'.$prog_name_new.'.config'));
    	$Project->{_ref_DIR} 			= $ref_DIR;
...
sub _change_basic_dirs {
    ...
    my ($mmodpg_logic);
    my ($sqlite_logic,$gmt_logic,$grass_logic);
    # TODO my ()$matlab,$fast,gmt); 
    # print(" 5. Project,_change_basic_dirs,ref_DIR:@{$Project->{_ref_DIR}}\n");
    ...
    $geopsy_logic				= $control->set_str2logic($CFG[19]);  	
  	...
  	$mmodpg_logic				= $control->set_str2logic($CFG[27]);
    $sqlite_logic				= $control->set_str2logic($CFG[29]);
 	
				# print("1. Project,_change_basic_dirs PROJECT_HOME=$Project->{_PROJECT_HOME}\n");
				# print("1. Project,_change_basic_dirs spare_dir=----$spare_dir----\n");
...
=cut
    ...
    my $spare_dir_bck = $spare_dir;
     	   print("Project, change_basic_dirs, spare_dir_bck: $spare_dir\n\n"); 
...
=head3 for old-stype Project_Variable files 
 ...
 assumes an up-to-date locally available configuration file
 ...
 if (-e  $old_configuration_file) {
   ...
   use Project_config_Variables;
    ...
    ($PROJECT_HOME)	= Project_Variables::PROJECT_HOME();
     $subUser       = ''; #only in  newer (>= 2018) configuration files;

 }
 
 	 # print("Project, Using up-to-date locally available configuration file\n\n");
 	 # print("Project, HOME: $HOME\n\n"); 
 	 # print("Project, line: $line\n\n");    
	...
	$Project->{_mmodpg_is_selected} = $mmodpg_logic;		
	$Project->{_sqlite_is_selected} = $sqlite_logic;
	
	print("Project, mmodpg_logic= $mmodpg_logic\n");
...
sub _system_dirs {
	...
	my $subUser 		= $Project->{_subUser};
				# print(" Project, _system_dirs, Before PROJECT_HOME $PROJECT_HOME\n");
	$PROJECT_HOME 		=~ s/\'//g;
				 #  print(" Project, _system_dirs, After PROJECT_HOME $PROJECT_HOME\n");
				 # print(" Project, _system_dirs, Before subUser: $subUser \n");

	if ($subUser) {
			$subUser 			=~ s/\'//g;
			# print(" Project, _system_dirs, After subUser: $subUser \n");
	} else {
		print("New L_SU project detected: Project, _system_dirs\n");
	}
...
# WELL DATA DIRECTORY
	...
	#print("3. Project,DATA_GEOMAPS_TEXT=$DATA_GEOMAPS_TEXT\n");
 	...
 	$Project->{_PL_SEISMIC}					= $PL_SEISMIC;	
 				# print("Project,_system_dirs,PL_SEISMIC = $PL_SEISMIC\n");					
...
sub PL_SEISMIC {
	...
	# This subroutine returns the value of PL_SEISMIC 
		# print ("\nProject, PL_SEISMIC,PL_SEISMIC: $PL_SEISMIC\n");
        return ($PL_SEISMIC);
}
