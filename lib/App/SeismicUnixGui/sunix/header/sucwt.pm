package App::SeismicUnixGui::sunix::header::sucwt;

=head1 DOCUMENTATION

=head2 SYNOPSIS

 PACKAGE NAME:  SUCWT - generates Continous Wavelet Transform amplitude, regularity	
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
sub nwavelet {
    ...
    if ( $nwavelet ne $empty_string ) {
        ...
        $sucwt->{_note} =
          $sucwt->{_note} . ' nwavelet=' . $sucwt->{_nwavelet};
        $sucwt->{_Step} =
          $sucwt->{_Step} . ' nwavelet=' . $sucwt->{_nwavelet};
