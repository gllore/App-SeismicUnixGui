...
#		_dropsite_token_blue                     => '',
	...
	$markers_href->{_help_menubutton_click_seq_href} 		= $help_menubutton_click_seq_href;	
...
sub _update_FileDialog_type {
	...
	if ( defined $FileDialog_type_href ) {
		...
		$FileDialog_type_href->{_most_recent} = $new_most_recent_FileDialog_type;
...
sub _update_button {
	...
	if ( $new_most_recent_button ne $empty_string ) {
		...
		if ( $new_most_recent_button eq 'FileDialog_button' ) {
			...
			( $gui_history->get_defaults )->{_help_menubutton_click_seq_href} = $help_menubutton_click_seq_href;
...
sub _update_add2flow_color {
	...
	if (   $new_most_recent_color ne 'nada'
		...
		if ( defined $add2flow_button_color_href ) {
			...
			# update the history of add2flow_button usage according to the color used
			$add2flow_button_color_href->{_earliest}    = $add2flow_button_color_href->{_prior};
			$add2flow_button_color_href->{_prior}       = $add2flow_button_color_href->{_most_recent};
			$add2flow_button_color_href->{_most_recent} = $new_most_recent_color;
...
sub _update_flow_listbox_color_w {
	...
	if ( defined $flow_listbox_color_w_href ) {
		...
		$flow_listbox_color_w_href->{_earliest}    = $flow_listbox_color_w_href->{_prior};
		$flow_listbox_color_w_href->{_prior}       = $flow_listbox_color_w_href->{_most_recent};
		$flow_listbox_color_w_href->{_most_recent} = $new_most_recent_flow_listbox_color_w;
...
sub _update_help_menubutton_type {
	my (
		$gui_history, $new_most_recent_help_menubutton_type,
	...
	if ( defined $help_menubutton_type_href ) {
		...
		# update the history of add2flow_button usage according to the color used
		$help_menubutton_type_href->{_earliest}    = $help_menubutton_type_href->{_prior};
		$help_menubutton_type_href->{_prior}       = $help_menubutton_type_href->{_most_recent};
		$help_menubutton_type_href->{_most_recent} = $new_most_recent_help_menubutton_type;

		( $gui_history->get_defaults )->{_help_menubutton_type_href} = $help_menubutton_type_href;
	}
...
sub _update_parameter_color_on_entry {
	...
	if ( defined $parameter_color_on_entry_href ) {
		...
		$parameter_color_on_entry_href->{_earliest}    = $parameter_color_on_entry_href->{_prior};
		$parameter_color_on_entry_href->{_prior}       = $parameter_color_on_entry_href->{_most_recent};
		$parameter_color_on_entry_href->{_most_recent} = $new_most_recent_parameter_color_on_entry;
...
sub _update_parameter_color_on_exit {
	...
	if ( defined $parameter_color_on_exit_href ) {
		...
		$parameter_color_on_exit_href->{_earliest}    = $parameter_color_on_exit_href->{_prior};
		$parameter_color_on_exit_href->{_prior}       = $parameter_color_on_exit_href->{_most_recent};
		$parameter_color_on_exit_href->{_most_recent} = $new_most_recent_parameter_color_on_exit;
...
sub _update_parameter_index_on_entry {
	...
	if ( defined $parameter_index_on_entry_href ) {
		...
		$parameter_index_on_entry_href->{_earliest}    = $parameter_index_on_entry_href->{_prior};
		$parameter_index_on_entry_href->{_prior}       = $parameter_index_on_entry_href->{_most_recent};
		$parameter_index_on_entry_href->{_most_recent} = $new_most_recent_parameter_index_on_entry;
...
sub _update_sunix_prog_group {
	...
	if ( defined $sunix_prog_group_href ) {
		...
		$sunix_prog_group_href->{_prior}       = $sunix_prog_group_href->{_most_recent};
		$sunix_prog_group_href->{_most_recent} = $new_most_recent_sunix_prog_group;
	...
	foreach my $key ( sort keys %$gui_history ) {
		...
		if ( $key eq 'defaults' ) {
			...
			if ( ( $gui_history->get_defaults() )->{_FileDialog_type_href} ) {
				...
				$key_hash_ref = ( $gui_history->{defaults} )->{_FileDialog_type_href};
			...
			# print values of values that are hashes
			if ( ( $gui_history->get_defaults() )->{_FileDialog_button_click_seq_href} ) {

				# print additional subset: _FileDialog_button_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_FileDialog_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_add2flow_button_click_seq_href} ) {

				# print additional subsets: _add2flow_button_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_add2flow_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_add2flow_button_color_href} ) {
				...
				$key_hash_ref = ( $gui_history->{defaults} )->{_add2flow_button_color_href};
			...
			if ( ( $gui_history->get_defaults() )->{_delete_from_flow_button_click_seq_href} ) {

				# print additional subsets: delete_from_flow_button
				$key_hash_ref = ( $gui_history->get_defaults() )->{_delete_from_flow_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_flow_item_down_arrow_button_click_seq_href} ) {

				# print additional subsets: flow_item_down_arrow_button
				$key_hash_ref = ( $gui_history->get_defaults() )->{_flow_item_down_arrow_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_flow_item_up_arrow_button_click_seq_href} ) {

				# print additional subsets: flow_item_up_arrow_button
				$key_hash_ref = ( $gui_history->get_defaults() )->{_flow_item_up_arrow_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_flow_select_index_href} ) {
				...
				$key_hash_ref = ( $gui_history->get_defaults() )->{_flow_select_index_href};
			...
			if ( ( $gui_history->get_defaults() )->{_flow_select_click_seq_href} ) {
				...
				$key_hash_ref = ( $gui_history->get_defaults() )->{_flow_select_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_color_on_exit_href} ) {

				# print additional subset: _parameter_color_on_exit_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_color_on_exit_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_color_on_entry_href} ) {

				# print additional subset: _parameter_color_on_entry_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_color_on_entry_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_index_on_entry_href} ) {

				# print additional subset: _parameter_index_on_entry_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_index_on_entry_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_index_on_entry_click_seq_href} ) {

				# print additional subset: _parameter_index_on_entry_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_index_on_entry_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_index_on_exit_href} ) {

				# print additional subset: _parameter_index_on_exit_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_index_on_exit_href};
			...
			if ( ( $gui_history->get_defaults() )->{_parameter_index_on_exit_click_seq_href} ) {

				# print additional subset: _parameter_index_on_exit_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_parameter_index_on_exit_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_run_button_click_seq_href} ) {

				# print additional subset: _run_button_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_run_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_save_button_click_seq_href} ) {
				...
				$key_hash_ref = ( $gui_history->{defaults} )->{_save_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_save_as_button_click_seq_href} ) {

				# print additional subset: _save_as_button_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_save_as_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_sunix_prog_group_href} ) {
				...
				$key_hash_ref = ( $gui_history->{defaults} )->{_sunix_prog_group_href};
			...
			if ( ( $gui_history->get_defaults() )->{_sunix_prog_group_click_seq_href} ) {

				# print additional subset: _sunix_prog_group_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_sunix_prog_group_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_superflow_select_button_click_seq_href} ) {

				# print additional subset: _superflow_select_button_click_seq_href
				$key_hash_ref = ( $gui_history->{defaults} )->{_superflow_select_button_click_seq_href};
			...
			if ( ( $gui_history->get_defaults() )->{_superflow_tool_href} ) {
				...
				$key_hash_ref = ( $gui_history->{defaults} )->{_superflow_tool_href};
			...
			if ( ( $gui_history->get_defaults() )->{_wipe_plots_button_click_seq_href} ) {
				...
				$key_hash_ref = ( $gui_history->get_defaults() )->{_wipe_plots_button_click_seq_href};
