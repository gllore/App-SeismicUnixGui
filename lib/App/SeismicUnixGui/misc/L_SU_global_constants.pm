...
=cut
	...
	my @sunix_well_programs = (
		"las2su",   "subackus", "subackush", "sugassman",
		"sulprime", "suwellrf",
	);
...
=cut
		...
		#						);
			...
			print(
				"L_SU_global_constants,__get_path4spec_file,file_name_missing\n"
			);
			return ();
		}
	}
...
=cut
	...
	sub get_colon_pathNmodule {
		...
		if ( length $L_SU_global_constants->{_program_name} ) {
			...
			my $new_pathNmodule_spec =
			  'App::SeismicUnixGui' . $pathNmodule_spec;
...
=cut
   ...
   #	  	  			);
					...
					my $directory_list_aref =
					  $manage_dirs_by->get_file_list_aref();
					my @directory_list  = @$directory_list_aref;
...
#				print("L_SU_global_constants,get_pathNfile2search,dir contents:@{$directory_contents[$parent][$child]}\n");
			...
			print(
				"GRANDPARENT_DIR=$L_SU_global_constants->{_GRANDPARENT_DIR}\n");
...
=cut 
		...
		@dimensions = (
			$parent_directory_specs_number_of,
			$child_directory_specs_number_of
		);
...
=cut 
		...
		@dimensions = (
			$parent_directory_tools_number_of,
			$child_directory_tools_number_of
		);
...
=cut
		 ...
		 #						);
			...
			print(
				"L_SU_global_constants,_get_path4spec_file,file_name_missing\n"
			);
			return ();
		}
	}
...
=cut
		...
		#						);
			...
			print(
				"L_SU_global_constants,_get_path4tools_file,file_name_missing\n"
			);
			return ();
		}
	}
...
=cut
	...
	sub set_program_name {
		...
		if ( length $program_name ) {
			...
			print(
				"L_SU_global_constants,set_program_name,missing program_name\n"
			);
		}
