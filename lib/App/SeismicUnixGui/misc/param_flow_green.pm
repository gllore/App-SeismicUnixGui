...
#		print(
#			"param_flow_green, _get_names_aref, @names_aref, index=$item_index\n"
#		);
#		
#			print 'b\n';
		return ( \@names_aref );
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
			...
			&& ( @{$values_aref}[$i] ne $empty_string ) )
		{
			...
			if ( @$values_aref[$i] ne "'nu'" ) {    #
				...
				print(
"param_flow_green, _set_good_labels_4item: unexpected ending\n"
				);
			}
...
#		print("param_flow_green, _set_good_values_4item: index=$i, values_aref= @$values_aref[$i]\n");
		...
		#				print("2. param_flow_green,_set_good_values4item,value:$value \n");
				...
				print(
"param_flow_green, _set_good_values_4item: unexpected ending\n"
				);
			}
...
# }
		...
		print(
"param_flow_green, clear_flow_items_version_aref, missing program_version_aref\n"
		);
	}
...
#		print("param_flow_green, get_values :flow index= $index\n");
		@values = @{ @{ $param_flow_green->{_values_aref2} }[$index] };
#		$length      = scalar @values;
#		print("param_flow_green, get_values :length=$length\n");

#		for my $in (@values) {
#			print("param_flow_green,get_values :value is--$in--\n");
#		}
#		print("\n");
...
#    print("param_flow_green,view_data: checkbuttons: @{@{$param_flow_green->{_checkbuttons_aref2}}[$i]}\n\n");
		...
		print(
"param_flow_green,get_values_aref :selection_index <=0  or vmissing alues_aref2\n"
		);
		return ();
	}
}
...
# all parameters values are changed when ANY flow item is selected
		...
		print(
"param_flow_green, set_flow_index,index does not exist, index:$index\n"
		);
...
#	 self,index4flow: $self,$index4flow\n");
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
		...
		if ( @$values_aref[$i] ne "'nu'" ) {
			...
			print(
				"param_flow_green,set_good_values4item, a bad value detected\n");
	...
	if ( $param_flow_green->{_selection_index} >= 0 ) {
		...
		if ( $index >= 0 ) {
			...
			$cloned_values_aref2[$index] =
			  clone( \@{ @{ $param_flow_green->{_values_aref2} }[$index] } );
			
			$length                  = scalar @{$cloned_values_aref2[$index]};
            # print(" OK 1. param_flow_green, set_values_aref:length=$length\n");          
			
			@{ @{ $param_flow_green->{_values_aref2} }[$index]} = @{$cloned_values_aref2[$index]};
			
#        print("1. param_flow_green, set_values_aref:@{@{ $param_flow_green->{_values_aref2} }[$index]}\n");
#         print("1. param_flow_green, set_values_aref:@{$cloned_values_aref2[$index]}\n");
...
sub view_data {
	...
	$num_progs[5] = scalar( @{ $param_flow_green->{_checkbuttons_aref2} } );

	print(
"\nparam_flow_green,view_data:number of programs in flow, in 6 different ways @num_progs \n"
	);

# print("\n param_flow_green,view_data, _prog_names @{$param_flow_green->{_prog_names_aref}}\n");
# print("param_flow_green,view_data:max index (# items in a flow) = $indices  \n\n");

# print("param_flow_green,view_data, param_flow_green_ has hash=$param_flow_green\n ");

	for ( my $i = 0 ; $i <= $indices ; $i++ ) {

		for my $name ( @{ @{ $param_flow_green->{_names_aref2} }[$i] } ) {
			print("param_flow_green,view_data: name:      $name\n");
		}

#     print("param_flow_green,view_data: checkbuttons: @{@{$param_flow_green->{_checkbuttons_aref2}}[$i]}\n\n");
		# for my $value ( @{ @{ $param_flow_green->{_values_aref2} }[$i] } ) {
		for my $value ( @{$cloned_values_aref2[$i]} ) {
			print("param_flow_green,view_data: value:     $value\n");
		}
