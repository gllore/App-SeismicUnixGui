package App::SeismicUnixGui::misc::param_flow_pink;

=head1 DOCUMENTATION

=head2 SYNOPSIS 

 Perl package: param_flow_pink.pm 
 AUTHOR: Juan Lorenzo
 DATE: Aug 3 2017 
...
sub _get_good_length4item {
	...
	my $idx    = $index4flow;
	my $length = scalar @{ $param_flow_pink->{_good_values_aref2}[$idx] };
#	print 'a\n';
	return ($length);
}
...
#		print(
#			"param_flow_pink, _get_names_aref, @names_aref, index=$item_index\n"
#		);
#		
#			print 'b\n';
		return ( \@names_aref );
...
#	print("param_flow_pink,set_good_indices4item,
#	 index4flow: $index4flow\n");
	my $idx = $index4flow;
	my (@good);
	my ($j);

	# print("1. param_flow_pink,_set_good_labels4item,
	# flow index:$idx, prog name:
	# @{$param_flow_pink->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
		...
		# print("param_flow_pink, _set_good_labels4item:
			...
			&& ( @{$values_aref}[$i] ne $empty_string ) )
		{
			...
			if ( @$values_aref[$i] ne "'nu'" ) {    #
				...
				my $name = ${ @{ $param_flow_pink->{_names_aref2} }[$idx] }[$i];

				# print("2. param_flow_pink,_set_good_labels4item,
				...
				print(
"param_flow_pink, _set_good_labels_4item: unexpected ending\n"
				);
			}
...
# NADA print("param_flow_pink, _set_good_labels_4item: no values are present, can not be saved\n");
	...
	$param_flow_pink->{_num_good_labels_aref} = \@num_good_labels;
	$good_labels[$idx]                        = \@good;
	$param_flow_pink->{_good_labels_aref2}    = \@good_labels;

	#print("	3. param_flow_pink,_set_good_labels4item,
	#num_good_labels= $num_good_labels[$idx] ,
	#labels/names are:	@{@{$param_flow_pink->{_good_labels_aref2}}[$idx]}\n");
...
#		print("param_flow_pink, _set_good_values_4item: index=$i, values_aref= @$values_aref[$i]\n");
		...
		#				print("2. param_flow_pink,_set_good_values4item,value:$value \n");
				...
				print(
"param_flow_pink, _set_good_values_4item: unexpected ending\n"
				);
			}
...
#			print("NADA,param_flow_pink, _set_good_values_4item: no values are present, can not be saved\n");
	...
	$param_flow_pink->{_good_values_aref2} = \@good_values;

# print("	3. param_flow_pink,_set_good_values4item,num_good_values= $num_good_values[$idx], values are:	@{@{$param_flow_pink->{_good_values_aref2}}[$idx]}\n");
	return ();
}
...
sub clear {
	...
	$param_flow_pink->{_checkbuttons_aref}       = '';
	$param_flow_pink->{_checkbuttons_aref2}      = '';
	$param_flow_pink->{_destination_index}       = '';
	$param_flow_pink->{_end}                     = '';
	$param_flow_pink->{_first_idx}               = 0;
	$param_flow_pink->{_flow_prog_names_aref}    = '';
	$param_flow_pink->{_good_checkbuttons_aref2} = '';
	$param_flow_pink->{_good_labels_aref2}       = '';
	$param_flow_pink->{_good_values_aref2}       = '';
	$param_flow_pink->{_index2move}              = '';
	...
	$param_flow_pink->{_num_good_labels_aref}    = 0;
	...
	$param_flow_pink->{_prog_version_aref}       = '';
	$param_flow_pink->{_selection_index}         = -1;
	$param_flow_pink->{_start}                   = '';
	$param_flow_pink->{_values_aref}             = '';
...
#	print("param_flow_pink, clear finished\n");
	return ();
...
sub clear_flow_items_version_aref {
	...
	if ( $param_flow_pink->{_prog_version_aref} ) {

		$param_flow_pink->{_prog_version_aref} = '';

		# if ($param_flow_pink->{_prog_version_aref} eq '') {
...
# }
		...
		print(
"param_flow_pink, clear_flow_items_version_aref, missing program_version_aref\n"
		);
	}
...
sub delete_selection {
	...
	my $end       = $param_flow_pink->{_indices};
	my $num_items = $param_flow_pink->{_num_items};
	my $first     = 0;
	$param_flow_pink->{_index2delete} = $index2delete;
...
sub first_idx {
	...
	$param_flow_pink->{_first_idx} = 0;
	return ( $param_flow_pink->{_first_idx} );
...
my ($i,$j, $length);
 		...
 		@values_aref 	= @{@{$param_flow_pink->{_checkbuttons_aref2}}[$index]};
 		$length			= scalar @values_aref;

   		print("param_flow_pink,get_check_buttons_settings: is @values_aref\n");

  		#for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
  		for ($i=0; $i < $length; $i++ ) {
    		#$values[$j]  = $values_aref[$i]; 
    			#print("param_flow_pink, get_check_buttons_settings :index $j values: $values[$j]\n");
     	if($values_aref[$i] eq $nu || ) {
     	  	$on_off[$i]     = $off;
		  	# print(" 1. param_flow_pink, get_check_buttons_settings,$on_off[$i]\n");
     	}
     	else {
       		$on_off[$i]     = $on;
		    # print(" 2. param_flow_pink, get_check_buttons_settings,$on_off[$i]\n");
     	}
     	# print("param_flow_pink: get_check_buttons_settings :index $i setting $nu is: $on_off[$i]\n");
   		}
...
sub get_check_buttons_settings {
	...
	if ( $index >= 0 ) {
		my @on_off = @{ @{ $param_flow_pink->{_checkbuttons_aref2} }[$index] };

#		print("param_flow_pink,get_check_buttons_settings: is @on_off\n");
...
sub get_flow_index {
	my ($self) = @_;
	my $current_idx = $param_flow_pink->{_index4flow};

#	print("param_flow_pink, get flow_index,
#	current_idx is $param_flow_pink->{_index4flow}\n");
	return ($current_idx);
}
...
sub get_flow_prog_names_aref {
	...
	if ( $param_flow_pink->{_prog_names_aref} ne $empty_string ) {

#		print("param_flow_pink, get_flow_prog_names_aref,
#		@{$param_flow_pink->{_prog_names_aref}}\n");
...
sub get_flow_items_version_aref {
	my ($self) = @_;
	my $program_version_aref = $param_flow_pink->{_prog_version_aref};

#	print("param_flow_pink, get_flow_items_version_aref
#	@{$param_flow_pink->{_prog_version_aref}}\n");
...
sub get_good_labels_aref2 {
	...
	if ( $param_flow_pink->{_good_labels_aref2} ) {
		my $good_labels_aref2 = $param_flow_pink->{_good_labels_aref2};

#		print(" param_flow_pink,get_good_labels_aref2,
#		good_labels for index 0=
#		@{@{$param_flow_pink->{_good_labels_aref2}}[0]}\n");
...
sub get_num_good_labels_aref {
	...
	if ( $param_flow_pink->{_num_good_labels_aref} ) {
		my $num_good_labels_aref = $param_flow_pink->{_num_good_labels_aref};
...
sub get_num_good_values_aref {
	...
	if ( $param_flow_pink->{_num_good_values_aref} ) {
		my $num_good_values_aref = $param_flow_pink->{_num_good_values_aref};
#			print("e\n");
...
#		print("param_flow_pink, get_values :flow index= $index\n");
		@values = @{ @{ $param_flow_pink->{_values_aref2} }[$index] };
#		$length      = scalar @values;
#		print("param_flow_pink, get_values :length=$length\n");

#		for my $in (@values) {
#			print("param_flow_pink,get_values :value is--$in--\n");
#		}
#		print("\n");
...
#    print("param_flow_pink,view_data: checkbuttons: @{@{$param_flow_pink->{_checkbuttons_aref2}}[$i]}\n\n");
		...
		print(
"param_flow_pink,get_values_aref :selection_index <=0  or vmissing alues_aref2\n"
		);
		return ();
	}
}
...
#	 print("param_flow_pink,insert_selection start is $idx2mv\n");
	# print("param_flow_pink,insert_selection,destn_id is $destn_idx\n");
	# print("param_flow_pink,insert_selection, last index  is $end\n");
...
sub length {
	...
	$index = $param_flow_pink->{_selection_index};
	if ( $index >= 0 ) {
...
#		print("e\n");
	...
	if ( $index >= 0 ) {
		...
		@{ $param_flow_pink->{_checkbuttons_aref2} }[$index] = $chkbut_aref;

		# $length			= scalar @$chkbut_aref;

	  # print("param_flow_pink,set_check_buttons_settings: is @$chkbut_aref\n");

#for ($i=1,$j=0; $i < $length; $i=$i+2,$j++ ) {
#for ($i=0; $i < $length; $i++ ) {
#$values[$j]  = $values_aref[$i];
#print("param_flow_pink, set_check_buttons_settings :index $j values: $values[$j]\n");
#if(@$values_aref[$i] eq $nu) {
#  	$on_off[$i]     = $off;
# print(" 1. param_flow_pink, set_check_buttons_settings,$on_off[$i]\n");
#}
#else {
#		$on_off[$i]     = $on;
# print(" 2. param_flow_pink, set_check_buttons_settings,$on_off[$i]\n");
#}
# print("param_flow_pink: set_check_buttons_settings :index $i setting $nu is: $on_off[$i]\n");
#}
#return();
	}
	return ();
...
sub set_flow_items_version_aref {
	...
	if ($program_version_aref) {
		$param_flow_pink->{_prog_version_aref} = $program_version_aref;

#print("param_flow_pink, set_flow_items_version_aref @{$param_flow_pink->{_prog_version_aref}}\n");
	}
}
...
sub set_insert_start {
	my ( $self, $start ) = @_;
	$param_flow_pink->{_index2move} = $start;

#	print("param_flow_pink,set_insert_start is $start\n");
	return ();
}
...
sub set_insert_end {
	my ( $self, $end ) = @_;
	$param_flow_pink->{_destination_index} = $end;

#	print("param_flow_pink,set_insert_end  $end \n");
...
=head2 sub set_flow_index 
 ...
 select an item from which to extract data
      			...
      			print("param_flow_pink, set flow_index, prog name is @{$param_flow_pink->{_prog_names_aref}}[$index]\n");
...
# all parameters values are changed when ANY flow item is selected
		...
		print(
"param_flow_pink, set_flow_index,index does not exist, index:$index\n"
		);
...
#	print(" param_flow_pink, set_names:  @{$names_aref}, index is $index\n");

	if ( $index >= 0 ) {
		my @names_aref;
		my ($length);

		@{ $param_flow_pink->{_names_aref2} }[$index] = $names_aref;
...
sub set_good_labels {
	...
	my ($self) = @_;
	my $length = $param_flow_pink->{_num_items4flow};

#	print("param_flow_pink,set_good_labels
#	num_items4flow:	$param_flow_pink->{_num_items4flow}\n");
...
sub set_good_values {
	...
	my ($self) = @_;
	my $length = $param_flow_pink->{_num_items4flow};    # no. programs in flow

# print("param_flow_pink,set_good_values num_items4flow:	$param_flow_pink->{_num_items4flow}\n");
...
#	 print("param_flow_pink,set_good_labels4item,
#	 self,index4flow: $self,$index4flow\n");
	...
	# print("1. param_flow_pink,set_good_labels4item,
	# flow index:$idx, prog name:
	# @{$param_flow_pink->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
		...
		if ( @$values_aref[$i] ne "'nu'" ) {
			...
			#print("2. param_flow_pink,set_good_labels4item,
			#good index #$i, name:$name \n");
	...
	# print("	3. param_flow_pink,set_good_,names4item,
	# num_good_labels= $num_good_labels , names are:
	# @{@{$param_flow_pink->{_good_labels_aref2}}[$idx]}\n");
	return ();
}
...
#	 print("param_flow_pink,set_good_values4item,
#	 self,index4flow: $self,$index4flow\n");
	...
	# print("1. param_flow_pink,set_good_values4item,
	# flow index:$idx, prog name:
	# @{$param_flow_pink->{_prog_names_aref}}[$idx] \n");
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
		...
		if ( @$values_aref[$i] ne "'nu'" ) {
			...
			# print("2. param_flow_pink,set_good_values4item,
			...
			print(
				"param_flow_pink,set_good_values4item, a bad value detected\n");
	...
	# print("	3. param_flow_pink,set_good_,values4item,
	# num_good_values= $num_good_values , values are:
	# @{@{$param_flow_pink->{_good_values_aref2}}[$idx]}\n");
	return ();
}
...
# print("param_flow_pink,set_values param_flow_pink->{_selection_index}= $param_flow_pink->{_selection_index}\n");
	...
	if ( $param_flow_pink->{_selection_index} >= 0 ) {
		...
		if ( $index >= 0 ) {
			...
			$cloned_values_aref2[$index] =
			  clone( \@{ @{ $param_flow_pink->{_values_aref2} }[$index] } );
			
			$length                  = scalar @{$cloned_values_aref2[$index]};
            # print(" OK 1. param_flow_pink, set_values_aref:length=$length\n");          
			
			@{ @{ $param_flow_pink->{_values_aref2} }[$index]} = @{$cloned_values_aref2[$index]};
			
#        print("1. param_flow_pink, set_values_aref:@{@{ $param_flow_pink->{_values_aref2} }[$index]}\n");
#         print("1. param_flow_pink, set_values_aref:@{$cloned_values_aref2[$index]}\n");
...
=head2 sub stack_checkbuttons_aref2
  ...
  #for (my $i=0; $i<=$index;$i++) {
    #print("param_flow_pink,checkbuttons_aref,@{@{$param_flow_pink->{_checkbuttons_aref2}}[$i]} index $i\n");
  #}
     #print("param_flow_pink, checkbuttons_aref2, num_items4checkbuttons $param_flow_pink->{_num_items4checkbuttons}\n");
...
sub stack_checkbuttons_aref2 {
	...
	my $index = $param_flow_pink->{_index4checkbuttons} + 1;
#		print("f\n");

	$checkbuttons[$index] = $checkbuttons_aref;
	$param_flow_pink->{_checkbuttons_aref2} = \@checkbuttons;
...
# print("param_flow_pink,stack_checkbuttons_aref2,  @{$checkbuttons[$index]},idx $index num_items $param_flow_pink->{_num_items}\n");
# print("param_flow_pink,stack_checkbuttons_aref2,  @{@{$param_flow_pink->{_checkbuttons_aref2}}[$index]},idx $index num_items $param_flow_pink->{_num_items}\n");
...
=head2 sub stack_names_aref2 
 ...
 DrBN
     ...
     #print("param_flow_pink, names_aref2, num_items4names $param_flow_pink->{_num_items4names}\n");

 #print("param_flow_pink,names_aref,$names[$index],item $index\n");
 # print("param_flow_pink, stack_names_aref2, names in program# $index are @{$names[$index]}[0]\n");
 for(my $i = 0; $i <=$index; $i++) {
   	print("param_flow_pink,stack_names_aref2, cumulative first names are:  @{$names[$i]}[0], prog num =$i\n");
 }
...
DB
  #for (my $i=0; $i<=$index;$i++) {
   # print("param_flow_pink,values_aref2,@{@{$param_flow_pink->{_values_aref2}}[$i]} index $i\n");
  #}
  #print("param_flow_pink,values_aref2,num_items=$param_flow_pink->{_num_items}\n");
  # print("param_flow_pink,values_aref2, index $index\n");
...
#		print("i\n");
		...
		my $index = $param_flow_pink->{_index4values} + 1;

		$values[$index] = $values_aref;
		$param_flow_pink->{_values_aref2} = \@values;

		$param_flow_pink->{_indices} = $index;
		$param_flow_pink->{_index4values}++;
		$param_flow_pink->{_num_items4values}++;
		$param_flow_pink->{_num_items} = $param_flow_pink->{_num_items4values};

# print("param_flow_pink,stack_values_aref2, values: @{$values[$index]},idx $index num_items $param_flow_pink->{_num_items}\n");
...
=head2 sub view_data
 ...
 Data viewer for debugging
   print("B4  delete: names are  $param_flow_pink->{_names_aref2}\n");    # ref_Array
   print("B4  delete: names are  @{$param_flow_pink->{_names_aref2}}\n");     # [ref_array0 ref_array1 .... 
   print("B4  delete: names are  @{$param_flow_pink->{_names_aref2}}[0]\n");  # ref_array0

   print("B4  delete: names are  @{$param_flow_pink->{_names_aref2}}[1]\n"); #ref_array1
     
   print("B4  delete: names are  @{@{$param_flow_pink->{_names_aref2}}[0]}\n"); # all names in ref_array 0 
   print("B4  delete: names are  @{@{$param_flow_pink->{_names_aref2}}[1]}[0]\n"); # first name  in ref_array 
   print("param_flow_pink,view_data: list for each item\n");
...
sub view_data {
	...
	my $indices   = $param_flow_pink->{_indices};
	$num_progs[0] = $param_flow_pink->{_num_items};
	$num_progs[1] = scalar( @{ $param_flow_pink->{_names_aref2} } );
	...
	$num_progs[5] = scalar( @{ $param_flow_pink->{_checkbuttons_aref2} } );

	print(
"\nparam_flow_pink,view_data:number of programs in flow, in 6 different ways @num_progs \n"
	);

# print("\n param_flow_pink,view_data, _prog_names @{$param_flow_pink->{_prog_names_aref}}\n");
# print("param_flow_pink,view_data:max index (# items in a flow) = $indices  \n\n");

# print("param_flow_pink,view_data, param_flow_pink_ has hash=$param_flow_pink\n ");

	for ( my $i = 0 ; $i <= $indices ; $i++ ) {

		for my $name ( @{ @{ $param_flow_pink->{_names_aref2} }[$i] } ) {
			print("param_flow_pink,view_data: name:      $name\n");
		}

#     print("param_flow_pink,view_data: checkbuttons: @{@{$param_flow_pink->{_checkbuttons_aref2}}[$i]}\n\n");
		# for my $value ( @{ @{ $param_flow_pink->{_values_aref2} }[$i] } ) {
		for my $value ( @{$cloned_values_aref2[$i]} ) {
			print("param_flow_pink,view_data: value:     $value\n");
		}
