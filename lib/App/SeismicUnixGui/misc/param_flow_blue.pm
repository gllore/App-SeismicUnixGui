...
#		print("param_flow_blue, _get_values_aref, @values_aref, index=$item_index\n");
		return ( \@values_aref );
	}
}
...
#	print("1. param_flow_blue,_set_good_values4item, flow index:$idx, 
#	prog name:@{$param_flow_blue->{_prog_names_aref}}[$idx] \n");
...
#		print("param_flow_blue, _set_good_values_4item: index=$i, values_aref= @$values_aref[$i]\n");
		if ( defined( @{$values_aref}[$i] )
			&& ( @{$values_aref}[$i] ne $empty_string ) )
		{
			...
			if ( @$values_aref[$i] ne "'nu'" ) {
				...
				my $value =
					${ @{ $param_flow_blue->{_values_aref2} }[$idx] }[$i];
...
#			print("NADA,param_flow_blue, _set_good_values_4item: no values are present, can not be saved\n");
		}
	}
...
sub delete_selection {
	...
	and $num_items > 0 ) {
		...
		for ( my $i = $index_after, my $j = $index2delete; $i <= $end; $i++, $j++ ) {
			...
			@{ @{ $param_flow_blue->{_checkbuttons_aref2} }[$j] } =
				@{ @{ $param_flow_blue->{_checkbuttons_aref2} }[$i] };
			@{ @{ $param_flow_blue->{_names_aref2} }[$j] } =
				@{ @{ $param_flow_blue->{_names_aref2} }[$i] };
			@{ @{ $param_flow_blue->{_values_aref2} }[$j] } =
				@{ @{ $param_flow_blue->{_values_aref2} }[$i] };
			@{ $param_flow_blue->{_prog_names_aref} }[$j] =
				@{ $param_flow_blue->{_prog_names_aref} }[$i];
		}
...
#		my $ans = @{@{$param_flow_blue->{_good_values_aref2}}[0]};
#		print("param_flow_blue,get_good_values_aref2,good_values for index=0:$ans\n");
...
sub insert_selection {
	...
	@tmp_checkbuttons_aref =
		@{ @{ $param_flow_blue->{_checkbuttons_aref2} }[$idx2mv] };
	...
	if ( $idx2mv > $first ) {
		...
		for ( my $i = $first, my $j = $first; $j < $idx2mv; $i++, $j++ ) {
			$swap_prog_names[$i] =
				@{ $param_flow_blue->{_prog_names_aref} }[$j];
			$swap_names_aref[$i] =
				clone( \@{ @{ $param_flow_blue->{_names_aref2} }[$j] } );
			$swap_values_aref[$i] =
				clone( \@{ @{ $param_flow_blue->{_values_aref2} }[$j] } );
			$swap_checkbuttons_aref[$i] =
				clone( \@{ @{ $param_flow_blue->{_checkbuttons_aref2} }[$j] } );
	...
	@{ @{ $param_flow_blue->{_checkbuttons_aref2} }[$destn_idx] } =
		@tmp_checkbuttons_aref;
...
sub length {
	...
	if ( $index >= 0 ) {
		...
		print("param_flow_blue,length,  index=$index does not exist\n");
		print("param_flow_blue, length, length can not be calculated\n");
		return ();
	}
}
...
sub set_flow_index {
	...
	if (  CORE::length($index)  ) {
		...
		if ( $index ne $empty_string && $index >= 0 ) {
			...
			# print(
			# "param_flow_blue, set_flow_index,unexpected index, index:$index NADA\n"
			# );
		}
	}
		...
		print("param_flow_blue, set_flow_index,unexpected index value\n");
	}
...
sub set_values_aref {
	...
	if ( $param_flow_blue->{_selection_index} >= 0 ) {
		...
		# print("param_flow_blue, set_values_aref: selection index < 0 NADA\n");
	}
...
sub stack_flow_item {
	...
	if ($program_name_sref) {
		...
		# rint("param_flow_blue, stack_flow_item, index: $index\n");
...
sub stack_names_aref2 {
	...
	if ($names_aref) {
		...
		# print("param_flow_blue,stack_names_aref2,index: $index\n");
		...
		# for (my $i=0; $i<=$index;$i++) {
		#  	print("param_flow_blue,stack_names_aref2, an accumulating array of arrays: @{@{$param_flow_blue->{_names_aref2}}[$i]} item $i\n");
		# }
