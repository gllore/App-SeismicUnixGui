	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
			...
			&& ( @{$values_aref}[$i] ne $empty_string ) )
		{
			...
			if ( @$values_aref[$i] ne "'nu'" ) {    #
				...
				print(
"param_flow_blue, _set_good_labels_4item: unexpected ending\n"
				);
			}
...
#		print("param_flow_blue, _set_good_values_4item: index=$i, values_aref= @$values_aref[$i]\n");
		...
		#				print("2. param_flow_blue,_set_good_values4item,value:$value \n");
				...
				print(
"param_flow_blue, _set_good_values_4item: unexpected ending\n"
				);
			}
...
sub get_max_index {
	...
	if ( $param_flow_blue->{_num_items} >= 0 ) {
		...
		my $max_index = $param_flow_blue->{_num_items} + 1;

		my $result = $max_index;

# print("param_flow_blue,get_max_index, max_index = $param_flow_blue->{_max_index} \n");
		return ($result);

	}
	else {
		print("param_flow_blue,get_max_index; num items <0\n");
		return ();
	}
...
#    print("param_flow_blue,view_data: checkbuttons: @{@{$param_flow_blue->{_checkbuttons_aref2}}[$i]}\n\n");
		...
		print(
"param_flow_blue,get_values_aref :selection_index <=0  or vmissing alues_aref2\n"
		);
		return ();
	}
}
...
# all parameters values are changed when ANY flow item is selected
		...
		print(
"param_flow_blue, set_flow_index,index does not exist, index:$index\n"
		);
...
#	 self,index4flow: $self,$index4flow\n");
	...
	for ( my $i = 0, $j = 0 ; $i < $length ; $i++ ) {
		...
		if ( @$values_aref[$i] ne "'nu'" ) {
			...
			print(
				"param_flow_blue,set_good_values4item, a bad value detected\n");
...
# print("param_flow_blue,set_values param_flow_blue->{_selection_index}= $param_flow_blue->{_selection_index}\n");
	...
	if ( $param_flow_blue->{_selection_index} >= 0 ) {
		...
		# print("param_flow_blue,set_values_aref, flow index: $index\n");
...
# print("param_flow_blue,set_values_aref :values_aref[0] are @$values_aref[0]\n");
	return ();
}
...
sub view_data {
	...
	print(
"\nparam_flow_blue,view_data:number of programs in flow, in 6 different ways @num_progs \n"
	);

# print("\n param_flow_blue,view_data, _prog_names @{$param_flow_blue->{_prog_names_aref}}\n");
# print("param_flow_blue,view_data:max index (# items in a flow) = $indices  \n\n");

# print("param_flow_blue,view_data, param_flow_blue_ has hash=$param_flow_blue\n ");

	for ( my $i = 0 ; $i <= $indices ; $i++ ) {

		for my $name ( @{ @{ $param_flow_blue->{_names_aref2} }[$i] } ) {
			print("param_flow_blue,view_data: name:      $name\n");
		}
