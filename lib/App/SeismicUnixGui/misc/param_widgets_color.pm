package App::SeismicUnixGui::misc::param_widgets_color;

=head1 DOCUMENTATION

=head2 SYNOPSIS 

 PERL PROGRAM NAME: param_widgets_color
 AUTHOR:  Juan Lorenzo

=head2 CHANGES and their DATES

 DATE: V 0.0.3 Nov 6 2025
...
param_widgets_$this_color, changes,
 ...
 currently changes in param_widgets_$this_color package only works with regular flows 
...
sub _update_value_changes {
	...
	if ( $param_widgets->{_entry_in_switch} eq $off ) {
		...
		my $index_on_entry =
		  $param_widgets_color_href->{_parameter_index_on_entry};
		my $prior_value =
		  @{ $param_widgets_color_href->{_values_w_aref} }[$index_on_entry]
		  ->get();
		@{ $param_widgets_color_href->{_values_aref} }[$index_on_entry] =
		  $prior_value;
		...
		my $index_on_exit =
		  $param_widgets_color_href->{_parameter_index_on_exit};
		my $prior_value =
		  @{ $param_widgets_color_href->{_values_w_aref} }[$index_on_exit]
		  ->get();
		@{ $param_widgets_color_href->{_values_aref} }[$index_on_exit] =
		  $prior_value;

	}
	else {
		print(
			" 1. param_widgets_color, _update_value_changes, missing value\n");
	}
...
sub get_check_buttons_w_aref {
	...
	if ( $param_widgets_color_href->{_check_buttons_w_aref} ) {
		...
		print(
"param_widgets_color, get_check_buttons_w_aref, missing check_buttons_w_aref \n"
		);
		return ();
	}
}
...
sub get_labels_w_aref {
	...
	if ( $param_widgets_color_href->{_labels_w_aref} ) {
		...
		print(
			"param_widgets_color,get_labels_w_aref, missing labels_w_aref \n");
		return ();
	}
}
...
sub get_values_w_aref {
	...
	if ( $param_widgets_color_href->{_values_w_aref} ) {
		...
		print(
			"param_widgets_color,get_values_w_aref, missing values_w_aref \n");
		return ();
	}
}
...
#	print("param_widgets_color,get_entry_button_chosen_index,length_=$param_widgets_color_href->{_length} \n");
	...
	for ( my $choice = $first ; $choice < $length ; $choice++ ) {
		...
		my $label = @{ $param_widgets_color_href->{_labels_w_aref} }[$choice]
		  ->cget('-text');
...
#	     print("param_widgets_color,get_entry_button_chosen_index,label=$label\n");
			...
			@{ $param_widgets_color_href->{_values_w_aref} }[$choice] )
		{
			...
			print(
" param_widgets_color,get_entry_button_chosen_index, #$choice \n"
			);
			$index = $choice;
		}
	}
...
# print("param_widgets_color,set_check_buttons_w_aref, $check_buttons_w_aref \n");
		...
		print(
"param_widgets_color, set_check_buttons_w_aref,missing check_buttons_w_aref \n"
		);
...
sub set_index {
	...
	if ( length $index ) {
		...
		$param_widgets_color_href->{_index} = 0;
		print("param-widgets,first_idx:$param_widgets_color_href->{_index}\n");
	}
	else {
		print("param_widgets,set_index, missing value\n");
	}
...
sub set_labels_w_aref {
	...
	if ($labels_w_aref) {
		...
		print(
			"param_widgets_color,set_labels_w_aref, missing labels_w_aref \n");
...
sub set_values_frame {
	...
	if ( defined $values_frame_href ) {
		...
		print(
"param_widgets_color, set_values_frame, missing  values_frame_href\n"
		);
	}
...
# print("param_widgets_color,set_values_w_aref,  $param_widgets_color_href->{_values_w_aref}\n");
		...
		print(
			"param_widgets_color,set_values_w_aref, missing values_w_aref \n");
	}
	return ();
}
...
# print("param_widgets_color,set_labels, llabels: @{$param_widgets_color_href->{_labels_aref}}\n");
		...
		print("param_widgets_$this_color,set_labels, missing labels\n");
	}
	return ();
}
...
set single value in widget array
index required separately
...
sub set_value {
	...
	if ( length $value 
	 && length $param_widgets_color_href->{_index} 
	 &&  length $param_widgets_color_href->{_values_aref} ) 
	 {
	 	
		my $index = $param_widgets_color_href->{_index};
		$param_widgets_color_href->{_value} = $value;
	    my @values = @{ $param_widgets_color_href->{_values_aref} };
	    
        $values[$index] = $value; # update value
		$param_widgets_color_href->{_values_aref} = \@values; # update widget 
		print("param-widgets,first_idx:$param_widgets_color_href->{_value}\n");
		
	}
	else {
		print("param_widgets,set_value, missing value\n");
	}
