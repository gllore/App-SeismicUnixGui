package App::SeismicUnixGui::misc::backup_project_selector;
...
=head2 SYNOPSIS 
 ...
 PERL PERL PROGRAM NAME:  backup_project_selector
 AUTHOR: 	Juan Lorenzo
 DATE: 		May 3, 2018
 VERSION:   1.0.1
...
#	print("backup_project_selector, _continue\n");
	my $mw = $backup_project_selector->{_mw};
...
sub _cancel {
	...
	# print("backup_project_selector, _cancel\n");
	my $mw = $backup_project_selector->{_mw};
	$mw->destroy() if Tk::Exists($mw);
	exit(1);
}
...
=head2 sub _create_new
	...
	$backup_project_selector	->set_check_buttons_w_aref($L_SU_backup_project_selector->{_check_buttons_w_aref});
	$backup_project_selector	->set_labels_w_aref($L_SU_backup_project_selector->{_labels_w_aref});
	$backup_project_selector	->set_values_w_aref($L_SU_backup_project_selector->{_values_w_aref});
...
sub _create_new {
	...
	if (    length( $backup_project_selector->{_current_program_name} )
		and length( $backup_project_selector->{_param_widgets_pkg} ) )
	{

		my $name              = $backup_project_selector->{_current_program_name};
		my $param_widgets_pkg = $backup_project_selector->{_param_widgets_pkg};

		# print("backup_project_selector,_create_new, name =$name \n");
...
=cut
		...
		my $length = $backup_project_selector->{_length};
...
#		print(
#			"backup_project_selector,_create_new,values=@{$project->{_values_aref}}\n"
#		);
...
#		print(
#"backup_project_selector,_create_new,chkb=@{$project->{_check_buttons_settings_aref}}\n"
#		);
...
sub _get_message_box_w {
	...
	if ( $backup_project_selector->{_message_box_w} ) {

		return ( $backup_project_selector->{_message_box_w} );
...
sub _get_project_names_aref {
	...
	# print("backup_project_selector, project names: @$ls_ref\n");
...
sub _ok {
	...
	my $run_name         = $get->var->{_backup_project_selector};

	# 1. CASES when an existing project is selected
	if ( $backup_project_selector->{_active_project} ) {
...
#		print("backup_project_selector,_ok, length_check_buttons_on: $length_check_buttons_on\n");

		# CASE 1.A More than one button is selected
		if ( $length_check_buttons_on > 1 ) {    # possible mistake by user

#			print("CASE 1A: backup_project_selector,_ok, >1 length_check_buttons_on: $length_check_buttons_on\n");
#			print("backup_project_selector,_ok, length_check_buttons_on: $length_check_buttons_on\n");
			$message_box_w->delete( "1.0", 'end' );
			my $message = $message_director->backup_project_selector(0)
...
#   print("CASE 1.B :backup_project_selector,_ok, length_project names $length\n");

				$message_box_w->delete( "1.0", 'end' );
				my $message = $message_director->backup_project_selector(2)
...
#			print("CASE 1.C: backup_project_selector,_ok, length_check_buttons_on: $length_check_buttons_on\n");
			use File::Copy;
...
#		 # print(" backup_project_selector,_ok,active_index: @$active_indices_aref\n");

			my @active_indices = @$active_indices_aref;
			my $active_index   = $active_indices[0];

#			print(" backup_project_selector,_ok,active_index: $active_index\n");
			# print(" backup_project_selector,_ok,message widget: $message_box_w\n");
...
# print(" backup_project_selector,_ok,active label/project name is: $labels[$active_index]\n");
...
#print("backup_project_selector,_ok,create new Project and its directories \n");
#print("backup_project_selector,_ok,copy FROM:$from TO:$to \n");

#		    print("backup_project_selector,_ok, sh $global_libs->{_script}$run_name \n");
			system("sh $global_libs->{_script}$run_name");

#      print("backup_project_selector,_ok,copying new active project configuration file \n FROM:$from TO:$to");
# kill LSU_backup_project_selector exit with 1
			_continue();

		}
		else {
			print(
"backup_project_selector,_ok,length_check_buttons_on, unexpected $length_check_buttons_on\n"
			);
		}
	}

	# 2. CASES for NEWLY created Project Backup Configuration File
	elsif ( $backup_project_selector->{_create_new} ) {
  ...
  #		print("backup_project_selector,_ok, labels @{$project->{_names_aref}}\n");
  #		print("backup_project_selector,_ok, values @{$project->{_values_aref}}\n");
  #		print("backup_project_selector,_ok, prog_name ${$project->{_prog_name_sref}} \n");
...
# saves the configuration file ONLY to ./L_SU/configuration/active/Project.config
   ...
   # print("backup_project_selector,_ok, active_project_name $active_project_name \n");
		my $CONFIGURATION  = $user_constants->get_CONFIGURATION();
...
# uipdate active project to lates changed Entry widget values in the backup_project_selector GUI
...
#			print("backup_project_selector,_ok, CASE 2A of new project copying from $FROM_project_config to $TO_project_config\n");
	...
	#			print("backup_project_selector,_ok,create new Project and its directories \n");
			...
			# print("backup_project_selector,_ok, kill windows but exit with 1 \n");
			...
			$message_box_w->delete( "1.0", 'end' );
			my $message =
			  $message_director->backup_project_selector(1);   # project already exists
			$message_box_w->insert( 'end', $message );

#			print("backup_project_selector,_ok, CASE 2B A project with that name exists already. Try again \n");
		}
...
sub _set_gui {
	...
	# print("backup_project_selector,_set_gui\n");
	_set_length();
	...
	#	print(" backup_project_selector,param_widgets_pkg: $param_widgets_pkg\n");

	if ($param_widgets_pkg) {
		my $project_names_aref = $backup_project_selector->{_project_names_aref};
		my $PROJECT_HOMES_aref = $backup_project_selector->{_PROJECT_HOMES_aref};
...
# print("backup_project_selector,PROJECT HOMES are @$PROJECT_HOMES_aref\n");# only part of array is full

		if ( $backup_project_selector->{_length} == 0 ) {    # no project name
			my @check_buttons = ('off');
			$param_widgets_pkg->set_check_buttons( \@check_buttons );
			$param_widgets_pkg->redisplay_check_buttons;

		}
		elsif ( $backup_project_selector->{_length} == 1 ) {    # only one project name
			my @check_buttons = ('on');
			$param_widgets_pkg->set_check_buttons( \@check_buttons );
			$param_widgets_pkg->redisplay_check_buttons;

		}
		elsif ( $backup_project_selector->{_length} > 1 )
...
# print("backup_project_selector,_set_gui,active_project_name = $active_project_name\n");
	  ...
	  # print("backup_project_selector,_set_gui i=$i,active project match chkn ON \n");

				}
				else {
					$check_buttons[$i] = 'off';

 # print("backup_project_selector,_set_gui,i=$i, NO active project match chkn OFF \n");
				}
			}

# print("backup_project_selector,_set_gui,labels: @$labels_aref n");  # most label spaces are empty
			...
			print("backup_project_selector, _set_gui, lost logic\n");
			return ();
		}
		...
		print(
"backup_project_selector,_set_gui, param_widgets_pkg must be first created\n"
		);
		return ();
	}
}
...
sub _set_start_of_create_new {
	...
	$backup_project_selector->{_create_new}     = 1;
	$backup_project_selector->{_active_project} = 0;

	my $create_new_button_w = $backup_project_selector->{_create_new_button_w};
...
sub _set_first_idx {
	...
	my ($self) = @_;
	$backup_project_selector->{_first_idx} = 0;

 # print("backup_project_selector, _set_first_idx: $backup_project_selector->{_first_idx}\n");
	return ();
}
...
sub _set_length {
	...
	# print("backup_project_selector, _set_length project names: @$ls_ref\n");
	my $length = scalar @$ls_ref;
	$backup_project_selector->{_length} = $length;

	# print("backup_project_selector, _set_length no. projects: $length\n");
...
sub _set_PROJECT_HOMES_aref {
	...
	# print("backup_project_selector, _set_PROJECT_HOMES: @$ls_aref\n");
	$backup_project_selector->{_PROJECT_HOMES_aref} = $ls_aref;
	return ();
}
...
sub create_new {
	...
	#	print("backup_project_selector,create_new,value: $value\n");
	...
	if ($value) {
		...
		# print("backup_project_selector,save,error, value: $value\n");
	}
	return ();
}
...
#	print("backup_project_selector,ok,value: $value\n");
	...
	if ($value) {
		...
		print("backup_project_selector,ok,error, value: $value\n");
	}
...
sub set_check_buttons_w_aref {
	...
	if ($check_buttons_w_aref) {
		...
		$backup_project_selector->{_check_buttons_w_aref} = my $check_buttons_w_aref;

	}
	else {
		print(
"backup_project_selector, set_check_buttons_w_aref,missing check_buttons_w_aref \n"
		);
	}
	return ();
}
...
sub set_create_new_button_w {
	...
	if ($create_new_button_w) {
		...
		$backup_project_selector->{_create_new_button_w} = $create_new_button_w;

	}
	else {
		print("backup_project_selector, set_,missing  create_new_button_w \n");
	}
	return ();
}
...
=head2 sub set_gui
	...
	Always carried out when main LSU_backup_project_selector begins
...
sub set_gui {
	...
	# print("backup_project_selector, set_gui\n");
	_set_gui();
	return ();
}
...
# print("backup_project_selector,set_hash_ref, hash-ref detected\n");
#$L_SU_backup_project_selector->{_values_w_aref} 			= $param_widgets	-> get_values_w_aref();
#$L_SU_backup_project_selector->{_labels_w_aref} 			= $param_widgets	-> get_labels_w_aref();
#$L_SU_backup_project_selector->{_check_buttons_w_aref} 	= $param_widgets	-> get_check_buttons_w_aref();
#print($hash-ref->{_});

	}
	else {
		print("backup_project_selector,set_hash_ref, no hash-ref deteced\n");
	}
	return ();
}
...
sub set_labels_w_aref {
	...
	if ($labels_w_aref) {
		...
		$backup_project_selector->{_labels_w_aref} = my $labels_w_aref;

	}
	else {
		print("backup_project_selector,set_labels_w_aref, missing labels_w_aref \n");
	}
	return ();
}
...
sub set_length {
	...
	# print("backup_project_selector, no. projects: $length\n");
...
sub set_message_box_w {
	...
	if ($widget_ref) {
		...
		$backup_project_selector->{_message_box_w} = $widget_ref;

	}
	else {
		print("backup_project_selector, set_message_box_w, no message box widget\n");
	}
...
sub set_mw {
	...
	if ($mw) {
		...
		$backup_project_selector->{_mw} = my $mw;

	}
	else {
		# print("backup_project_selector, set_mw,missing check_buttons_w_aref \n");
	}
	return ();
}
...
sub set_param_widgets_pkg {
	...
	if ( length $pkg_ref ) {
		...
		$backup_project_selector->{_param_widgets_pkg} = $pkg_ref;

#		print("backup_project_selector, set_param_widgets_pkg: $backup_project_selector->{_param_widgets_pkg}\n");

	}
	else {
		print("backup_project_selector, no package reference\n");
	}
	return ();
}
...
sub set_current_program_name {
		...
		and length( $backup_project_selector->{_param_widgets_pkg} ) )
	{
		...
		$backup_project_selector->{_current_program_name} = $current_program_name;
		...
		print(
"backup_project_selector, set_current_program_name: missing name or package \n"
		);
	}
	return ();
}
...
sub _set_project_names_aref {
	...
	$backup_project_selector->{_project_names_aref} = $project_names_aref;

 # print("backup_project_selector, _set_project_names_aref: @{$project_names_aref}\n");
	return ();
}
...
sub set_values_w_aref {
	...
	if ($values_w_aref) {
		...
		my $backup_project_selector->{_values_w_aref} = $values_w_aref;

	}
	else {
		print("backup_project_selector,set_values_w_aref, missing values_w_aref \n");
	}
	return ();
}
...
sub set_widget {
	...
	if ( length($widget) ) {
		...
		$backup_project_selector->{_widget} = $widget;

		#		print("backup_project_selector, set_widget=$widget\n");

	}
	else {
		print("backup_project_selector, set_widget,missing widget \n");
	}
	return ();
}
