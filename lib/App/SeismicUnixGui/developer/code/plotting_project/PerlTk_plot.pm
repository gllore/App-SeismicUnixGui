...
sub _BOX {
		...
		&& $PerlTk_plot->{_x_max4plot} ne $empty_string )
	{
		...
		$PerlTk_plot->{_BOX} = [ $t_min_s, $t_max_s, $x_min, $x_max ];
...
sub _VIEWPORT {
		...
		&& $PerlTk_plot->{_viewport_ymax} ne $empty_string )
	{
		...
		$PerlTk_plot->{_VIEWPORT} = [ $viewport_xmin, $viewport_xmax, $viewport_ymin, $viewport_ymax ];
...
sub _first_sample_index2plot {
		...
		&& $PerlTk_plot->{_t_min_s4data} ne $empty_string )
	{
		...
		# correct if sample is less than the minimum time plottted, outside bounds
		if ( $t_min_s4plot <= $t_min_s ) {

			$t_min_s4plot = $t_min_s;

		}
		elsif ( $t_min_s4plot > $t_min_s ) {

			$t_min_s4plot = $t_min_s4plot;
		}
		else {
			print("PerlTk_plot,_first_sample_index2plot, unexpected time value\n");
		}

		my $first_sample_index2plot = ( ( $t_min_s4plot - $t_min_s ) * 1000000. / $samples_interval_us ) - 1;

		$first_sample_index2plot = ceil($first_sample_index2plot);
...
=head2 sub _last_sample_index2plot

=cut

sub _last_sample_index2plot {

	my ($self) = @_;

	if (   $PerlTk_plot->{_t_max_s4plot} ne $empty_string
		&& $PerlTk_plot->{_sample_interval_us} ne $empty_string
		...
		&& $PerlTk_plot->{_samples_per_trace} ne $empty_string )
	{
		...
		use POSIX;
		my $t_max_s4plot           = $PerlTk_plot->{_t_max_s4plot};
		my $t_max_s                = $PerlTk_plot->{_t_max_s4data};
		my $samples_interval_us    = $PerlTk_plot->{_sample_interval_us};
		my $max_sample_index_2plot = $PerlTk_plot->{_samples_per_trace} - 1;

		# correct if sample is less than the minimum time plottted, outside bounds
		if ( $t_max_s4plot >= $t_max_s ) {

			$t_max_s4plot = $t_max_s;

		}
		elsif ( $t_max_s4plot < $t_max_s ) {

			$t_max_s4plot = $t_max_s4plot;
		}
		else {
			print("PerlTk_plot,_last_sample_index2plot, unexpected time value\n");
		}

		my $last_sample_index2plot = ( ( $t_max_s - $t_max_s4plot ) * 1000000. / $samples_interval_us ) - 1;
		$last_sample_index2plot = ceil($last_sample_index2plot);

		if ( $last_sample_index2plot >= $max_sample_index_2plot ) {

			$last_sample_index2plot = $max_sample_index_2plot;

		}
		elsif ( $last_sample_index2plot < $max_sample_index_2plot ) {

			$last_sample_index2plot = $last_sample_index2plot;

		}
		else {

			print("PerlTk_plot,_last_sample_index2plot, unexpected index\n");
		}

		$PerlTk_plot->{_last_sample_index2plot} = $last_sample_index2plot;

		print("PerlTk_plot,_last_sample_index2plot: $last_sample_index2plot \n");

	}
	else {
		print("PerlTk_plot,_last_sample_index2plot, missing parameters \n");
	}
	return ();
		...
		&& $PerlTk_plot->{_viewport_ymax} ne $empty_string )
	{
		...
		if ( $png_page_width <= $png_page_height ) {
			...
			my $seismic_plot_width =
				$png_page_width * ( 1 - ( $x_frac_min + ( 1 - $x_frac_max ) ) );
			...
			my $seismic_plot_height =
				$png_page_height * ( 1 - ( $y_frac_min + ( 1 - $y_frac_max ) ) );
			$PerlTk_plot->{_seismic_plot_height} = $seismic_plot_height;
...
sub set_pl_plot {
		...
		&& $PerlTk_plot->{_png_page_height} ne $empty_string )
	{
		...
		$PerlTk_plot->{_pl} = PDL::Graphics::PLplot->new(
			...
			FILE        => "test.png",
			ORIENTATION => 1,
			PAGESIZE    => [ $PerlTk_plot->{_png_page_width}, $PerlTk_plot->{_png_page_height} ],
...
sub wiggle_png {
	...
	if (   $PerlTk_plot->{_x_min4data} ne $empty_string
		...
		&& nelem( $PerlTk_plot->{_ampls} ) != 0 )
	{
		...
		my $amplitudes = slice $ampls( $first_sample_index2plot : $last_sample_index2plot, $first_trace_index2plot : $last_trace_index2plot );
		...
		my $ampl_shift = ones( $samples_per_trace2plot, $number_of_traces2plot )->ylinvals( $first_x4plot, $last_x4plot );

		my @dims_ampl_shift = dims $ampl_shift;

		my $t2plot = zeros( $samples_per_trace2plot, $number_of_traces2plot )->xlinvals( $first_t_s4plot, $last_t_s4plot );
