...
=head2 SYNOPSIS 
 ...
 PERL PROGRAM NAME: L_SUV0.82.2.pl 
 AUTHOR: 	Juan Lorenzo
 DATE: 		June 22 2017 
 ...
 V0.82.2 April 2023, ready for automatic installation and Earthscope summer program 
...
$main_href->{_my_dialog_box_w} = ( $main_href->{_mw} )->Toplevel(
	-background => $var->{_my_yellow},
);
...
$main_href->{_my_dialog_cancel_button} = $main_href->{_my_dialog_box_w}->Button(
	-text    => "cancel",
	-font    => $arial_14_bold,
);	
...
$main_href->{_blank_button_spacer_center} = $main_href->{_my_dialog_box_w}->Button(
	-text                => " ",
	-relief              => 'flat',
...
$main_href->{ _blank_button_spacer_left} = $main_href->{_my_dialog_box_w}->Button(
		-text                => "",
		-relief              => 'flat',
		-highlightbackground => $var->{_my_yellow},
		-background          => $var->{_my_yellow},
...
$main_href->{ _blank_button_spacer_right} = $main_href->{_my_dialog_box_w}->Button(
		-text                => "",
		-relief              => 'flat',
		-highlightbackground => $var->{_my_yellow},
		-background          => $var->{_my_yellow},
...
=cut
	...
	my $cross_cartoon = ( $main_href->{_mw} )->Pixmap( -file => $global_libs->{_images} . 'cross.xpm', );
	my $minus_cartoon = ( $main_href->{_mw} )->Pixmap( -file => $global_libs->{_images} . 'minus.xpm', );
	my $flow_item_up_arrow
		= ( $main_href->{_mw} )->Bitmap( -file => $global_libs->{_images} . 'file_item_up_arrow.xbm', );
	my $flow_item_down_arrow
		= ( $main_href->{_mw} )->Bitmap( -file => $global_libs->{_images} . 'file_item_down_arrow.xbm', );
...
=cut	
	...
	my $side_menu_bar_right = ( $main_href->{_help_menubutton} )->Menu( -font => $arial_16 );
...
=cut
	...
	my $side_menu_bar_left = ( $main_href->{_file_menubutton} )->Menu( -font => $arial_16 );
...
=cut
	...
	( $main_href->{_delete_from_flow_button} )
		->bind( '<1>' => [ \&_L_SU_flow_bindings_any_color, 'delete_from_flow_button' ], );

	( $main_href->{_flow_item_up_arrow_button} )
		->bind( '<1>' => [ \&_L_SU_flow_bindings_any_color, 'flow_item_up_arrow_button' ], );

	( $main_href->{_flow_item_down_arrow_button} )
		->bind( '<1>' => [ \&_L_SU_flow_bindings_any_color, 'flow_item_down_arrow_button' ], );

	( $main_href->{_delete_whole_flow_button} )
		->bind( '<1>' => [ \&_L_SU_flow_bindings_any_color, 'delete_whole_flow_button' ], );
...
=cut  
	...
	my $sunix_NMO_Vel_Stk_programs_tab
		= $sunix_programs_III_book->add( "NMO_Vel_Stk programs tab", -label => "NMO_Vel_Stk" );
...
=cut
	...
	$sunix_NMO_Vel_Stk_programs_listbox->bind(
		'<1>' => [ \&_L_SU_sunix_bindings, 'sunix_select', 'neutral', 'NMO_Vel_Stk' ]
	);
