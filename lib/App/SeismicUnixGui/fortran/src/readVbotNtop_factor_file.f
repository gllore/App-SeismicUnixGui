...
!         print *, 'readVbotNtop_factor_file.f, err_message=',err_message
!         rest a little before trying again
!         call sleep(1)
          go to 10
         end if
       else
         print *, 'readVbotNtop_factor_file.f,locked, try again,
     +     read =',ready
         go to 10
       end if
!       remove lock file
11      close (status='delete',unit=30,iostat=err_message)
        if (err_message.ne.0) then
         go to 11
         print *, 'readVbotNtop_factor_file.f, err_messg=',err_message
        end if
