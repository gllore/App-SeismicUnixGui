      subroutine read_thickness_m_file(result,inbound)
         implicit none
...
!      print *, 'read_thickness_m_file.f, inbound is:', trim(inbound)
...
10     open(status='new',unit=30,file=inbound_locked,iostat=ready)
 ...
 !       check whether file opens data file
         if (err_msg.eq.0) then
          ...
          print *, 'read_thickness_m_file.f, result',result
          close (unit=29)

         else
          print *, 'read_thickness_m_file.f,locked, try again,read =',ready
...
11      close (status='delete',unit=30,iostat=err_msg)
        if (err_msg.ne.0) then
         go to 11
         print *, 'read_thickness_m_file.f, err_messg=',err_msg
        end if

       end if
       print *, 'read_thickness_m_file, result',result
      end subroutine read_thickness_m_file
