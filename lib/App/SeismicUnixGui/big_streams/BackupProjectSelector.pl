...
=head2 SYNOPSIS 
 ...
 PERL PROGRAM NAME: BackupProjectSelector.pl 
 AUTHOR: 	Juan Lorenzo
 DATE: 		June 30 2024 
...
$BackupProjectSelector->{_mw}                = $mw;
$BackupProjectSelector->{_param_widgets_pkg} = $param_widgets;

# print("main,package BackupProjectSelector->{_param_widgets}: $BackupProjectSelector->{_param_widgets_pkg}\n");
...
$BackupProjectSelector->{_create_new_button} = $save_button;
...
$BackupProjectSelector->{_values_w_aref}        = $param_widgets->get_values_w_aref();
$BackupProjectSelector->{_labels_w_aref}        = $param_widgets->get_labels_w_aref();
$BackupProjectSelector->{_check_buttons_w_aref} = $param_widgets->get_check_buttons_w_aref();

# $project_selector	->set_hash_ref($BackupProjectSelector);
$project_selector->set_check_buttons_w_aref( $BackupProjectSelector->{_check_buttons_w_aref} );
$project_selector->set_labels_w_aref( $BackupProjectSelector->{_labels_w_aref} );
$project_selector->set_values_w_aref( $BackupProjectSelector->{_values_w_aref} );
$project_selector->set_mw( $BackupProjectSelector->{_mw} );

# pass the package reference to another package
# MUST be called before the first gui is started (i.e., set_gui)
$project_selector->set_param_widgets_pkg( $BackupProjectSelector->{_param_widgets_pkg} );
$project_selector->set_current_program_name( $BackupProjectSelector->{_prog_name} );
$project_selector->set_message_box_w( $BackupProjectSelector->{_message_box_w} );
$project_selector->set_create_new_button_w( $BackupProjectSelector->{_create_new_button} );
