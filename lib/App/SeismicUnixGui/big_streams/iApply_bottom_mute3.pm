package App::SeismicUnixGui::big_streams::iApply_bottom_mute3;

=head1 DOCUMENTATION

=head2 SYNOPSIS 

 PACKAGE NAME: iApply_bottom_mute3
 AUTHOR: Juan Lorenzo
...
=head2
  ...
  Assume that the name of this parameter file is:
     $itemp_bot_mute_picks_sorted_par_.$iApply_bottom_mute3->{_file_in}

=cut

my $iApply_bottom_mute3 = {
    _gather_num      => '',
    _gather_header   => '',
    _offset_type     => '',
    _file_in         => '',
...
sub clear {
    $iApply_bottom_mute3->{_gather_num}      = '';
    $iApply_bottom_mute3->{_file_in}         = '';
    $iApply_bottom_mute3->{_freq}            = '';
    $iApply_bottom_mute3->{_inbound}         = '';
    $iApply_bottom_mute3->{_message_type}    = '';
    $iApply_bottom_mute3->{_gather_header}   = '';
    $iApply_bottom_mute3->{_offset_type}     = '';
    $iApply_bottom_mute3->{_number_of_tries} = '';
    $iApply_bottom_mute3->{_file_in}         = '';
    $iApply_bottom_mute3->{_parfile_in}      = '';
    $iApply_bottom_mute3->{_textfile_in}     = '';
}
...
sub gather_header {
    ...
    $iApply_bottom_mute3->{_gather_header} = $gather_header
      if defined($gather_header);

    #print(" header type is $iApply_bottom_mute3->{_gather_header}\n\n");
}
...
sub offset_type {
    ...
    $iApply_bottom_mute3->{_offset_type} = $offset_type
      if defined($offset_type);

    #print(" header type is $iApply_bottom_mute3->{_offset_type}\n\n");
}
...
sub gather_num {
    ...
    $iApply_bottom_mute3->{_gather_num} = $gather_num if defined($gather_num);
}
...
sub freq {
    my ( $variable, $freq ) = @_;
    $iApply_bottom_mute3->{_freq} = $freq if defined($freq);

    #print("freq is $iApply_bottom_mute3->{_freq}\n\n");
}
...
sub file_in {
    ...
    $iApply_bottom_mute3->{_file_in} = $file_in if defined($file_in);
    $iApply_bottom_mute3->{_inbound} =
      $DATA_SEISMIC_SU . '/' . $iApply_bottom_mute3->{_file_in} . $suffix_su;
}
...
sub min_amplitude {
    ...
    $iApply_bottom_mute3->{_min_amplitude} = $min_amplitude
      if defined($min_amplitude);

    #print("min_amplitude is $iApply_bottom_mute3->{_min_amplitude}\n\n");
}
...
sub max_amplitude {
    ...
    $iApply_bottom_mute3->{_max_amplitude} = $max_amplitude
      if defined($max_amplitude);

    #print("max_amplitude is $iApply_bottom_mute3->{_max_amplitude}\n\n");
}
...
=cut 
    ...
    #print("mute pick file is $iApply_bottom_mute3->{_parfile_in}\n\n");
...
=cut
    ...
    $sumute->clear();
    $sumute->parfile( $iApply_bottom_mute3->{_parfile_in} );
    $sumute->headerword( $iApply_bottom_mute3->{_offset_type} );
    $sumute->type('bottom');
...
=cut
    ...
    $suwind->clear();
    $suwind->setheaderword( $iApply_bottom_mute3->{_gather_header} );
    $suwind->min( $iApply_bottom_mute3->{_gather_num} );
    $suwind->max( $iApply_bottom_mute3->{_gather_num} );

    #print("gather num is $iApply_bottom_mute3->{_gather_num}\n\n");
...
=cut
    ...
    $sufilter->clear();
    $sufilter->freq( $iApply_bottom_mute3->{_freq} );
...
=cut
    ...
    $suximage->xlabel( $iApply_bottom_mute3->{_offset_type} );
    $suximage->legend($on);
    $suximage->cmap('rgb0');
    $suximage->loclip( $iApply_bottom_mute3->{_min_amplitude} );
    $suximage->hiclip( $iApply_bottom_mute3->{_max_amplitude} );
...
=cut
    ...
    @items = (
        $suwind[1],                       $in,
        $iApply_bottom_mute3->{_inbound}, $to,
        $suwind[2],                       $to,
        $sumute[1],                       $to,
        $sufilter[1],                     $to,
        $sugain[2],                       $to,
