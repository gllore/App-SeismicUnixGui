...
use aliased 'App::SeismicUnixGui::sunix::statsMath::sufwmix';
my $get					= L_SU_global_constants->new();
my $Project 				= Project_config->new();
my $sufwmix		= sufwmix->new();
...
#	$index[2]	= 8; # outbound item is  bound
	...
	$sufwmix_spec ->{_binding_index_aref} = \@index;
	return();
...
#	$type[$index[2]]	=  $file_dialog_type->{_Data};
	...
	$sufwmix_spec ->{_file_dialog_type_aref} = \@type;
	return();
...
=cut
 ...
 sub flow_type_aref {
	...
	$sufwmix_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($sufwmix_spec->{_binding_index_aref} ) {

		my $index_aref = $sufwmix_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("sufwmix_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($sufwmix_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$sufwmix_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("sufwmix_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($sufwmix_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sufwmix_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("sufwmix_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($sufwmix_spec->{_flow_type_aref} ) {

			my $index_aref = $sufwmix_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("sufwmix_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("sufwmix, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($sufwmix_spec->{_prefix_aref} ) {

		my $prefix_aref= $sufwmix_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("sufwmix_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($sufwmix_spec->{_suffix_aref} ) {

			my $suffix_aref= $sufwmix_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$sufwmix_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
#	$prefix[ $index[2] ] = '$DATA_SEISMIC_SU' . ".'/'.";
	...
	$sufwmix_spec ->{_prefix_aref} = \@prefix;
	return();
...
#	$suffix[ $index[2] ] = ''.'' . '$suffix_su';
	...
	$sufwmix_spec ->{_suffix_aref} = \@suffix;
	return();
