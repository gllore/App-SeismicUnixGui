package App::SeismicUnixGui::specs::statsMath::sualford_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::statsMath::sualford;
my $get      = new L_SU_global_constants();
my $Project  = new Project_config;
my $sualford = new sualford;
...
sub binding_index_aref {
	...
	$index[0] = 3;     # inbound item
	$index[1] = 4;     # inbound item
	$index[2] = 5;     # inbound item
	$index[3] = 6;     # inbound item
	$index[4] = 10;    # outbound item
	$index[5] = 11;    # outbound item
	$index[6] = 12;    # outbound item
	$index[7] = 13;    # outbound item

	$sualford_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$sualford_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$sualford_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $sualford_spec->{_binding_index_aref} ) {

		my $index_aref = $sualford_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("sualford_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $sualford_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $sualford_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("sualford_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $sualford_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sualford_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("sualford_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sualford_spec->{_flow_type_aref} ) {

		my $index_aref = $sualford_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("sualford_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("sualford, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $sualford_spec->{_prefix_aref} ) {

		my $prefix_aref = $sualford_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("sualford_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $sualford_spec->{_suffix_aref} ) {

		my $suffix_aref = $sualford_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$sualford_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 11 in GUI is output yy_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 12 in GUI is output zz_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 13 in GUI is output yy_file and needs a home directory
	...
	$sualford_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 6 in GUI is input xx_file and needs a home directory
	$suffix[ $index[2] ] = '' . '' . '$suffix_bin';

	# label 7 in GUI is input yy_file and needs a home directory
	$suffix[ $index[3] ] = '' . '' . '$suffix_bin';

	# label 11 in GUI is output xx_file and needs a home directory
	$suffix[ $index[4] ] = '' . '' . '$suffix_bin';

	# label 12 in GUI is output yy_file and needs a home directory
	$suffix[ $index[5] ] = '' . '' . '$suffix_bin';

	# label 13 in GUI is output xx_file and needs a home directory
	$suffix[ $index[6] ] = '' . '' . '$suffix_bin';

	# label 14 in GUI is output yy_file and needs a home directory
	$suffix[ $index[7] ] = '' . '' . '$suffix_bin';

	$sualford_spec->{_suffix_aref} = \@suffix;
	return ();
