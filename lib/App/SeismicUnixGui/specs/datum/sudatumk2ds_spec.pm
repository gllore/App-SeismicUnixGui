...
use App::SeismicUnixGui::sunix::datum::sudatumk2ds;
my $get         = new L_SU_global_constants();
my $Project     = new Project_config;
my $sudatumk2ds = new sudatumk2ds;
...
sub binding_index_aref {
	...
	$index[0] = 22;    # inbound item is  bound
	$index[1] = 23;    # outbound item is  bound
	$index[2] = 34;    # outbound item is  bound
	$index[3] = 37;    # inbound item is  bound

	$sudatumk2ds_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$sudatumk2ds_spec->{_file_dialog_type_aref} = \@type;
...
sub flow_type_aref {
	...
	$sudatumk2ds_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $sudatumk2ds_spec->{_binding_index_aref} ) {

		my $index_aref = $sudatumk2ds_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("sudatumk2ds_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $sudatumk2ds_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $sudatumk2ds_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("sudatumk2ds_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $sudatumk2ds_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sudatumk2ds_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("sudatumk2ds_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sudatumk2ds_spec->{_flow_type_aref} ) {

		my $index_aref = $sudatumk2ds_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("sudatumk2ds_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("sudatumk2ds, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $sudatumk2ds_spec->{_prefix_aref} ) {

		my $prefix_aref = $sudatumk2ds_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("sudatumk2ds_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $sudatumk2ds_spec->{_suffix_aref} ) {

		my $suffix_aref = $sudatumk2ds_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$sudatumk2ds_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 24 in GUI is output yy_file and needs a home directory
	$prefix[ $index[1] ] = '$DATA_SEISMIC_TXT' . ".'/'.";

	# label 35 in GUI is output zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 38 in GUI is input zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$sudatumk2ds_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	$sudatumk2ds_spec->{_suffix_aref} = \@suffix;
	return ();
