package App::SeismicUnixGui::specs::model::elatriuni_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::model::elatriuni;
my $get       = new L_SU_global_constants();
my $Project   = new Project_config;
my $elatriuni = new elatriuni;
...
sub binding_index_aref {
	...
	$index[2] = 2;     # outbound item is  bound
	$index[3] = 3;     # outbound item is  bound
	$index[4] = 4;     # outbound item is  bound
	$index[5] = 5;     # outbound item is  bound
	$index[6] = 6;     # outbound item is  bound
	$index[7] = 7;     # outbound item is  bound
	$index[8] = 8;     # outbound item is  bound
	$index[9] = 15;    # outbound item is  bound

	$elatriuni_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$elatriuni_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$elatriuni_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $elatriuni_spec->{_binding_index_aref} ) {

		my $index_aref = $elatriuni_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("elatriuni_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $elatriuni_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $elatriuni_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("elatriuni_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $elatriuni_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $elatriuni_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("elatriuni_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $elatriuni_spec->{_flow_type_aref} ) {

		my $index_aref = $elatriuni_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("elatriuni_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("elatriuni, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $elatriuni_spec->{_prefix_aref} ) {

		my $prefix_aref = $elatriuni_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("elatriuni_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $elatriuni_spec->{_suffix_aref} ) {

		my $suffix_aref = $elatriuni_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$elatriuni_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 3 in GUI is output xx_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 4 in GUI is output xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 5 in GUI is output xx_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 6 in GUI is output xx_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 7 in GUI is output xx_file and needs a home directory
	$prefix[ $index[6] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 8 in GUI is output xx_file and needs a home directory
	$prefix[ $index[7] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 9 in GUI is output xx_file and needs a home directory
	$prefix[ $index[8] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 16 in GUI is output xx_file and needs a home directory
	$prefix[ $index[9] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$elatriuni_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 2 in GUI is input yy_file and needs a home directory
	$suffix[ $index[1] ] = '' . '' . '$suffix_bin';

	# label 3 in GUI is output xx_file and needs a home directory
	$suffix[ $index[2] ] = '' . '' . '$suffix_bin';

	# label 4 in GUI is input yy_file and needs a home directory
	$suffix[ $index[3] ] = '' . '' . '$suffix_bin';

	# label 5 in GUI is output xx_file and needs a home directory
	$suffix[ $index[4] ] = '' . '' . '$suffix_bin';

	# label 6 in GUI is input yy_file and needs a home directory
	...
	# label 16 in GUI is output yy_file and needs a home directory
	$suffix[ $index[9] ] = '' . '' . '$suffix_bin';

	$elatriuni_spec->{_suffix_aref} = \@suffix;
	return ();
