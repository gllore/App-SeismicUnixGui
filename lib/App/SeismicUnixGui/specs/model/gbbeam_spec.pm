package App::SeismicUnixGui::specs::model::gbbeam_spec;
	use Moose;
our $VERSION = '0.0.1';
...
use aliased 'App::SeismicUnixGui::sunix::model::gbbeam';
my $get					= L_SU_global_constants->new();
my $Project 				= Project_config->new();
my $gbbeam		= gbbeam->new();
...
=cut
 ...
 sub binding_index_aref {
	...
	$index[0] = 14; # inbound item is  bound 
	$index[1]	= 15; # inbound item is  bound

	$gbbeam_spec ->{_binding_index_aref} = \@index;
	return();
...
=cut
 ...
 sub file_dialog_type_aref {
	...
	$gbbeam_spec ->{_file_dialog_type_aref} = \@type;
	return();
...
=cut
 ...
 sub flow_type_aref {
	...
	$gbbeam_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($gbbeam_spec->{_binding_index_aref} ) {

		my $index_aref = $gbbeam_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("gbbeam_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($gbbeam_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$gbbeam_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("gbbeam_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($gbbeam_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $gbbeam_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("gbbeam_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($gbbeam_spec->{_flow_type_aref} ) {

			my $index_aref = $gbbeam_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("gbbeam_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("gbbeam, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($gbbeam_spec->{_prefix_aref} ) {

		my $prefix_aref= $gbbeam_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("gbbeam_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($gbbeam_spec->{_suffix_aref} ) {

			my $suffix_aref= $gbbeam_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$gbbeam_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	# label 15 in GUI is input xx_file and needs a home directory
	$prefix[ $index[0] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 16 in GUI is input yy_file and needs a home directory
	$prefix[ $index[1] ] = '$DATA_SEISMIC_BIN' . ".'/'.";


	$gbbeam_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	$gbbeam_spec ->{_suffix_aref} = \@suffix;
	return();
