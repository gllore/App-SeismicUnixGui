...
use App::SeismicUnixGui::sunix::model::grm;
...
sub get_file_dialog_type_aref {
	...
	if ( $grm_spec->{_file_dialog_type_aref} ) {
		...
		print(
"grm_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"grm, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
