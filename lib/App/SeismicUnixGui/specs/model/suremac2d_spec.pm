...
use App::SeismicUnixGui::sunix::model::suremac2d;
my $get       = new L_SU_global_constants();
my $Project   = new Project_config;
my $suremac2d = new suremac2d;
...
sub binding_index_aref {
	...
	$index[1] = 24;    # outbound item is  bound
	$index[2] = 25;    # outbound item is  bound
	$index[3] = 27;    # outbound item is  bound
	$index[4] = 28;    # inbound item is  bound
	$index[5] = 30;    # inbound item is  bound

	$suremac2d_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$suremac2d_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$suremac2d_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $suremac2d_spec->{_binding_index_aref} ) {

		my $index_aref = $suremac2d_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("suremac2d_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $suremac2d_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $suremac2d_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("suremac2d_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $suremac2d_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $suremac2d_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("suremac2d_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $suremac2d_spec->{_flow_type_aref} ) {

		my $index_aref = $suremac2d_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("suremac2d_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("suremac2d, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $suremac2d_spec->{_prefix_aref} ) {

		my $prefix_aref = $suremac2d_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("suremac2d_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $suremac2d_spec->{_suffix_aref} ) {

		my $suffix_aref = $suremac2d_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$suremac2d_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 3 in GUI is input xx_file and needs a home directory
	$prefix[ $index[0] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 25 in GUI is output yy_file and needs a home directory
	$prefix[ $index[1] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 26 in GUI is output zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 28 in GUI is input zz_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 29 in GUI is output zz_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 31 in GUI is input zz_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$suremac2d_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 28 in GUI is input zz_file and needs a home directory
	$suffix[ $index[3] ] = '' . '' . '$suffix_su';

	# label 29 in GUI is output zz_file and needs a home directory
	$suffix[ $index[4] ] = '' . '' . '$suffix_su';

	# label 31 in GUI is input zz_file and needs a home directory
	$suffix[ $index[5] ] = '' . '' . '$suffix_bin';

	$suremac2d_spec->{_suffix_aref} = \@suffix;
	return ();
