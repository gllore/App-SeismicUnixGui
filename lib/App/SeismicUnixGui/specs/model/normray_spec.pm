package App::SeismicUnixGui::specs::model::normray_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::model::normray;
my $get     = new L_SU_global_constants();
my $Project = new Project_config;
my $normray = new normray;
...
sub binding_index_aref {
	...
	$index[0]                            = 6;         # outbound item is  bound
	$index[1]                            = 7;         # outbound item is  bound
	$index[2]                            = 14;        # outbound item is  bound
	$index[3]                            = 18;        # outbound item is  bound
	$index[4]                            = 19;        # inbound item is  bound
	$index[5]                            = 20;        # inbound item is  bound
	$normray_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$normray_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$normray_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $normray_spec->{_binding_index_aref} ) {

		my $index_aref = $normray_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("normray_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $normray_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $normray_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("normray_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $normray_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $normray_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("normray_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $normray_spec->{_flow_type_aref} ) {

		my $index_aref = $normray_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("normray_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("normray, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $normray_spec->{_prefix_aref} ) {

		my $prefix_aref = $normray_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("normray_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $normray_spec->{_suffix_aref} ) {

		my $suffix_aref = $normray_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$normray_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 8 in GUI is output yy_file and needs a home directory
	$prefix[ $index[1] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 15 in GUI is output zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_TXT' . ".'/'.";

	# label 19 in GUI is output xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 20 in GUI is input yy_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 21 in GUI is input zz_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$normray_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 20 in GUI is output yy_file and needs a home directory
	...
	$normray_spec->{_suffix_aref} = \@suffix;
	return ();
