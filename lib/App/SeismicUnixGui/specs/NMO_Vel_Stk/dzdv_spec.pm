package App::SeismicUnixGui::specs::NMO_Vel_Stk::dzdv_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::NMO_Vel_Stk::dzdv;
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $dzdv_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $dzdv_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"dzdv_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $dzdv_spec->{_flow_type_aref} ) {

		my $index_aref = $dzdv_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("dzdv_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"dzdv, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $dzdv_spec->{_prefix_aref} ) {

		my $prefix_aref = $dzdv_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("dzdv_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $dzdv_spec->{_suffix_aref} ) {

		my $suffix_aref = $dzdv_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$dzdv_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
