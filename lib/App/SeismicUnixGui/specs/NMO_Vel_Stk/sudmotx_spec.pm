package App::SeismicUnixGui::specs::NMO_Vel_Stk::sudmotx_spec;
use Moose;
our $VERSION = '0.0.1';
...
use aliased 'App::SeismicUnixGui::sunix::NMO_Vel_Stk::sudmotx';
my $get     = L_SU_global_constants->new();
my $Project = Project_config->new();
my $sudmotx = sudmotx->new();
...
sub get_file_dialog_type_aref {
	...
	if ( $sudmotx_spec->{_file_dialog_type_aref} ) {
		...
		print(
"sudmotx_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sudmotx, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
