package App::SeismicUnixGui::specs::NMO_Vel_Stk::sudmofk_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::NMO_Vel_Stk::sudmofk;
...
sub get_file_dialog_type_aref {
	...
	if ( $sudmofk_spec->{_file_dialog_type_aref} ) {
		...
		print(
"sudmofk_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sudmofk, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
