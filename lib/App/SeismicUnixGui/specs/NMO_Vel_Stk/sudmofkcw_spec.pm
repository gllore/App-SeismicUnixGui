...
use App::SeismicUnixGui::sunix::NMO_Vel_Stk::sudmofkcw;
...
sub get_file_dialog_type_aref {
	...
	if ( $sudmofkcw_spec->{_file_dialog_type_aref} ) {
		...
		print(
"sudmofkcw_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sudmofkcw, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
