...
use App::SeismicUnixGui::sunix::NMO_Vel_Stk::sucvs4fowler;
...
sub get_file_dialog_type_aref {
	...
	if ( $sucvs4fowler_spec->{_file_dialog_type_aref} ) {
		...
		print(
"sucvs4fowler_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sucvs4fowler_spec->{_flow_type_aref} ) {
		...
		print(
			"sucvs4fowler_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sucvs4fowler, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
