...
=cut
 ...
 sub binding_index_aref {
	...
	$index[3] = 9; # inbound/outbound item is bound
	$index[4] = 12; # inbound/outbound item is bound
	$index[5] = 15; # inbound/outbound item is bound
...
=cut
 ...
 sub flow_type_aref {
	...
	$sumute_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($sumute_spec->{_binding_index_aref} ) {

		my $index_aref = $sumute_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("sumute_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($sumute_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$sumute_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("sumute_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($sumute_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sumute_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("sumute_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($sumute_spec->{_flow_type_aref} ) {

			my $index_aref = $sumute_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("sumute_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("sumute, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($sumute_spec->{_prefix_aref} ) {

		my $prefix_aref= $sumute_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("sumute_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($sumute_spec->{_suffix_aref} ) {

			my $suffix_aref= $sumute_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$sumute_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	$prefix[ $index[4] ] =  '$PL_SEISMIC' . ".'/'.";
 
	# label 16 in GUI is input/output xx_file and needs a home directory
	$prefix[ $index[5] ] =  '$PL_SEISMIC' . ".'/'.";

	$sumute_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	# label 16 in GUI is input/ouput xx_file and needs a home directory
	$suffix[ $index[5] ] =  ''.'' . '';

 
	$sumute_spec ->{_suffix_aref} = \@suffix;
	return();
