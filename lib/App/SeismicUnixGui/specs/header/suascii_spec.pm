package App::SeismicUnixGui::specs::header::suascii_spec;
	use Moose;
our $VERSION = '0.0.1';
...
use aliased 'App::SeismicUnixGui::sunix::header::suascii';
my $get					= L_SU_global_constants->new();
my $Project 				= Project_config->new();
my $suascii		= suascii->new();
...
=cut
 ...
 sub binding_index_aref {
	...
	$suascii_spec ->{_binding_index_aref} = \@index;
	return();
...
=cut
 ...
 sub flow_type_aref {
	...
	$suascii_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($suascii_spec->{_binding_index_aref} ) {

		my $index_aref = $suascii_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("suascii_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($suascii_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$suascii_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("suascii_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($suascii_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $suascii_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("suascii_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($suascii_spec->{_flow_type_aref} ) {

			my $index_aref = $suascii_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("suascii_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("suascii, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($suascii_spec->{_prefix_aref} ) {

		my $prefix_aref= $suascii_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("suascii_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($suascii_spec->{_suffix_aref} ) {

			my $suffix_aref= $suascii_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$suascii_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
