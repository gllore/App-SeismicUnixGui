package App::SeismicUnixGui::specs::header::sustaticrrs_spec;
	use Moose;
our $VERSION = '0.0.1';
...
use aliased 'App::SeismicUnixGui::sunix::header::sustaticrrs';
my $get					= L_SU_global_constants->new();
my $Project 				= Project_config->new();
my $sustaticrrs		= sustaticrrs->new();
...
=cut
 ...
 sub binding_index_aref {
	...
	$index[1]	= 12; # inbound item is  bound
	$index[2]	= 13; # outbound item is  bound
	$index[3]	= 15; # outbound item is  bound
	$index[4]	= 18; # outbound item is  bound
	
	$sustaticrrs_spec ->{_binding_index_aref} = \@index;
	return();
...
#	$type[0]	= '';
	...
	$sustaticrrs_spec ->{_file_dialog_type_aref} = \@type;
	return();
...
=cut
 ...
 sub flow_type_aref {
	...
	$sustaticrrs_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($sustaticrrs_spec->{_binding_index_aref} ) {

		my $index_aref = $sustaticrrs_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("sustaticrrs_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($sustaticrrs_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$sustaticrrs_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("sustaticrrs_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($sustaticrrs_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sustaticrrs_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("sustaticrrs_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($sustaticrrs_spec->{_flow_type_aref} ) {

			my $index_aref = $sustaticrrs_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("sustaticrrs_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("sustaticrrs, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($sustaticrrs_spec->{_prefix_aref} ) {

		my $prefix_aref= $sustaticrrs_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("sustaticrrs_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($sustaticrrs_spec->{_suffix_aref} ) {

			my $suffix_aref= $sustaticrrs_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$sustaticrrs_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	# label 13 in GUI is input yy_file and needs a home directory
	...
	# label 16 in GUI is input xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";
	
	# label 19 in GUI is input xx_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$sustaticrrs_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	$sustaticrrs_spec ->{_suffix_aref} = \@suffix;
	return();
