...
use aliased 'App::SeismicUnixGui::sunix::filter::supolar';
my $get					= L_SU_global_constants->new();
my $Project 				= Project_config->new();
my $supolar		= supolar->new();
...
my $flow_type				= $get->flow_type_href();
  ...
  my $PL_SEISMIC		    = $Project->PL_SEISMIC();
	my $max_index           = 17;# Insert a number here
...
=cut
 ...
 sub binding_index_aref {
	...
	$supolar_spec ->{_binding_index_aref} = \@index;
	return();
...
=cut
 ...
 sub file_dialog_type_aref {
	...
	$supolar_spec ->{_file_dialog_type_aref} = \@type;
	return();
...
=cut
 ...
 sub flow_type_aref {
	...
	$supolar_spec ->{_flow_type_aref} = \@type;
	return();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($supolar_spec->{_binding_index_aref} ) {

		my $index_aref = $supolar_spec->{_binding_index_aref};
		return($index_aref);

	} else {
		print("supolar_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($supolar_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$supolar_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("supolar_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($supolar_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $supolar_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("supolar_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($supolar_spec->{_flow_type_aref} ) {

			my $index_aref = $supolar_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("supolar_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("supolar, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($supolar_spec->{_prefix_aref} ) {

		my $prefix_aref= $supolar_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("supolar_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($supolar_spec->{_suffix_aref} ) {

			my $suffix_aref= $supolar_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$supolar_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	$supolar_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	$supolar_spec ->{_suffix_aref} = \@suffix;
	return();
