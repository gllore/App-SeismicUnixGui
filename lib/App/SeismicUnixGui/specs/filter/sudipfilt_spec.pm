package App::SeismicUnixGui::specs::filter::sudipfilt_spec;
use Moose;
our $VERSION = '0.0.1';
...
my $max_index       = $sudipfilt->get_max_index();
...
sub get_binding_length {
    ...
    if ( $sudipfilt_spec->{_binding_index_aref} ) {
        ...
        my $binding_length = scalar @{ $sudipfilt_spec->{_binding_index_aref} };
        return ($binding_length);

    }
    else {
        print("sudipfilt_spec, get_binding_length, missing binding_length\n");
        return ();
    }
...
sub get_file_dialog_type_aref {
    ...
    if ( $sudipfilt_spec->{_file_dialog_type_aref} ) {
        ...
        my $index_aref = $sudipfilt_spec->{_file_dialog_type_aref};
        return ($index_aref);

    }
    else {
        print(
"sudipfilt_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
        );
        return ();
    }
...
sub get_flow_type_aref {
    ...
    if ( $sudipfilt_spec->{_flow_type_aref} ) {
        ...
        my $index_aref = $sudipfilt_spec->{_flow_type_aref};
        return ($index_aref);
...
sub get_incompatibles {
    ...
    if ( $len_1_needed >= 1 ) {
        ...
        for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
            ...
            print(
"sudipfilt, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
            );
...
sub get_prefix_aref {
    ...
    if ( $sudipfilt_spec->{_prefix_aref} ) {
