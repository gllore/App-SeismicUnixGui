package App::SeismicUnixGui::specs::migration::sukdmig2d_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::migration::sukdmig2d;
my $get       = new L_SU_global_constants();
my $Project   = new Project_config;
my $sukdmig2d = new sukdmig2d;
...
sub binding_index_aref {
	...
	$index[1] = 20;    # inbound item is  bound
	$index[2] = 37;    # outbound item is  bound
	$index[3] = 38;    # outbound item is  bound
	$index[4] = 40;    # inbound item is  bound
	$index[5] = 41;    # inbound item is  bound

	$sukdmig2d_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$sukdmig2d_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$sukdmig2d_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $sukdmig2d_spec->{_binding_index_aref} ) {

		my $index_aref = $sukdmig2d_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"sukdmig2d_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $sukdmig2d_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $sukdmig2d_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("sukdmig2d_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $sukdmig2d_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sukdmig2d_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"sukdmig2d_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sukdmig2d_spec->{_flow_type_aref} ) {

		my $index_aref = $sukdmig2d_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("sukdmig2d_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sukdmig2d, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $sukdmig2d_spec->{_prefix_aref} ) {

		my $prefix_aref = $sukdmig2d_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("sukdmig2d_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $sukdmig2d_spec->{_suffix_aref} ) {

		my $suffix_aref = $sukdmig2d_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$sukdmig2d_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 21 in GUI is input yy_file and needs a home directory
	$prefix[ $index[1] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 38 in GUI is output zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_SU' . ".'/'.";

	# label 39 in GUI is output xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 41 in GUI is input yy_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 42 in GUI is input zz_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$sukdmig2d_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	$sukdmig2d_spec->{_suffix_aref} = \@suffix;
	return ();
