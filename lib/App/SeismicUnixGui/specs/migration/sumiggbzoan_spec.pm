package App::SeismicUnixGui::specs::migration::sumiggbzoan_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::migration::sumiggbzoan;
my $get         = new L_SU_global_constants();
my $Project     = new Project_config;
my $sumiggbzoan = new sumiggbzoan;
...
sub binding_index_aref {
	...
	$index[1] = 1;     # inbound item is  bound
	$index[2] = 2;     # outbound item is  bound
	$index[3] = 3;     # inbound item is  bound
	$index[4] = 4;     # inbound item is  bound
	$index[5] = 5;     # outbound item is  bound
	$index[6] = 9;     # inbound item is  bound
	$index[7] = 13;    # outbound item is  bound
	$index[8] = 19;    # outbound item is  bound

	$sumiggbzoan_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$sumiggbzoan_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$sumiggbzoan_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $sumiggbzoan_spec->{_binding_index_aref} ) {

		my $index_aref = $sumiggbzoan_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"sumiggbzoan_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $sumiggbzoan_spec->{_binding_index_aref} ) {
		...
		my $binding_length =
		  scalar @{ $sumiggbzoan_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("sumiggbzoan_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $sumiggbzoan_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sumiggbzoan_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"sumiggbzoan_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $sumiggbzoan_spec->{_flow_type_aref} ) {

		my $index_aref = $sumiggbzoan_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("sumiggbzoan_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"sumiggbzoan, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $sumiggbzoan_spec->{_prefix_aref} ) {

		my $prefix_aref = $sumiggbzoan_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("sumiggbzoan_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $sumiggbzoan_spec->{_suffix_aref} ) {

		my $suffix_aref = $sumiggbzoan_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$sumiggbzoan_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 3 in GUI is input zz_file and needs a home directory
	...
	# label 14 in GUI is input yy_file and needs a home directory
	$prefix[ $index[7] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 19 in GUI is input zz_file and needs a home directory
	$prefix[ $index[8] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$sumiggbzoan_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 3 in GUI is output zz_file and needs a home directory
	...
	# label 6 in GUI is output zz_file and needs a home directory
	...
	# label 20 in GUI is output zz_file and needs a home directory
	$suffix[ $index[8] ] = '' . '' . '$suffix_bin';

	$sumiggbzoan_spec->{_suffix_aref} = \@suffix;
	return ();
