package App::SeismicUnixGui::specs::plot::xmovie_spec;
use Moose;
our $VERSION = '0.0.1';

use App::SeismicUnixGui::configs::big_streams::Project_config;
use App::SeismicUnixGui::misc::SeismicUnix qw ($su $bin $suffix_bin $suffix_su);
use App::SeismicUnixGui::misc::L_SU_global_constants;
use App::SeismicUnixGui::sunix::plot::xmovie;
...
sub binding_index_aref {
	...
	$xmovie_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$xmovie_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$xmovie_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $xmovie_spec->{_binding_index_aref} ) {
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $xmovie_spec->{_file_dialog_type_aref} ) {
...
sub get_flow_type_aref {
	...
	if ( $xmovie_spec->{_flow_type_aref} ) {
...
sub prefix_aref {
	...
	$xmovie_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	$xmovie_spec->{_suffix_aref} = \@suffix;
	return ();
