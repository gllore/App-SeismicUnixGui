package App::SeismicUnixGui::specs::inversion::suinvco3d_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::inversion::suinvco3d;
my $get       = new L_SU_global_constants();
my $Project   = new Project_config;
my $suinvco3d = new suinvco3d;
...
sub binding_index_aref {
	...
	$index[2]                              = 6;        # outbound item is  bound
	$index[3]                              = 7;        # outbound item is  bound
	$index[4]                              = 8;        # outbound item is  bound
	$index[5]                              = 9;        # outbound item is  bound
	$index[6]                              = 10;       # outbound item is  bound
	$index[7]                              = 11;       # outbound item is  bound
	$index[8]                              = 47;       # outbound item is  bound
	$index[9]                              = 49;       # outbound item is  bound
	$suinvco3d_spec->{_binding_index_aref} = \@index;
	return ();
...
sub file_dialog_type_aref {
	...
	$suinvco3d_spec->{_file_dialog_type_aref} = \@type;
	return ();
...
sub flow_type_aref {
	...
	$suinvco3d_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $suinvco3d_spec->{_binding_index_aref} ) {

		my $index_aref = $suinvco3d_spec->{_binding_index_aref};
		return ($index_aref);

	}
	else {
		print(
"suinvco3d_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $suinvco3d_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $suinvco3d_spec->{_binding_index_aref} };
		return ($binding_length);

	}
	else {
		print("suinvco3d_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $suinvco3d_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $suinvco3d_spec->{_file_dialog_type_aref};
		return ($index_aref);

	}
	else {
		print(
"suinvco3d_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $suinvco3d_spec->{_flow_type_aref} ) {

		my $index_aref = $suinvco3d_spec->{_flow_type_aref};
		return ($index_aref);

	}
	else {
		print("suinvco3d_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"suinvco3d, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
...
sub get_prefix_aref {
	...
	if ( $suinvco3d_spec->{_prefix_aref} ) {

		my $prefix_aref = $suinvco3d_spec->{_prefix_aref};
		return ($prefix_aref);

	}
	else {
		print("suinvco3d_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $suinvco3d_spec->{_suffix_aref} ) {

		my $suffix_aref = $suinvco3d_spec->{_suffix_aref};
		return ($suffix_aref);

	}
	else {
		print("$suinvco3d_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 7 in GUI is input zz_file and needs a home directory
	$prefix[ $index[2] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 8 in GUI is input xx_file and needs a home directory
	$prefix[ $index[3] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 9 in GUI is input yy_file and needs a home directory
	$prefix[ $index[4] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 10 in GUI is input zz_file and needs a home directory
	$prefix[ $index[5] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 11 in GUI is input xx_file and needs a home directory
	...
	# label 48 in GUI is input zz_file and needs a home directory
	$prefix[ $index[8] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	# label 50 in GUI is input xx_file and needs a home directory
	$prefix[ $index[9] ] = '$DATA_SEISMIC_BIN' . ".'/'.";

	$suinvco3d_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	# label 7 in GUI is output zz_file and needs a home directory
	...
	# label 10 in GUI is output zz_file and needs a home directory
	...
	# label 48 in GUI is output zz_file and needs a home directory
	$suffix[ $index[8] ] = '' . '' . '$suffix_bin';

	# label 50 in GUI is input xx_file and needs a home directory
	$suffix[ $index[9] ] = '' . '' . '$suffix_bin';

	$suinvco3d_spec->{_suffix_aref} = \@suffix;
	return ();
