package App::SeismicUnixGui::specs::data::segbread_spec;
use Moose;
our $VERSION = '0.0.1';
...
use App::SeismicUnixGui::sunix::data::segbread;
my $get      = new L_SU_global_constants();
my $Project  = new Project_config;
my $segbread = new segbread;
...
my $segbread_spec = {
	_CONFIG                => $PL_SEISMIC,
	_DATA_DIR_IN           => $DATA_SEISMIC_SEGB,
...
sub binding_index_aref {
	...
	$index[0] = 3;    # inbound item is  bound to _DATA_DIR_IN

	$segbread_spec->{_binding_index_aref} = \@index;
	return ();
...
sub flow_type_aref {
	...
	$segbread_spec->{_flow_type_aref} = \@type;
	return ();
...
sub get_binding_index_aref {
	...
	if ( $segbread_spec->{_binding_index_aref} ) {

		my $index_aref = $segbread_spec->{_binding_index_aref};
		return ($index_aref);

	} else {
		print("segbread_spec, get_binding_index_aref, missing binding_index_aref\n");
		return ();
	}
...
sub get_binding_length {
	...
	if ( $segbread_spec->{_binding_index_aref} ) {
		...
		my $binding_length = scalar @{ $segbread_spec->{_binding_index_aref} };
		return ($binding_length);

	} else {
		print("segbread_spec, get_binding_length, missing binding_length\n");
		return ();
	}
...
sub get_file_dialog_type_aref {
	...
	my $self = @_;
	if ( $segbread_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $segbread_spec->{_file_dialog_type_aref};
		return ($index_aref);

	} else {
		print("segbread_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return ();
	}
...
sub get_flow_type_aref {
	...
	if ( $segbread_spec->{_flow_type_aref} ) {

		my $index_aref = $segbread_spec->{_flow_type_aref};
		return ($index_aref);

	} else {
		print("segbread_spec, get_flow_type_aref, missing flow_type_aref\n");
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0; $i < $len_1_needed; $i++ ) {
			...
			print("segbread, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
sub get_prefix_aref {
	...
	if ( $segbread_spec->{_prefix_aref} ) {

		my $prefix_aref = $segbread_spec->{_prefix_aref};
		return ($prefix_aref);

	} else {
		print("segbread_spec, get_prefix_aref, missing prefix_aref\n");
		return ();
	}
...
sub get_suffix_aref {
	...
	if ( $segbread_spec->{_suffix_aref} ) {

		my $suffix_aref = $segbread_spec->{_suffix_aref};
		return ($suffix_aref);

	} else {
		print("$segbread_spec, get_suffix_aref, missing suffix_aref\n");
		return ();
	}
...
sub prefix_aref {
	...
	# label 4 in GUI is input segb file and needs a home directory
	$prefix[ $index[0] ] =  '$DATA_SEISMIC_SEGB' . ".'/'.";

	$segbread_spec->{_prefix_aref} = \@prefix;
	return ();
...
sub suffix_aref {
	...
	$suffix[ $index[0] ] = '' . '' . '$suffix_segb';

	$segbread_spec->{_suffix_aref} = \@suffix;
	return ();
