...
use App::SeismicUnixGui::sunix::data::wpc1uncomp2;
...
sub get_binding_index_aref {
	...
	if ( $wpc1uncomp2_spec->{_binding_index_aref} ) {
		...
		print(
"wpc1uncomp2_spec, get_binding_index_aref, missing binding_index_aref\n"
		);
		return ();
	}
...
sub get_binding_length {
	...
	if ( $wpc1uncomp2_spec->{_binding_index_aref} ) {
		...
		my $binding_length =
		  scalar @{ $wpc1uncomp2_spec->{_binding_index_aref} };
...
sub get_file_dialog_type_aref {
	...
	if ( $wpc1uncomp2_spec->{_file_dialog_type_aref} ) {
		...
		print(
"wpc1uncomp2_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n"
		);
		return ();
	}
...
sub get_incompatibles {
	...
	if ( $len_1_needed >= 1 ) {
		...
		for ( my $i = 0 ; $i < $len_1_needed ; $i++ ) {
			...
			print(
"wpc1uncomp2, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n"
			);
