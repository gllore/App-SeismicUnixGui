 ...
 sub _get_list_aref {
	...
	if ( $prog_doc2pm->{_group_directory} ne $empty_string ) {
		...
		while (my $thing = readdir $dh) {
    		...
    		if ($thing ne '.' or $thing ne '..') { # tod? does not filter these 2 out
    			
        		$file_name[$count] = $thing;
        		# print("prog_doc2pm,_get_list_aref,file:$file_name[$count]\n");
        		$count++;
        		    		
    		} else {
    			print("prog_doc2pm,_get_list_aref,skipping thing\n");
    		}
		}
		closedir $dh;
...
# 		# my $path 	= '/usr/local/cwp_su_all_48/src/doc/Stripped';
#
#		my $path_out 			= '/usr/local/pl/developer';
#		print("prog_doc2pm,_get_path_out = $path_out\n");		
#		return($path_out);
#
#	} else { 
#		print("prog_doc2pm, _get_path_out, missing directory,\n");
#		return();
#	}	
# }
	...
	if ( $prog_doc2pm->{_group_directory} ne $empty_string ) {
		...
		while (my $thing = readdir $dh) {
    		...
    		if ($thing ne '.' or $thing ne '..') { # tod? does not filter these 2 out
        		...
        		$file_name[$count] = $thing;
        		# print("prog_doc2pm,get_list_aref,file:$file_name[$count]\n");
        		$count++;
	...
	if ( $prog_doc2pm->{_group_directory} ne $empty_string ) {
		...
		while (my $thing = readdir $dh) {
    		...
    		if ($thing ne '.' or $thing ne '..') { # tod? does not filter these 2 out
    			...
    			my @fields 	= split (/\./,$thing);
        		$file_name[$count] = $fields[0];
        		# print("prog_doc2pm,get_program_aref,file:$file_name[$count]\n");
        		$count++;
        		    		
    		} else {
    			print("prog_doc2pm,_get_program_aref,skipping thing\n");
    		}
		}
		closedir $dh;
