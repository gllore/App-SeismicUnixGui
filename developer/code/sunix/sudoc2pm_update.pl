...
=cut
	...
	#	print("sudoc2pm_update.pl,num_lines= $length_of_slurp\n");
...
=cut
	...
	if (   $global_constant_success
		...
		for ( my $i = 0; $i < $length_line_terminator_success; $i++ ) {
			...
			#			print("sudoc2pm_update, differences = $differences[$i]\n");
		}
		...
		for ( my $i = 0; $i < $length_line_terminator_success; $i++ ) {
			...
			if ( $differences[$i] >= 0 ) {
				...
				if ( $differences[$i] < $minimum ) {
					...
					#					print("sudoc2pm_update, minimum = $differences[$i]\n");
					#					print("sudoc2pm_update, index_of_minimum = $index_of_minimum\n");
		...
		#		print("sudoc2pm_update,  first_index_of_item= $first_index_of_item\n");
		#		print("sudoc2pm_update,  last_index_of_item= $last_index_of_item\n");

		#		for (my $i=$first_index_of_item; $i <= $last_index_of_item; $i++) {
		#			print("sudoc2pm_update,  sought-after lines= $slurp[$i]\n");
		#		}
		...
		#		print("sudoc2pm_update,  program_list= @program_list\n");
	...
	for ( my $i = 0; $i < $length; $i++ ) {
		...
		#   		print("sudoc2pm_update,  item #$i in program_list=....$program_list[$i]\n");

	}

	# add a program name to the end
	push @program_list, $program_name;
	print("2. sudoc2pm_update,  program_list= @program_list\n");

	# sort alphabetically
	@program_list = sort(@program_list);
	print("3. sudoc2pm_update,  program_list= @program_list\n");
	...
	for ( my $i = 0; $i < $length; $i++ ) {
		...
		#remove white space and first "
		$program_list[$i] =~ s/\s"//;
		print("4. sudoc2pm_update,  item #$i in program_list=....$program_list[$i]\n");
	...
	print("5 sudoc2pm_update,  program_list=@program_list\n");
...
#	print("5.1 sudoc2pm_update,  slurp_b4_extraction: @slurp_b4_extraction\n");

	# add torso to head
	push @slurp_b4_extraction, @new_program_list;
	my @digested_slurp = @slurp_b4_extraction;
#	print("5.2 sudoc2pm_update,  digested_slurp: @digested_slurp\n");

	# add tail to (torso+head)
	push @digested_slurp, @slurp_after_extraction;

	#	print("5.3 sudoc2pm_update,  digested_slurp: @digested_slurp\n");

	#	Write out the corrected file
#	print("writing out to  $global_constants_outbound[1] n");
	my $length_digested_slurp = scalar @digested_slurp;

	open( OUT, ">$global_constants_outbound[1] " ) or die("File  $global_constants_outbound[1] not found");
	for ( my $i = 0; $i < ($length_digested_slurp); $i++ ) {

		if ( $digested_slurp[$i] ne "\t" ) {
#			print("$digested_slurp[$i]\n");
			print OUT $slurp[$i] . "\n";
		}
