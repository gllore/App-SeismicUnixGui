...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SMOOTHINT2 --- SMOOTH non-uniformly sampled INTerfaces, via the damped
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub tfile {
	...
	if ( $tfile ne $empty_string ) {
		...
		$smoothint2->{_tfile}		= $tfile;
		$smoothint2->{_note}		= $smoothint2->{_note}.' tfile='.$smoothint2->{_tfile};
		$smoothint2->{_Step}		= $smoothint2->{_Step}.' tfile='.$smoothint2->{_tfile};

	} else { 
		print("smoothint2, tfile, missing tfile,\n");
	 }
 }
