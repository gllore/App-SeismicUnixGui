...
=cut
 ...
 sub get_binding_length{
	...
	if ($b2a_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$b2a_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("b2a_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	if ($b2a_spec->{_file_dialog_type_aref} ) {
		...
		my $index_aref = $b2a_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("b2a_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($b2a_spec->{_flow_type_aref} ) {
			...
			my $index_aref = $b2a_spec->{_flow_type_aref};
			return($index_aref);
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("b2a, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
