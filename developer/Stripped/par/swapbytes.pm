...
=cut
 ...
 sub in {
	...
	if ( $in ne $empty_string ) {
		...
		$swapbytes->{_in}		= $in;
		$swapbytes->{_note}		= $swapbytes->{_note}.' in='.$swapbytes->{_in};
		$swapbytes->{_Step}		= $swapbytes->{_Step}.' in='.$swapbytes->{_in};
...
=cut
 ...
 sub n1 {
	...
	if ( $n1 ne $empty_string ) {
		...
		$swapbytes->{_n1}		= $n1;
		$swapbytes->{_note}		= $swapbytes->{_note}.' n1='.$swapbytes->{_n1};
		$swapbytes->{_Step}		= $swapbytes->{_Step}.' n1='.$swapbytes->{_n1};

	} else { 
		print("swapbytes, n1, missing n1,\n");
	 }
 }
...
=cut
 ...
 sub outpar {
	...
	if ( $outpar ne $empty_string ) {
		...
		$swapbytes->{_note}		= $swapbytes->{_note}.' outpar='.$swapbytes->{_outpar};
		$swapbytes->{_Step}		= $swapbytes->{_Step}.' outpar='.$swapbytes->{_outpar};
