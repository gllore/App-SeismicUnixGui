...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  RAYT2DAN -- P- and SV-wave raytracing in 2D anisotropic media		
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub parameterfiles {
	...
	my ( $self,$parameterfiles )		= @_;
	if ( $parameterfiles ne $empty_string ) {

		$rayt2dan->{_parameterfiles}		= $parameterfiles;
		$rayt2dan->{_note}		= $rayt2dan->{_note}.' parameterfiles='.$rayt2dan->{_parameterfiles};
		$rayt2dan->{_Step}		= $rayt2dan->{_Step}.' parameterfiles='.$rayt2dan->{_parameterfiles};

	} else { 
		print("rayt2dan, parameterfiles, missing parameterfiles,\n");
	 }
 }
