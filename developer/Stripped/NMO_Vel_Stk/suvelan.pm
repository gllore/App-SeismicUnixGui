...
=cut
 ...
 sub 2 {
	...
	if ( $2 ne $empty_string ) {
		...
		$suvelan->{_2}		= $2;
		$suvelan->{_note}		= $suvelan->{_note}.' 2='.$suvelan->{_2};
		$suvelan->{_Step}		= $suvelan->{_Step}.' 2='.$suvelan->{_2};

	} else { 
		print("suvelan, 2, missing 2,\n");
	 }
 }
...
=cut
 ...
 sub dtratio {
	...
	if ( $dtratio ne $empty_string ) {
		...
		$suvelan->{_note}		= $suvelan->{_note}.' dtratio='.$suvelan->{_dtratio};
		$suvelan->{_Step}		= $suvelan->{_Step}.' dtratio='.$suvelan->{_dtratio};
...
=cut
 ...
 sub j {
	...
	if ( $j ne $empty_string ) {
		...
		$suvelan->{_j}		= $j;
		$suvelan->{_note}		= $suvelan->{_note}.' j='.$suvelan->{_j};
		$suvelan->{_Step}		= $suvelan->{_Step}.' j='.$suvelan->{_j};

	} else { 
		print("suvelan, j, missing j,\n");
	 }
 }
...
=cut
 ...
 sub nsmooth {
	...
	if ( $nsmooth ne $empty_string ) {
		...
		$suvelan->{_note}		= $suvelan->{_note}.' nsmooth='.$suvelan->{_nsmooth};
		$suvelan->{_Step}		= $suvelan->{_Step}.' nsmooth='.$suvelan->{_nsmooth};
...
=cut
 ...
 sub verbose {
	...
	if ( $verbose ne $empty_string ) {
		...
		$suvelan->{_note}		= $suvelan->{_note}.' verbose='.$suvelan->{_verbose};
		$suvelan->{_Step}		= $suvelan->{_Step}.' verbose='.$suvelan->{_verbose};
