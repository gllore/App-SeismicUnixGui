...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SUSTACK - stack adjacent traces having the same key header word
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub normpow {
	...
	if ( $normpow ne $empty_string ) {
		...
		$sustack->{_note}		= $sustack->{_note}.' normpow='.$sustack->{_normpow};
		$sustack->{_Step}		= $sustack->{_Step}.' normpow='.$sustack->{_normpow};
...
=cut
 ...
 sub verbose {
	...
	if ( $verbose ne $empty_string ) {
		...
		$sustack->{_note}		= $sustack->{_note}.' verbose='.$sustack->{_verbose};
		$sustack->{_Step}		= $sustack->{_Step}.' verbose='.$sustack->{_verbose};
