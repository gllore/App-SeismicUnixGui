...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SUSTATICRRS - Elevation STATIC corrections, apply corrections from	
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub gstat {
	...
	if ( $gstat ne $empty_string ) {
		...
		$sustaticrrs->{_gstat}		= $gstat;
		$sustaticrrs->{_note}		= $sustaticrrs->{_note}.' gstat='.$sustaticrrs->{_gstat};
		$sustaticrrs->{_Step}		= $sustaticrrs->{_Step}.' gstat='.$sustaticrrs->{_gstat};

	} else { 
		print("sustaticrrs, gstat, missing gstat,\n");
	 }
 }
...
=cut
 ...
 sub sstat {
	...
	if ( $sstat ne $empty_string ) {
		...
		$sustaticrrs->{_sstat}		= $sstat;
		$sustaticrrs->{_note}		= $sustaticrrs->{_note}.' sstat='.$sustaticrrs->{_sstat};
		$sustaticrrs->{_Step}		= $sustaticrrs->{_Step}.' sstat='.$sustaticrrs->{_sstat};

	} else { 
		print("sustaticrrs, sstat, missing sstat,\n");
	 }
 }
...
=cut
 ...
 sub tstat {
	...
	if ( $tstat ne $empty_string ) {
		...
		$sustaticrrs->{_tstat}		= $tstat;
		$sustaticrrs->{_note}		= $sustaticrrs->{_note}.' tstat='.$sustaticrrs->{_tstat};
		$sustaticrrs->{_Step}		= $sustaticrrs->{_Step}.' tstat='.$sustaticrrs->{_tstat};

	} else { 
		print("sustaticrrs, tstat, missing tstat,\n");
	 }
 }
