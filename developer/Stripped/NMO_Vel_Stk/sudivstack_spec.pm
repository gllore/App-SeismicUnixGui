package sudivstack_spec;
	...
	my $max_index           = $sudivstack->get_max_index();
...
=cut
 ...
 sub get_binding_index_aref{
	...
	if ($sudivstack_spec->{_binding_index_aref} ) {
		...
		print("sudivstack_spec, get_binding_index_aref, missing binding_index_aref\n");
		return();
	}
...
=cut
 ...
 sub get_binding_length{
	...
	if ($sudivstack_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$sudivstack_spec->{_binding_index_aref}};
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	my $self 	= @_;
	if ($sudivstack_spec->{_file_dialog_type_aref} ) {

		my $index_aref = $sudivstack_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("sudivstack_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($sudivstack_spec->{_flow_type_aref} ) {

			my $index_aref = $sudivstack_spec->{_flow_type_aref};
			return($index_aref);

	} else {
		print("sudivstack_spec, get_flow_type_aref, missing flow_type_aref\n");
		return();
	}
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("sudivstack, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($sudivstack_spec->{_prefix_aref} ) {

		my $prefix_aref= $sudivstack_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("sudivstack_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($sudivstack_spec->{_suffix_aref} ) {

			my $suffix_aref= $sudivstack_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$sudivstack_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	$sudivstack_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	$sudivstack_spec ->{_suffix_aref} = \@suffix;
	return();
