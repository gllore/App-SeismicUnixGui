...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SUSTATICB - Elevation static corrections, apply corrections from	
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub a {
	...
	if ( $a ne $empty_string ) {
		...
		$sustaticB->{_a}		= $a;
		$sustaticB->{_note}		= $sustaticB->{_note}.' a='.$sustaticB->{_a};
		$sustaticB->{_Step}		= $sustaticB->{_Step}.' a='.$sustaticB->{_a};

	} else { 
		print("sustaticB, a, missing a,\n");
	 }
 }
...
=cut
 ...
 sub gstat {
	...
	my ( $self,$gstat )		= @_;
	if ( $gstat ne $empty_string ) {

		$sustaticB->{_gstat}		= $gstat;
		$sustaticB->{_note}		= $sustaticB->{_note}.' gstat='.$sustaticB->{_gstat};
		$sustaticB->{_Step}		= $sustaticB->{_Step}.' gstat='.$sustaticB->{_gstat};

	} else { 
		print("sustaticB, gstat, missing gstat,\n");
	 }
 }
...
=cut
 ...
 sub key1 {
	...
	if ( $key1 ne $empty_string ) {
		...
		$sustaticB->{_key1}		= $key1;
		$sustaticB->{_note}		= $sustaticB->{_note}.' key1='.$sustaticB->{_key1};
		$sustaticB->{_Step}		= $sustaticB->{_Step}.' key1='.$sustaticB->{_key1};

	} else { 
		print("sustaticB, key1, missing key1,\n");
	 }
 }
...
=cut
 ...
 sub sstat {
	...
	my ( $self,$sstat )		= @_;
	if ( $sstat ne $empty_string ) {

		$sustaticB->{_sstat}		= $sstat;
		$sustaticB->{_note}		= $sustaticB->{_note}.' sstat='.$sustaticB->{_sstat};
		$sustaticB->{_Step}		= $sustaticB->{_Step}.' sstat='.$sustaticB->{_sstat};

	} else { 
		print("sustaticB, sstat, missing sstat,\n");
	 }
 }
...
=cut
 ...
 sub tstat {
	...
	my ( $self,$tstat )		= @_;
	if ( $tstat ne $empty_string ) {

		$sustaticB->{_tstat}		= $tstat;
		$sustaticB->{_note}		= $sustaticB->{_note}.' tstat='.$sustaticB->{_tstat};
		$sustaticB->{_Step}		= $sustaticB->{_Step}.' tstat='.$sustaticB->{_tstat};

	} else { 
		print("sustaticB, tstat, missing tstat,\n");
	 }
 }
