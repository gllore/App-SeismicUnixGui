...
=cut
 ...
 sub get_binding_length{
	...
	if ($suamp_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$suamp_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("suamp_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	if ($suamp_spec->{_file_dialog_type_aref} ) {
		...
		my $index_aref = $suamp_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("suamp_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($suamp_spec->{_flow_type_aref} ) {
			...
			my $index_aref = $suamp_spec->{_flow_type_aref};
			return($index_aref);
 ...
 sub get_incompatibles{
	...
	if ($len_1_needed >= 1) {
		...
		for (my $i=0; $i < $len_1_needed; $i++) {
			...
			print("suamp, get_incompatibles,need_only_1:  @{@{$params->{_need_only_1}}[$i]}\n");
...
=cut
 ...
 sub get_prefix_aref {
	...
	if ($suamp_spec->{_prefix_aref} ) {

		my $prefix_aref= $suamp_spec->{_prefix_aref};
		return($prefix_aref);

	} else {
		print("suamp_spec, get_prefix_aref, missing prefix_aref\n");
		return();
	}
...
=cut
 ...
 sub get_suffix_aref {
	...
	if ($suamp_spec->{_suffix_aref} ) {

			my $suffix_aref= $suamp_spec->{_suffix_aref};
			return($suffix_aref);

	} else {
			print("$suamp_spec, get_suffix_aref, missing suffix_aref\n");
			return();
	}
...
=cut
 ...
 sub prefix_aref {
	...
	$suamp_spec ->{_prefix_aref} = \@prefix;
	return();
...
=cut
 ...
 sub suffix_aref {
	...
	$suamp_spec ->{_suffix_aref} = \@suffix;
	return();
