...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SUAMP - output amp, phase, real or imag trace from			
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub zeromean {
	...
	if ( $zeromean ne $empty_string ) {
		...
		$suamp->{_note}		= $suamp->{_note}.' zeromean='.$suamp->{_zeromean};
		$suamp->{_Step}		= $suamp->{_Step}.' zeromean='.$suamp->{_zeromean};
