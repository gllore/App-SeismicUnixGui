...
=head2 SYNOPSIS
 ...
 PACKAGE NAME:  SUSPECFX - Fourier SPECtrum (T -> F) of traces 		
 AUTHOR: Juan Lorenzo
 DATE:   
 DESCRIPTION:
 Version: 
...
=cut
 ...
 sub NULL {
	...
	if ( $NULL ne $empty_string ) {
		...
		$suspecfx->{_NULL}		= $NULL;
		$suspecfx->{_note}		= $suspecfx->{_note}.' NULL='.$suspecfx->{_NULL};
		$suspecfx->{_Step}		= $suspecfx->{_Step}.' NULL='.$suspecfx->{_NULL};
