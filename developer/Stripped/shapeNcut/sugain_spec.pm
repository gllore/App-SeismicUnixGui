...
=cut
 ...
 sub get_binding_length{
	...
	if ($sugain_spec->{_binding_index_aref} ) {
		...
		my $binding_length= scalar @{$sugain_spec->{_binding_index_aref}};
		return($binding_length);

	} else {
		print("sugain_spec, get_binding_length, missing binding_length\n");
		return();
	}
...
=cut
 ...
 sub get_file_dialog_type_aref{
	...
	if ($sugain_spec->{_file_dialog_type_aref} ) {
		...
		my $index_aref = $sugain_spec->{_file_dialog_type_aref};
		return($index_aref);

	} else {
		print("sugain_spec, get_file_dialog_type_aref, missing get_file_dialog_type_aref\n");
		return();
	}
...
=cut
 ...
 sub get_flow_type_aref {
	...
	if ($sugain_spec->{_flow_type_aref} ) {
			...
			my $index_aref = $sugain_spec->{_flow_type_aref};
			return($index_aref);
